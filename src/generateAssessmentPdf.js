...
function composeNarrative(input) {
  ...
  return {
    scope:
      `We confirm that we have been instructed to provide a preliminary migration assessment in relation to a potential ${resolveMatter(input)} application. This assessment is based solely on the information presently supplied and is prepared for initial case screening, professional review, and client guidance before formal lodgement. It should not be treated as a substitute for a full documentary audit or final legal advice on a completed application package.`,
    background:
...
function buildStrengths(input) {
  ...
  if (normalizeBoolean(input.includePartner) === 'Yes') strengths.push('Secondary applicant information has been flagged for spouse or partner inclusion.');
...
function drawLogoFallback(doc, x, y) {
  ...
  doc.font('Helvetica-Bold').fontSize(18).fillColor(brand.colors.navy).text('BM', x, y + 20, { width: 74, align: 'center', lineBreak: false });
  doc.font('Helvetica').fontSize(7.8).fillColor(brand.colors.blue).text('Migration', x, y + 44, { width: 74, align: 'center', lineBreak: false });
  doc.restore();
}

function drawPageChrome(doc, pageNumber, totalPages, options = {}) {
  const marginX = 52;
  const left = marginX;
  const right = doc.page.width - marginX;
  const top = marginX - 12;
  ...
  if (options.logoPath && fs.existsSync(options.logoPath)) {
    try {
      doc.image(options.logoPath, left, top + 6, { fit: [78, 78], align: 'left', valign: 'center' });
    } catch (_) {
      drawLogoFallback(doc, left, top + 6);
    }
  } else {
    drawLogoFallback(doc, left, top + 6);
  }
  ...
  doc.font('Helvetica-Bold').fontSize(15).fillColor(colors.navy).text(brand.principal, headerTextX, top + 8, {
    width: 240,
    lineBreak: false
  });

  doc.font('Helvetica').fontSize(9).fillColor(colors.text).text(brand.credentials.join(' | '), headerTextX, top + 29, {
    width: 290,
    lineBreak: false
  });

  const contactX = right - 190;
  doc.font('Helvetica-Bold').fontSize(10.2).fillColor(colors.blue).text(brand.firmName, contactX, top + 8, {
    width: 190,
    align: 'right',
    lineBreak: false
  });
  doc.font('Helvetica').fontSize(8.6).fillColor(colors.text);
  doc.text(brand.website, contactX, top + 26, { width: 190, align: 'right', lineBreak: false, ellipsis: true });
  doc.text(brand.email, contactX, top + 38, { width: 190, align: 'right', lineBreak: false, ellipsis: true });
  doc.text(brand.phone, contactX, top + 50, { width: 190, align: 'right', lineBreak: false });
  doc.text(brand.address, contactX, top + 62, { width: 190, height: 18, align: 'right', ellipsis: true });

  doc.moveTo(left, top + 90).lineTo(right, top + 90).lineWidth(1.2).strokeColor(colors.gold).stroke();

  const footerY = doc.page.height - marginX + 8;
  ...
  doc.font('Helvetica').fontSize(8).fillColor(colors.muted).text(
    'This preliminary assessment is subject to documentary verification and final legal review.',
    left,
    footerY - 4,
    { width: 330, lineBreak: false, ellipsis: true }
  );
  doc.text(`Page ${pageNumber} of ${totalPages}`, right - 80, footerY - 4, {
    width: 80,
    align: 'right',
    lineBreak: false
  });
...
async function generateAssessmentPdf({ assessment, outputPath, logoPath, title } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const builder = new AssessmentPdfBuilder(doc);
    ...
    for (let i = range.start; i < range.start + range.count; i += 1) {
      doc.switchToPage(i);
      drawPageChrome(doc, i - range.start + 1, range.count, { logoPath });
    }
