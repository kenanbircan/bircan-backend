...
function writeKnowledgebaseEnforcementRecord(doc, adviceBundle) {
  ...
  const audit = adviceBundle && (adviceBundle.criteriaRegistryAudit || adviceBundle.grantCriteriaCoverageAudit);
  if (audit && audit.pdfIssuedWithReviewWarning) {
    writeSubheading(doc, 'Grant criteria coverage review note');
    writePara(doc, cleanText(audit.warning || 'Grant criteria registry coverage is incomplete. This letter has been issued as preliminary professional advice and requires further migration-agent review before lodgement-ready advice is given.'), { size: 9.4 });
    const missingCount = Array.isArray(audit.missingRegistryCriteria) ? audit.missingRegistryCriteria.length : Array.isArray(audit.unsupportedSourceCriteria) ? audit.unsupportedSourceCriteria.length : 0;
    const rate = audit.registryCoverageRate === undefined || audit.registryCoverageRate === null ? 'not confirmed' : `${audit.registryCoverageRate}%`;
    writePara(doc, `Registry coverage audit: ${rate}. Items requiring review: ${missingCount}. This is not a refusal of service; it is a professional limitation disclosure and file-control warning.`, { size: 9.2 });
  }
