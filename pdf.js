...
function makeCriterionNarrative(item) {
  ...
  const recommendation = premiumPhrase(item.recommendation || '', 'Obtain supporting evidence and reassess this issue before final advice is issued.');
...
function addPageFooter(doc) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    const pageNo = i + 1;
    doc.save();
    doc.rect(0, 800, 595.28, 42).fill(BRAND.navy);
    doc.fillColor('#ffffff').font('Helvetica').fontSize(7.5)
      .text('Bircan Migration & Education | Preliminary migration assessment | Subject to professional review and verification', 50, 813, { width: 410, lineBreak: false });
    doc.text(`Page ${pageNo}`, 500, 813, { width: 45, align: 'right' });
    doc.restore();
  }
}
...
function buildPathwayRows(adviceBundle, stream) {
  const rows = [];
  const comparison = adviceBundle.pathwayComparison || {};
  const raw = comparison.pathways || comparison.results || comparison.items || [];
  for (const item of ensureArray(raw)) {
    if (item && typeof item === 'object') {
      rows.push([
        item.name || item.pathway || item.label || 'Pathway',
        premiumPhrase(item.position || item.status || item.recommendation || 'Further review required'),
        premiumPhrase(item.strength || item.mainStrength || item.positive || 'Potential evidence pathway'),
        premiumPhrase(item.concern || item.mainIssue || item.issue || 'Evidence verification')
      ]);
    }
  }
  if (!rows.length) {
    rows.push(['186 TRT', stream === 'Temporary Residence Transition' ? 'Primary pathway for review' : 'Alternative ENS stream to consider', 'Employer continuity may assist if verified', 'Employment and sponsor continuity']);
    rows.push(['186 Direct Entry', stream === 'Direct Entry' ? 'Primary pathway for review' : 'Alternative ENS stream to consider', 'Occupation and skills evidence may assist', 'Skills assessment, occupation and experience']);
    rows.push(['186 Labour Agreement', stream === 'Labour Agreement' ? 'Primary pathway for review' : 'Only available if agreement terms apply', 'Concessions may assist where available', 'Agreement coverage and employer compliance']);
    rows.push(['482 / 494', 'Alternative employer-sponsored pathway observation', 'May remain available depending on sponsor and role', 'Temporary/regional nomination requirements']);
  }
...
function buildAssessmentPdfBuffer(assessment, adviceBundle) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      premiumParagraph(doc, `Based on the currently available information, the matter appears to require further detailed professional review before any lodgement strategy is finalised. The immediate priority is to verify the supporting documentation, clarify ${primaryIssue.toLowerCase()}, and confirm the applicable legislative, instrument and policy requirements at the relevant time.

This report should be treated as a preliminary professional assessment product. It is suitable for identifying evidence priorities and preparing the matter for migration-agent review, but it is not a substitute for final legal advice based on original documents and current law.`);
