...
export async function generateAssessmentPdf(submission) {
  const outputDir = process.env.PDF_OUTPUT_DIR || "./data/pdfs";
  ...
  doc.text(`Client Name: ${safe(submission?.client?.fullName)}`);
  doc.text(`Client Email: ${safe(submission?.client?.email)}`);
  doc.text(`Visa Subclass: ${safe(submission?.visa?.subclass)}`);
  doc.text(`Selected Plan: ${safe(submission?.visa?.selectedPlan)}`);
  doc.moveDown();
  ...
  const addSection = (title, content) => {
    ...
    if (Array.isArray(content)) {
      if (!content.length) {
        doc.fontSize(10).text("No items noted.");
      } else {
        content.forEach((item, idx) => {
          doc.fontSize(10).text(`${idx + 1}. ${safe(item)}`);
        });
      }
    } else {
  ...
  addSection("Missing Information", analysis.missingInformation || []);
  addSection("Recommended Next Steps", analysis.nextSteps || []);
  addSection("Client Letter Draft", analysis.clientLetterDraft || "");
