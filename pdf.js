...
function writeFooter(doc) {
  // Keep footer above bottom margin. Writing inside the margin can make PDFKit
  // auto-create blank footer-only pages.
  const range = doc.bufferedPageRange();
  const originalIndex = Math.max(range.start, range.start + range.count - 1);
  for (let i = range.start; i < range.start + range.count; i++) {
    doc.switchToPage(i);
    const y = doc.page.height - doc.page.margins.bottom - 12;
    doc.font('Helvetica').fontSize(7.5).fillColor('#667085')
      .text('Generated by Bircan Migration client portal. Preliminary advice only and subject to evidence verification.', 50, y, {
        width: 390,
        lineBreak: false
      });
    doc.text("Page " + (i - range.start + 1) + " of " + range.count, 470, y, {
      width: 75,
      align: 'right',
      lineBreak: false
    });
  }
  doc.switchToPage(originalIndex);
}
