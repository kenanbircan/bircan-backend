...
export function createSubmission(payload) {
  ...
  db.submissions[id] = {
    ...
    checkoutKey: payload?.checkoutKey || payload?.metadata?.checkoutKey || null,
    client: {
...
export function listSubmissionsByUser(user, limit = 50) {
  ...
  return Object.values(db.submissions || {})
    .filter((item) => {
      const itemUserId = String(item?.userId || item?.metadata?.userId || item?.metadata?.accountUserId || '').trim();
      const itemEmail = String(item?.client?.email || item?.stripeCustomerEmail || item?.metadata?.accountEmail || item?.metadata?.clientEmail || '').trim().toLowerCase();
      return (userId && itemUserId === userId) || (email && itemEmail === email);
    })
    .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
    .slice(0, limit);
}

export function findSubmissionByCheckoutKey(checkoutKey) {
  const key = String(checkoutKey || '').trim();
  if (!key) return null;
  const db = loadDb();
  return Object.values(db.submissions || {}).find((item) => String(item?.metadata?.checkoutKey || item?.checkoutKey || '') === key) || null;
}

export function listSubmissionsByCheckoutKey(checkoutKey) {
  const key = String(checkoutKey || '').trim();
  if (!key) return [];
  const db = loadDb();
  return Object.values(db.submissions || {}).filter((item) => String(item?.metadata?.checkoutKey || item?.checkoutKey || '') === key);
}
