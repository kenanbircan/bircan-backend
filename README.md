...
- Prevents page-loop style duplication by using a single controlled page decoration handler
...
- using one `pageAdded` handler only
- keeping all page decorations inside a single builder class
- never recursively calling page creation from header/footer rendering
- keeping content flow and page breaks separate from the decoration logic
