 ...
 *   -> legal satisfaction based on declared facts
 *   -> separate evidence status based on documents
 *   -> points/stream/validity checks
 *   -> advisory outcome with conditions
 *   -> GPT-safe advice bundle
 *
 * GPT must only draft wording from this bundle. It must not change legal outcome.
 */

const ENGINE_VERSION = "11.0.0-dual-layer-advisory";
...
const STATUS = Object.freeze({
  ...
  FAIL: "FAIL",
  UNKNOWN: "UNKNOWN",
  PROVISIONAL: "PROVISIONALLY_SATISFIED",
  RISK: "RISK"
});
...
function buildEvidenceIndex(assessment, payload, flat) {
  ...
  return {
    ...
    status(types, answerKeys = []) {
      ...
      const rawAnswer = answerKeys.length ? pick(flat, answerKeys) : null;
      const answerYes = answerKeys.length ? yes(flat, answerKeys) : false;
      const answerNo = answerKeys.length ? no(flat, answerKeys) : false;
      const answerProvided = rawAnswer !== null && rawAnswer !== undefined && str(rawAnswer) !== "";
      const today = new Date();

      if (doc) {
        if (doc.expiryDate && doc.expiryDate < today) return { status: STATUS.FAIL, legalStatus: STATUS.FAIL, evidenceStatus: "EXPIRED", doc, reason: "document expired" };
        if (doc.verified === false) return { status: STATUS.PROVISIONAL, legalStatus: STATUS.PROVISIONAL, evidenceStatus: "UNVERIFIED", doc, reason: "document uploaded but not verified" };
        return { status: STATUS.PASS, legalStatus: STATUS.PASS, evidenceStatus: "VERIFIED", doc, reason: "document available" };
      }
      if (answerNo) return { status: STATUS.FAIL, legalStatus: STATUS.FAIL, evidenceStatus: "MISSING", doc: null, reason: "negative questionnaire answer" };
      if (answerYes || answerProvided) return { status: STATUS.PROVISIONAL, legalStatus: STATUS.PROVISIONAL, evidenceStatus: "DECLARED_NOT_VERIFIED", doc: null, reason: "declared or recorded in questionnaire but no matching document verified" };
      return { status: STATUS.UNKNOWN, legalStatus: STATUS.UNKNOWN, evidenceStatus: "NOT_PROVIDED", doc: null, reason: "no verified evidence or clear questionnaire answer" };
    }
  };
}
...
function criterion({ id, criterion, evidenceTypes = [], answerKeys = [], severity, effect, legalSource, missing = [], consequence, recommendation, forceStatus = null, facts = [] }, ctx) {
  ...
  return {
    ...
    criterion,
    status,
    legalSatisfaction: ev.legalStatus || status,
    evidenceStatus: ev.evidenceStatus || (ev.doc ? "VERIFIED" : "NOT_PROVIDED"),
    severity,
    legalEffect: effect,
    legalSource,
...
function findingText(status) {
  ...
  if (status === STATUS.PROVISIONAL) return "Provisionally satisfied on the declared information, subject to documentary verification before lodgement.";
  ...
  return "Unable to determine because the questionnaire answer and evidence position are insufficient.";
}
...
function recommendationText(status, effect) {
  ...
  if (status === STATUS.PROVISIONAL) return "Request and verify supporting documents before lodgement; do not treat this as final advice until verified.";
...
function commonChecks(ctx) {
  const f = ctx.flat;
  const healthAdverse = yes(f, ["healthIssue","medicalIssue","healthConcern"]);
  const healthClear = yes(f, ["healthMet"]) || no(f, ["healthIssue","medicalIssue","healthConcern"]);
  const characterAdverse = yes(f, ["characterIssue","criminalHistory","conviction"]);
  const characterClear = yes(f, ["characterMet"]) || no(f, ["characterIssue","criminalHistory","conviction"]);
  const integrityAdverse = yes(f, ["pic4020","falseDocument","misleadingInformation","integrityIssue"]);
  const integrityClear = no(f, ["pic4020","falseDocument","misleadingInformation","integrityIssue"]);
  return [
    criterion({
      id: "HEALTH",
      ...
      forceStatus: healthAdverse ? STATUS.RISK : healthClear ? STATUS.PROVISIONAL : null
    }, ctx),
    criterion({
      id: "CHARACTER",
      ...
      forceStatus: characterAdverse ? STATUS.RISK : characterClear ? STATUS.PROVISIONAL : null
    }, ctx),
    criterion({
      id: "PIC4020",
      ...
      forceStatus: integrityAdverse ? STATUS.RISK : integrityClear ? STATUS.PROVISIONAL : null
    }, ctx)
  ];
}
...
function classify(findings) {
  ...
  const provisional = findings.find(f => f.status === STATUS.PROVISIONAL);
  const unknown = findings.find(f => f.status === STATUS.UNKNOWN);

  // V2 rule: missing documents alone do not equal legal failure. A declared positive answer is treated as
  // provisional legal satisfaction and separated from evidence verification.
  ...
  if (blockerUnknown) return { decisionStatus: "INFORMATION_REQUIRED", lodgementPosition: "NOT_READY_INFORMATION_REQUIRED", riskLevel: "HIGH", legalStatus: EFFECT.EVIDENCE, primaryReason: blockerUnknown.criterion };
  if (unknown) return { decisionStatus: "INFORMATION_REQUIRED", lodgementPosition: "NOT_READY_INFORMATION_REQUIRED", riskLevel: "MEDIUM", legalStatus: EFFECT.EVIDENCE, primaryReason: unknown.criterion };
  if (provisional) return { decisionStatus: "PROVISIONALLY_SATISFIED_SUBJECT_TO_EVIDENCE", lodgementPosition: "POTENTIALLY_LODGEABLE_SUBJECT_TO_EVIDENCE", riskLevel: "MEDIUM", legalStatus: "PROVISIONALLY_ELIGIBLE_SUBJECT_TO_DOCUMENTARY_VERIFICATION", primaryReason: provisional.criterion };
...
function finalise(ctx, findings, extras = {}) {
  ...
  const blockers = findings.filter(f => f.severity === SEVERITY.BLOCKER && [STATUS.FAIL, STATUS.UNKNOWN, STATUS.RISK].includes(f.status));
  const provisionalFindings = findings.filter(f => f.status === STATUS.PROVISIONAL);
...
function buildNextSteps(outcome, evidenceRequired) {
  ...
  if (outcome.lodgementPosition.includes("POTENTIALLY_LODGEABLE_SUBJECT_TO_EVIDENCE")) steps.push("Treat the matter as potentially viable only after documentary verification and registered migration agent review.");
  if (outcome.lodgementPosition.includes("NOT_READY_INFORMATION_REQUIRED")) steps.push("Obtain missing client instructions before forming final prospects advice.");
  ...
  if (evidenceRequired.length) steps.push("Collect, verify and retain the listed evidence before final advice or lodgement.");
  steps.push("Regenerate the advice only after the evidence or instruction position changes.");
  return unique(steps);
}
function buildQualityFlags(ctx, findings) {
  ...
  if (findings.some(f => f.status === STATUS.UNKNOWN)) flags.push("One or more criteria are undetermined because instructions or evidence are incomplete.");
  if (findings.some(f => f.status === STATUS.PROVISIONAL)) flags.push("One or more criteria are provisionally satisfied on declared facts but require documentary verification.");
...
function buildLegalEngineBundle(decision, assessment) {
  ...
  const bundle = {
    ...
    criterionFindings: (decision.findings || []).map(f => ({
      ...
      status: f.status,
      legalSatisfaction: f.legalSatisfaction || f.status,
      evidenceStatus: f.evidenceStatus || null,
      finding: f.finding,
...
function toPdfAdviceBundle(adviceBundle, decision, assessment) {
  const bundle = adviceBundle && typeof adviceBundle === 'object' ? adviceBundle : {};
  if (bundle.advice && Array.isArray(bundle.advice.criterion_findings)) return bundle;

  const findings = Array.isArray(bundle.criterionFindings) ? bundle.criterionFindings : Array.isArray(decision && decision.findings) ? decision.findings : [];
  const criterion_findings = findings.map((f) => ({
    criterion: f.criterion || f.heading || f.title || 'Criterion',
    finding: f.finding || f.status || 'Evidence required / not verified.',
    legal_consequence: f.legalConsequence || f.legalEffect || 'Further legal review required before lodgement.',
    evidence_gap: f.evidenceGap || (Array.isArray(f.evidenceMissing) ? f.evidenceMissing.join('; ') : '') || 'Supporting evidence required.',
    recommendation: f.recommendation || 'Obtain and verify supporting evidence before proceeding.'
  }));

  const sections = Array.isArray(bundle.sections) && bundle.sections.length ? bundle.sections.map(s => ({
    heading: s.heading || s.title || 'Assessment section',
    body: s.body || s.text || s.content || ''
  })) : [
    { heading: 'Scope and basis of preliminary advice', body: bundle.summary || bundle.executiveSummary || 'This preliminary migration assessment is based on questionnaire information and available evidence metadata. It is subject to review of original documents and current law and policy.' },
    { heading: 'Delegate-simulator outcome', body: `Decision status: ${bundle.decisionStatus || (decision && decision.decisionStatus) || 'EVIDENCE_REQUIRED'}. Risk level: ${bundle.riskLevel || (decision && decision.riskLevel) || 'HIGH'}. Lodgement position: ${bundle.lodgementPosition || (decision && decision.lodgementPosition) || 'EVIDENCE REQUIRED BEFORE LODGEMENT'}. Primary reason: ${bundle.primaryReason || (decision && decision.primaryReason) || 'Evidence not verified'}.` },
    { heading: 'Evidence and document verification', body: 'Questionnaire answers are not treated as verified evidence. Supporting documents must be reviewed and retained on file before final advice or lodgement action.' },
  ...
  const evidence_required = Array.from(new Set([].concat(bundle.evidenceRequired || [], bundle.evidence_required || [], findings.map(f => f.evidenceGap || (Array.isArray(f.evidenceMissing) ? f.evidenceMissing.join('; ') : '')).filter(Boolean))));
  const client_next_steps = Array.from(new Set([].concat(bundle.nextSteps || [], bundle.recommendedNextSteps || [], [
  ...
  return {
    ...bundle,
    advice: {
      title: bundle.title || `Preliminary Migration Advice – Subclass ${(assessment && assessment.visa_type) || (decision && decision.subclass) || ''}`,
      subclass: bundle.subclass || (assessment && assessment.visa_type) || (decision && decision.subclass),
      risk_level: bundle.risk_level || bundle.riskLevel || (decision && decision.riskLevel) || 'HIGH',
      lodgement_position: bundle.lodgement_position || bundle.lodgementPosition || (decision && decision.lodgementPosition) || 'EVIDENCE REQUIRED BEFORE LODGEMENT',
      sections,
      criterion_findings: criterion_findings.length ? criterion_findings : [{ criterion: 'Evidence validation', finding: 'Evidence required / not verified.', legal_consequence: 'Legal review required before lodgement.', evidence_gap: 'Supporting evidence required.', recommendation: 'Obtain and verify evidence.' }],
      evidence_required,
      client_next_steps,
      disclaimer: bundle.disclaimer || 'This preliminary advice is based only on questionnaire answers and available evidence metadata. Final advice requires review of original documents and confirmation of current law, instruments, policy and Department requirements at the relevant time.'
    },
    facts: bundle.facts || { applicant: { name: assessment && assessment.applicant_name, email: assessment && assessment.applicant_email } },
    gptAdviceBundle: bundle.gptAdviceBundle || { role: 'drafting_only', controlledBy: 'migrationDecisionEngine', cannotOverrideRules: true }
  };
}

function supportedDelegateSimulatorSubclasses() {
  return Array.isArray(SUPPORTED_SUBCLASSES) ? SUPPORTED_SUBCLASSES.slice() : [];
}
...
function buildDelegateSimulatorPdfInputs(assessment) {
  ...
  const rawBundle = buildLegalEngineBundle(decision, assessment || {});
  const adviceBundle = toPdfAdviceBundle(rawBundle, decision, assessment || {});
  return {
    decision,
    adviceBundle,
    bundle: adviceBundle,
