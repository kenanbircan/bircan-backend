...
// ---- Paid citizenship exam compatibility routes ----
// The paid exam page calls POST /api/exam/start. Older backend builds only had
// citizenship access/status routes, so the paid exam button could not activate.
// This route resolves the authenticated client's latest paid citizenship access
// and returns a stable exam session payload without creating a duplicate purchase.
async function resolvePaidCitizenshipAccessForExam(req) {
  ...
  const params = [req.client.email, req.client.id];
  let where = `(lower(client_email)=lower($1) OR client_id=$2)`;
  ...
  const { rows } = await query(
    `SELECT id, client_id, client_email, selected_plan, active_plan,
            COALESCE(exam_allowance, 20) AS exam_allowance,
            COALESCE(attempts_used, 0) AS attempts_used,
            GREATEST(0, COALESCE(exam_allowance, 20) - COALESCE(attempts_used, 0)) AS attempts_remaining,
            status, payment_status, stripe_session_id, amount_cents, currency, created_at, updated_at
     FROM citizenship_access
     WHERE ${where}
       AND (payment_status='paid' OR status='active')
     ORDER BY GREATEST(0, COALESCE(exam_allowance, 20) - COALESCE(attempts_used, 0)) DESC,
  ...
  const access = rows[0];
  if (!access) {
    const err = new Error('No active paid citizenship exam access was found for this account. Please choose a plan and complete payment first.');
    err.statusCode = 402;
    err.code = 'CITIZENSHIP_ACCESS_REQUIRED';
    throw err;
  }
  if (Number(access.attempts_remaining || 0) <= 0) {
    const err = new Error('This citizenship test pack has no remaining attempts. Please purchase another test pack.');
    err.statusCode = 402;
    err.code = 'CITIZENSHIP_ATTEMPTS_EXHAUSTED';
    throw err;
  }
  return access;
}

async function startPaidCitizenshipExam(req, res) {
  const access = await resolvePaidCitizenshipAccessForExam(req);
  const plan = normaliseCitizenshipPlan(access.active_plan || access.selected_plan || '20');
  const examSessionId = `exam_${Date.now()}_${access.id}_${Math.random().toString(16).slice(2, 8)}`;
  res.json({
    ok: true,
    ...
    citizenship_access_id: access.id,
    plan,
    selectedPlan: plan,
    activePlan: plan,
    examAllowance: Number(access.exam_allowance || citizenshipExamAllowance(plan)),
    exam_allowance: Number(access.exam_allowance || citizenshipExamAllowance(plan)),
    attemptsUsed: Number(access.attempts_used || 0),
    attempts_used: Number(access.attempts_used || 0),
    attemptsRemaining: Number(access.attempts_remaining || 0),
    attempts_remaining: Number(access.attempts_remaining || 0),
    stripeSessionId: access.stripe_session_id || null,
    stripe_session_id: access.stripe_session_id || null,
    message: 'Paid citizenship exam access verified.'
  });
}

app.post('/api/exam/start', requireAuth, asyncRoute(startPaidCitizenshipExam));
app.get('/api/exam/start', requireAuth, asyncRoute(startPaidCitizenshipExam));
app.post('/api/citizenship/exam/start', requireAuth, asyncRoute(startPaidCitizenshipExam));
app.post('/api/citizenship-test/exam/start', requireAuth, asyncRoute(startPaidCitizenshipExam));
app.get('/api/exam/status', requireAuth, asyncRoute(async (req, res) => {
  const access = await resolvePaidCitizenshipAccessForExam(req);
  const plan = normaliseCitizenshipPlan(access.active_plan || access.selected_plan || '20');
  res.json({
    ok: true,
