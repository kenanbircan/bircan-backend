...
function buildUnifiedServiceCard(row) {
  const serviceType = normaliseServiceType(row.service_type);
  ...
  const expectedAmountCents = (() => { try { return expectedAmountCentsForService(serviceType, plan); } catch (_err) { return null; } })();
  ...
  return {
    ...
    actionLabel,
    finalPdfUrl,
    amountCents: row.amount_cents || expectedAmountCents || null,
