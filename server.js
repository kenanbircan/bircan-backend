...
function dbServiceToDashboard(row) {
  ...
  return {
    ...
    downloadUrl: row.document_url || meta.documentUrl || meta.downloadUrl || null,
    stripeSessionId: row.stripe_session_id,
    amountTotal: row.amount_total,
    currency: row.currency,
    paidAt: row.paid_at ? new Date(row.paid_at).toISOString() : null,
...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null) {
  ...
  if (existingSubmission && existingSubmission.paymentStatus === 'paid') {
    ...
    const updatedExisting = Object.keys(patch).length ? await updateSubmission(existingSubmission.id, patch) : existingSubmission;
...
function publicDashboardAssessment(submission = {}) {
  ...
  return {
    ...
    clientName: submission.client?.fullName || submission.client?.name || submission.clientName || submission.fullName || submission.applicantName || null,
    applicantName: submission.client?.fullName || submission.client?.name || submission.clientName || submission.fullName || submission.applicantName || null,
    clientEmail: submission.client?.email || submission.clientEmail || submission.applicantEmail || submission.email || submission.accountEmail || submission.stripeCustomerEmail || null,
    email: submission.client?.email || submission.clientEmail || submission.applicantEmail || submission.email || submission.accountEmail || submission.stripeCustomerEmail || null,
    userId: submission.userId || null,
    plan: submission.plan || null,
    selectedPlan: (submission.plan && typeof submission.plan === 'object') ? (submission.plan.code || submission.plan.label || null) : (submission.selectedPlan || submission.assessmentPlan || submission.plan || null),
    activePlan: (submission.plan && typeof submission.plan === 'object') ? (submission.plan.code || submission.plan.label || null) : (submission.activePlan || submission.plan || null),
    ...
    amount: submission.stripeAmountTotal || null,
