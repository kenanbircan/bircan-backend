...
app.use(['/stripe/webhook', '/api/stripe/webhook'], express.raw({ type: 'application/json' }));
...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null, options = {}) {
  ...
  // Idempotency: Stripe webhooks can be retried. Never create a second
  // assessment for the same Checkout Session. Reuse the existing record and
  // only restart background processing if the PDF is still missing.
  const existingByStripeSession = (await getAllSubmissions()).find(item => String(item.stripeSessionId || '') === String(session.id || ''));
  if (existingByStripeSession) {
    if (options.finalize === false) {
      if (options.background !== false && (!existingByStripeSession.pdfPath || !fs.existsSync(existingByStripeSession.pdfPath))) {
        startPaidVisaSubmissionProcessing(existingByStripeSession.id);
      }
      return existingByStripeSession;
    }
    return finalizePaidVisaSubmissionForDashboard(existingByStripeSession.id);
  }
  ...
  // Robust production rule: checkout creates a pending submission before redirecting
  // to Stripe. The webhook or dashboard reconciliation then marks that exact
  // submission as paid. This prevents "No paid visa assessment found yet" when
  // a webhook is delayed, retried, or the temporary payload file is unavailable.
  if (existingSubmission) {
    const patched = await updateSubmission(existingSubmission.id, {
      paymentStatus: 'paid',
      status: existingSubmission.status === 'completed' ? existingSubmission.status : 'queued',
      productType: 'visa_assessment',
      paidAt: existingSubmission.paidAt || nowIso(),
      accountEmail: String(existingSubmission.accountEmail || metadata.email || session.customer_details?.email || session.customer_email || '').trim().toLowerCase() || null,
      userId: existingSubmission.userId || String(metadata.userId || '').trim() || null,
      stripeSessionId: session.id,
      stripePaymentIntentId: typeof session.payment_intent === 'string' ? session.payment_intent : (session.payment_intent?.id || null),
      stripeCustomerEmail: session.customer_details?.email || session.customer_email || metadata.email || existingSubmission.stripeCustomerEmail || null,
      stripeAmountTotal: session.amount_total || existingSubmission.stripeAmountTotal || null,
      stripeCurrency: session.currency || existingSubmission.stripeCurrency || null,
      analysisStatus: existingSubmission.analysisStatus || 'queued',
      pdfStatus: existingSubmission.pdfStatus || 'queued',
      emailStatus: existingSubmission.emailStatus || 'pending',
    });
    if (options.finalize === false) {
      if (options.background !== false && (!patched.pdfPath || !fs.existsSync(patched.pdfPath))) startPaidVisaSubmissionProcessing(patched.id);
      return patched;
    }
    return finalizePaidVisaSubmissionForDashboard(patched.id);
  }
...
app.post(['/create-visa-checkout-session', '/create-assessment-checkout-session', '/api/assessment/create-checkout-session', '/api/assessments/create-checkout-session', '/api/visa/create-checkout-session', '/api/checkout/visa'], async (req, res, next) => {
  try {
    ...
    // Create the assessment record before going to Stripe. It stays unpaid until
    // Stripe confirms payment, but it gives the webhook/reconciliation a durable
    // local record to mark as paid later.
    const pendingSubmissionId = safeId('sub');

    // Permanent production flow: Stripe webhook finalises payment.
    // The dashboard must not verify session_id or wait for PDF generation.
    // Keep account continuity with auth_return only; never expose session_id to the dashboard.
    const successUrlObj = new URL('/account-dashboard.html', APP_BASE_URL);
    successUrlObj.searchParams.set('checkout', 'success');
    successUrlObj.searchParams.set('auth_return', makeDashboardReturnToken(user));
    ...
    const extractedPending = extractAssessmentPayload({
      ...assessmentPayload,
      subclass,
      visaType: subclass,
      plan: plan.code,
    ...
    await saveSubmission({
      ...
      client: extractedPending.client,
      email: extractedPending.client?.email || user.email,
      clientEmail: extractedPending.client?.email || user.email,
      visaType: normaliseVisaType(subclass),
      subclass: normaliseVisaType(subclass),
      plan: plan.code,
      planLabel: plan.label,
      selectedPlan: plan.code,
      selectedPlanKey: plan.code,
      ...
      answers: extractedPending.answers,
    ...
    await updateSubmission(pendingSubmissionId, {
      stripeSessionId: session.id,
...
app.post('/create-checkout-session', async (req, res, next) => {
  try {
    ...
    if (!user) return res.status(401).json({ ok: false, error: 'Login required before Stripe checkout.' });
    // Permanent production flow: Stripe webhook creates the entitlement.
    // Return to the dashboard without session_id so the dashboard never loops.
    // Keep account continuity with auth_return only.
    const successUrlObj = new URL(STRIPE_SUCCESS_PATH || '/account-dashboard.html', APP_BASE_URL);
    successUrlObj.searchParams.set('checkout', 'success');
    successUrlObj.searchParams.set('plan', config.code);
    successUrlObj.searchParams.set('auth_return', makeDashboardReturnToken(user));
    const successUrl = successUrlObj.toString();
...
function publicDashboardAssessment(submission = {}) {
  ...
  const hasPdfOnDisk = Boolean(submission.pdfPath && fs.existsSync(submission.pdfPath));
  return {
    ...
    pdfStatus: hasPdfOnDisk ? 'generated' : (String(submission.paymentStatus || '').toLowerCase() === 'paid' ? 'available' : (submission.pdfStatus || null)),
    ...
    downloadUrl: submission.id ? `/api/assessment/${encodeURIComponent(submission.id)}/pdf` : (submission.pdfUrl || submission.downloadUrl || (submission.pdfFileName ? `/pdfs/${submission.pdfFileName}` : null)),
  };
}

async function reconcilePaidVisaSessionsForUser(user) {
  if (!stripe || !user) return { checked: false, createdOrUpdated: 0 };
  const userId = String(user.id || '').trim();
  const email = String(user.email || '').trim().toLowerCase();
  if (!userId && !email) return { checked: false, createdOrUpdated: 0 };

  let createdOrUpdated = 0;
  try {
    // Safety net only. Stripe webhooks remain the primary payment finalisation path.
    // This repairs cases where Stripe succeeded but the webhook was not received,
    // was misconfigured, or Render restarted before the webhook processed.
    const sessions = await stripe.checkout.sessions.list({ limit: 50 });
    for (const session of sessions.data || []) {
      const meta = session.metadata || {};
      if (String(meta.product || '') !== 'visa_assessment') continue;
      const paid = session.payment_status === 'paid' || session.status === 'complete';
      if (!paid) continue;

      const sessionUserId = String(meta.userId || '').trim();
      const sessionEmail = String(meta.email || session.customer_email || session.customer_details?.email || '').trim().toLowerCase();
      const belongsToUser = (userId && sessionUserId === userId) || (email && sessionEmail === email);
      if (!belongsToUser) continue;

      const expanded = await stripe.checkout.sessions.retrieve(session.id, { expand: ['line_items.data.price', 'payment_intent'] });
      const payloadToken = String(expanded.metadata?.payloadToken || '').trim();
      const payload = payloadToken ? readJsonSafe(path.join(STORAGE_DIR, 'visa-checkout-payloads', `${sanitizeFileName(payloadToken)}.json`), null) : null;
      const updated = await createVisaAssessmentFromPaidSession(expanded, payload, { finalize: false, background: true });
      if (updated) createdOrUpdated += 1;
    }
  } catch (error) {
    console.error('[dashboard-stripe-reconcile-error]', error.message || error);
  }
  return { checked: true, createdOrUpdated };
}
...
app.get('/api/account/dashboard', requireCitizenshipAuth, async (req, res, next) => {
  try {
    // Permanent production rule: the dashboard is read-only account data.
    // Stripe payment finalisation is handled by /api/stripe/webhook or /stripe/webhook.
    // PDF generation is background work and must never block dashboard loading.
    const entitlement = findUserEntitlement(req.user.id);
    const stripeReconciliation = await reconcilePaidVisaSessionsForUser(req.user);
    ...
    return res.json({
      ...
      citizenship: { entitlement: publicEntitlement(entitlement) },
      visaAssessments,
...
app.get('/api/assessment/:submissionId/pdf', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    if (!readySubmission.pdfPath || !fs.existsSync(readySubmission.pdfPath)) {
      // Final production rule: a paid assessment must not remain stuck at "PDF: generating".
      // Generate synchronously on first download if the background processor has not completed it.
      readySubmission = await finalizePaidVisaSubmissionForDashboard(readySubmission.id);
    }

    if (!readySubmission.pdfPath || !fs.existsSync(readySubmission.pdfPath)) {
      const pdfResult = await generateProfessionalPdf(readySubmission);
      readySubmission = await markSubmissionStage(readySubmission.id, {
        paymentStatus: 'paid',
        status: 'completed',
        pdfStatus: 'generated',
        pdfPath: pdfResult.pdfPath,
    ...
    return res.download(readySubmission.pdfPath, path.basename(readySubmission.pdfPath));
  } catch (error) {
    next(error);
  }
});
