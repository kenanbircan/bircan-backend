...
async function ensureSchema() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS citizenship_exam_attempts (
      id text PRIMARY KEY,
      access_id text REFERENCES citizenship_access(id) ON DELETE CASCADE,
      client_id uuid REFERENCES clients(id) ON DELETE SET NULL,
      client_email text NOT NULL,
      plan text,
  ...
  const updated = await tx(async (client) => {
    const inserted = await client.query(
      `INSERT INTO citizenship_exam_attempts
       ...
       RETURNING *`,
      [attemptId, access.id, req.client.id, req.client.email, plan, result.score, result.totalQuestions || questions.length, result.valuesCorrect, result.valuesTotal, result.passed, Boolean(req.body && req.body.timedOut), JSON.stringify(questions), JSON.stringify(answers), JSON.stringify(result)]
    );
