...
const STRIPE_PRICE_CITIZENSHIP_20 = process.env.STRIPE_PRICE_CITIZENSHIP_20 || '';
const STRIPE_PRICE_CITIZENSHIP_50 = process.env.STRIPE_PRICE_CITIZENSHIP_50 || '';
const STRIPE_PRICE_CITIZENSHIP_100 = process.env.STRIPE_PRICE_CITIZENSHIP_100 || '';
const STRIPE_PRICE_CITIZENSHIP_UNLIMITED = process.env.STRIPE_PRICE_CITIZENSHIP_UNLIMITED || '';
const CITIZENSHIP_SUCCESS_URL = String(process.env.CITIZENSHIP_SUCCESS_URL || `${APP_BASE_URL}/citizenship-test.html`).trim();
const CITIZENSHIP_CANCEL_URL = String(process.env.CITIZENSHIP_CANCEL_URL || CITIZENSHIP_SUCCESS_URL).trim();
...
const CITIZENSHIP_ACCESS_DIR = path.join(STORAGE_DIR, 'citizenship-access');
...
const CITIZENSHIP_PLANS = {
  ...
  plan_20: {
    ...
    attemptsAllowed: 20,
    paid: true,
    priceId: STRIPE_PRICE_CITIZENSHIP_20,
  },
  plan_50: {
    code: 'plan_50',
    label: '50 Full Tests',
    attemptsAllowed: 50,
    paid: true,
    priceId: STRIPE_PRICE_CITIZENSHIP_50,
  },
  plan_100: {
    code: 'plan_100',
    label: '100 Full Tests',
    attemptsAllowed: 100,
    paid: true,
    priceId: STRIPE_PRICE_CITIZENSHIP_100,
  },
  unlimited: {
    code: 'unlimited',
    label: 'Unlimited Full Tests',
    attemptsAllowed: null,
    paid: true,
    priceId: STRIPE_PRICE_CITIZENSHIP_UNLIMITED,
  },
};
...
[STORAGE_DIR, SUBMISSIONS_DIR, PDF_DIR, PUBLIC_DIR, KNOWLEDGEBASE_DIR, CITIZENSHIP_ACCESS_DIR].forEach(ensureDirSync);
...
function normaliseCitizenshipPlan(input) {
  ...
  const aliases = {
    ...
    '20': 'plan_20',
    '20 tests': 'plan_20',
    '20 full tests': 'plan_20',
    plan_50: 'plan_50',
    '50': 'plan_50',
    '50 tests': 'plan_50',
    '50 full tests': 'plan_50',
    plan_100: 'plan_100',
    '100': 'plan_100',
    '100 tests': 'plan_100',
    '100 full tests': 'plan_100',
    unlimited: 'unlimited',
    'unlimited tests': 'unlimited',
    'unlimited full tests': 'unlimited',
  };
  return CITIZENSHIP_PLANS[aliases[raw]] || CITIZENSHIP_PLANS.free;
}

function citizenshipAccessPathForEmail(email) {
  return path.join(CITIZENSHIP_ACCESS_DIR, `${hashEmail(email)}.json`);
}

function citizenshipSessionIndexPath(sessionId) {
  return path.join(CITIZENSHIP_ACCESS_DIR, `session_${sanitizeFileName(sessionId)}.json`);
}

async function getCitizenshipAccessByEmail(email) {
  if (!email) return null;
  return readJsonSafe(citizenshipAccessPathForEmail(email), null);
}

async function getCitizenshipAccessBySessionId(sessionId) {
  if (!sessionId) return null;
  const index = readJsonSafe(citizenshipSessionIndexPath(sessionId), null);
  if (index?.email) return getCitizenshipAccessByEmail(index.email);
  return null;
}

async function saveCitizenshipAccess(record) {
  if (!record?.email) throw new Error('Citizenship access record requires an email.');
  const cleanEmail = String(record.email || '').trim().toLowerCase();
  const next = {
    createdAt: nowIso(),
    updatedAt: nowIso(),
    status: 'active',
  ...
  const existing = readJsonSafe(citizenshipAccessPathForEmail(cleanEmail), null);
  if (existing?.createdAt && !record.createdAt) next.createdAt = existing.createdAt;
  next.updatedAt = nowIso();
  await writeJson(citizenshipAccessPathForEmail(cleanEmail), next);
  if (next.stripeSessionId) {
    await writeJson(citizenshipSessionIndexPath(next.stripeSessionId), {
      email: cleanEmail,
      stripeSessionId: next.stripeSessionId,
      updatedAt: nowIso(),
    });
  }
  return next;
...
function getCitizenshipAttemptsRemaining(record) {
  if (!record) return 0;
  if (record.attemptsAllowed === null) return null;
  const allowed = Number(record.attemptsAllowed || 0);
  const used = Number(record.attemptsUsed || 0);
  return Math.max(0, allowed - used);
}

function buildCitizenshipAccessResponse(record) {
  if (!record) {
    return {
      ok: true,
      planCode: 'free',
      planLabel: CITIZENSHIP_PLANS.free.label,
      paid: false,
      status: 'inactive',
      attemptsAllowed: CITIZENSHIP_PLANS.free.attemptsAllowed,
      attemptsUsed: 0,
      attemptsRemaining: CITIZENSHIP_PLANS.free.attemptsAllowed,
      email: null,
      stripeSessionId: null,
      paidAt: null,
    };
  }
  const plan = normaliseCitizenshipPlan(record.planCode);
  return {
    ok: true,
    planCode: plan.code,
    ...
    attemptsAllowed: record.attemptsAllowed === undefined ? plan.attemptsAllowed : record.attemptsAllowed,
    attemptsUsed: Number(record.attemptsUsed || 0),
    attemptsRemaining: getCitizenshipAttemptsRemaining(record),
...
function requireStripeConfigured() {
  if (!stripe) {
    const error = new Error('Stripe is not configured. Install the stripe package and set STRIPE_SECRET_KEY.');
...
app.post('/api/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  if (!STRIPE_WEBHOOK_SECRET) return res.status(500).send('Stripe webhook secret is not configured.');
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      const plan = normaliseCitizenshipPlan(session.metadata?.planCode);

      if (email && plan.paid) {
        const existing = await getCitizenshipAccessByEmail(email);
        const nextRecord = {
          id: existing?.id || safeId('cit_access'),
          email,
          planCode: plan.code,
          planLabel: plan.label,
          attemptsAllowed: plan.attemptsAllowed,
          attemptsUsed: existing && existing.planCode === plan.code ? Number(existing.attemptsUsed || 0) : 0,
          status: 'active',
        ...
        await saveCitizenshipAccess(nextRecord);
      }
    }
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    citizenshipPlans: Object.values(CITIZENSHIP_PLANS).map(plan => ({
      code: plan.code,
      label: plan.label,
      paid: plan.paid,
      attemptsAllowed: plan.attemptsAllowed,
      hasPriceId: Boolean(plan.priceId),
    })),
...
app.post('/api/citizenship/create-checkout-session', async (req, res, next) => {
  try {
    requireStripeConfigured();
    const plan = normaliseCitizenshipPlan(req.body?.planCode);
    const email = String(req.body?.email || '').trim().toLowerCase();
    const fullName = String(req.body?.fullName || '').trim();

    if (!plan.paid) return res.status(400).json({ ok: false, error: 'Select a paid plan before creating Stripe Checkout.' });
    if (!plan.priceId) return res.status(500).json({ ok: false, error: `Stripe price is not configured for ${plan.label}.` });
    if (!email) return res.status(400).json({ ok: false, error: 'Customer email is required.' });

    const successBase = CITIZENSHIP_SUCCESS_URL.replace(/\/$/, '');
    const cancelBase = CITIZENSHIP_CANCEL_URL.replace(/\/$/, '');
...
app.get('/api/citizenship/access', async (req, res, next) => {
  try {
    const email = String(req.query?.email || '').trim().toLowerCase();
    const sessionId = String(req.query?.session_id || req.query?.sessionId || '').trim();

    let record = null;
    if (sessionId) record = await getCitizenshipAccessBySessionId(sessionId);
    if (!record && email) record = await getCitizenshipAccessByEmail(email);

    res.json(buildCitizenshipAccessResponse(record));
  } catch (error) {
    next(error);
  }
});

app.post('/api/citizenship/attempts/start', async (req, res, next) => {
  try {
    const email = String(req.body?.email || '').trim().toLowerCase();
    if (!email) return res.status(400).json({ ok: false, error: 'Customer email is required.' });

    const record = await getCitizenshipAccessByEmail(email);
    if (!record) return res.status(403).json({ ok: false, error: 'No paid citizenship test access record was found.' });
    if (record.status !== 'active') return res.status(403).json({ ok: false, error: 'Citizenship test access is not active.' });

    const attemptsRemaining = getCitizenshipAttemptsRemaining(record);
    if (attemptsRemaining !== null && attemptsRemaining <= 0) {
      return res.status(403).json({ ok: false, error: 'No attempts remaining for this plan.' });
    }

    const attemptId = safeId('cit_attempt');
    const attempt = {
      id: attemptId,
      startedAt: nowIso(),
      status: 'in_progress',
    ...
    record.attempts = attempts.slice(-200);
    record.lastAttemptStartedAt = attempt.startedAt;
    await saveCitizenshipAccess(record);

    res.json({
      ok: true,
      attemptId,
      attemptsRemainingBeforeFinish: attemptsRemaining,
      access: buildCitizenshipAccessResponse(record),
    });
  } catch (error) {
    next(error);
  }
...
app.post('/api/citizenship/attempts/finish', async (req, res, next) => {
  try {
    const email = String(req.body?.email || '').trim().toLowerCase();
    const attemptId = String(req.body?.attemptId || '').trim();
    const score = Number(req.body?.score || 0);
    const valuesScore = Number(req.body?.valuesScore || 0);
    ...
    const attempts = Array.isArray(record.attempts) ? record.attempts : [];
    const attemptIndex = attemptId ? attempts.findIndex(item => item && item.id === attemptId) : -1;
    const attemptAlreadyFinished = attemptIndex >= 0 && attempts[attemptIndex]?.status === 'completed';

    if (!attemptAlreadyFinished) {
      const attemptsRemaining = getCitizenshipAttemptsRemaining(record);
      if (attemptsRemaining !== null && attemptsRemaining <= 0) {
        return res.status(403).json({ ok: false, error: 'No attempts remaining for this plan.' });
      }
      if (record.attemptsAllowed !== null) record.attemptsUsed = Number(record.attemptsUsed || 0) + 1;
    }

    const completedAt = nowIso();
    const payload = {
      id: attemptId || safeId('cit_attempt'),
      startedAt: attemptIndex >= 0 ? attempts[attemptIndex]?.startedAt || completedAt : completedAt,
      completedAt,
      status: 'completed',
      score,
      valuesScore,
    ...
    if (attemptIndex >= 0) attempts[attemptIndex] = { ...attempts[attemptIndex], ...payload };
    else attempts.push(payload);

    record.attempts = attempts.slice(-200);
    record.lastAttemptCompletedAt = completedAt;
    record.lastAttemptResult = payload;
    await saveCitizenshipAccess(record);

    res.json({
      ok: true,
      access: buildCitizenshipAccessResponse(record),
      lastAttemptResult: payload,
    });
  } catch (error) {
    next(error);
  }
