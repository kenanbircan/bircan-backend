...
async function ensureSubmissionPdfReady(submission) {
  ...
  for (const candidate of candidatePaths) {
    if (candidate && fs.existsSync(candidate) && hasRealAdviceContent(submission)) {
...
// -----------------------------------------------------------------------------
// Advice PDF finality guard
// -----------------------------------------------------------------------------
// Prevents old template PDFs from being treated as final issued advice letters.
function extractAdviceAnalysisForFinality(submission = {}) {
  const a = submission.analysis || submission.aiAnalysis || submission.assessmentAnalysis || {};
  if (a && a.analysis && typeof a.analysis === 'object') return a.analysis;
  if (a && typeof a === 'object') return a;
  return {};
}

function adviceTextForFinality(value) {
  if (!value) return '';
...
function hasRealAdviceContent(submission = {}) {
  const analysis = extractAdviceAnalysisForFinality(submission);
  const fields = [
    analysis.outcomeRationale,
    analysis.professionalOpinion,
    analysis.overallAssessment,
    analysis.executiveSummary,
    analysis.legalStyleSummary,
    analysis.detailedAssessment,
  ...
  const joined = fields.map(adviceTextForFinality).join('\n').trim();

  const listCount =
    (Array.isArray(analysis.decisionFramework) ? analysis.decisionFramework.length : 0) +
    ...
    (Array.isArray(analysis.recommendedNextSteps) ? analysis.recommendedNextSteps.length : 0) +
    (Array.isArray(analysis.documentChecklist) ? analysis.documentChecklist.length : 0);

  const placeholder = [
    /assessment summary not available/i,
    /outcome rationale not available/i,
    /professional opinion was not generated/i,
    /no criterion-by-criterion framework was generated/i,
    /detailed assessment was not generated/i,
    /strategy advice was not generated/i,
    /not generated/i,
    /not available/i,
  ];

  if (joined.length < 900) return false;
  if (placeholder.some(rx => rx.test(joined))) return false;
...
function assertRealAdviceContent(submission = {}) {
  if (!hasRealAdviceContent(submission)) {
    const error = new Error('Advice letter content is not generated yet. Template PDF blocked.');
...
async function syncDbServiceDocumentFromSubmission(submission) {
  try {
    ...
    const pdfReady = Boolean((pdfFileExists || fallbackFileExists) && hasRealAdviceContent(submission));
...
// -----------------------------------------------------------------------------
// Missing body-based generate PDF route + queue/status endpoints
// -----------------------------------------------------------------------------
// Frontend dashboards sometimes POST the assessment ID in the JSON body rather
// than using /api/assessment/:submissionId/generate-pdf. This route fixes:
// Route not found: POST /api/assessment/generate-pdf
app.post('/api/assessment/generate-pdf', requireCitizenshipAuth, async (req, res, next) => {
  try {
    const submissionId = normaliseDocumentSubmissionId(req.body?.assessmentId || req.body?.submissionId || req.body?.id);
    if (!submissionId) return res.status(400).json({ ok: false, ready: false, error: 'Missing assessment ID.' });

    let submission = await getSubmission(submissionId);
    if (!submission) return res.status(404).json({ ok: false, ready: false, error: 'Submission not found.' });

    const userEmail = String(req.user?.email || '').trim().toLowerCase();
    const docEmails = [submission.accountEmail, submission.client?.email, submission.email, submission.stripeCustomerEmail, submission.clientEmail]
      .map(v => String(v || '').trim().toLowerCase()).filter(Boolean);
    if (userEmail && docEmails.length && !docEmails.includes(userEmail)) {
      return res.status(403).json({ ok: false, ready: false, error: 'This assessment belongs to a different account.' });
    }

    // If already final and valid, return ready.
    if (hasRealAdviceContent(submission) && submission.pdfPath && fs.existsSync(submission.pdfPath)) {
      submission = await ensureSubmissionPdfReady(submission);
      return res.json({
        ok: true,
        ready: true,
    ...
    // Queue async processing using your existing processSubmission pipeline.
    await updateSubmission(submission.id, {
      status: 'queued',
      analysisStatus: 'queued',
      pdfStatus: 'generating',
    ...
    const queuedSubmission = await getSubmission(submission.id);
    if (queuedSubmission) await syncDbServiceDocumentFromSubmission(queuedSubmission);

    processSubmission(submission.id).catch(async (error) => {
      console.error('[manual-generate-pdf-error]', submission.id, error);
      await updateSubmission(submission.id, {
        processingError: error.message,
    ...
    return res.status(202).json({
      ...
      status: 'queued',
      submissionId: submission.id,
      message: 'Full advice-letter generation has been queued.',
    });
  } catch (error) {
    next(error);
  }
...
app.get('/api/assessment/:submissionId/generation-status', requireCitizenshipAuth, async (req, res, next) => {
  try {
    const submissionId = normaliseDocumentSubmissionId(req.params.submissionId);
    const submission = await getSubmission(submissionId);
    if (!submission) return res.status(404).json({ ok: false, ready: false, error: 'Submission not found.' });

    const ready = Boolean(hasRealAdviceContent(submission) && submission.pdfPath && fs.existsSync(submission.pdfPath) && String(submission.pdfStatus).toLowerCase() === 'ready');
    return res.json({
      ok: true,
      ready,
      submissionId: submission.id,
      status: ready ? 'ready' : (submission.pdfStatus || submission.analysisStatus || submission.status || 'processing'),
      pdfStatus: submission.pdfStatus || null,
      documentStatus: submission.documentStatus || null,
      analysisStatus: submission.analysisStatus || null,
      error: submission.pdfError || submission.processingError || '',
      pdfUrl: ready ? (submission.pdfUrl || `/api/assessment/${encodeURIComponent(submission.id)}/pdf`) : '',
      downloadUrl: ready ? (submission.downloadUrl || `/api/assessment/${encodeURIComponent(submission.id)}/pdf`) : '',
      updatedAt: submission.updatedAt || null,
    });
  } catch (error) {
    next(error);
