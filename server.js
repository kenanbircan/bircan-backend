...
app.get('/api/documents/:serviceId', async (req, res, next) => {
  try {
    ...
    const submissionRaw = await getSubmission(submissionId);
    const submission = await ensureSubmissionPdfReady(submissionRaw);
    if (!submission) return res.status(404).json({ ok: false, error: 'PDF not ready. The advice letter has not been generated yet.' });
...
async function handleAssessmentEmailPdfRequest(req, res, next) {
  try {
    ...
    submission = await ensureSubmissionPdfReady(submission);
    if (!submission || !submission.pdfPath || !fs.existsSync(submission.pdfPath)) {
      return res.status(409).json({ ok: false, error: 'Advice letter is not ready yet.' });
    }
    ...
    const result = await trySendAssessmentEmail(submission, submission.pdfPath, { to: allowedTo });
