...
function normaliseAnalysisShape(ai = {}, submission = {}) {
  ...
  if (!Array.isArray(merged.nextSteps) || !merged.nextSteps.length) {
    merged.nextSteps = Array.isArray(merged.recommendedNextSteps) ? merged.recommendedNextSteps : [];
  }
  if (!Array.isArray(merged.risks) || !merged.risks.length) {
    merged.risks = Array.isArray(merged.concerns) ? merged.concerns : [];
  }
  ...
  if (!Array.isArray(merged.visaCriteriaBreakdown)) merged.visaCriteriaBreakdown = [];
  merged.visaCriteriaBreakdown = merged.visaCriteriaBreakdown
    .map(item => ({
      criterion: String(item?.criterion || '').trim(),
      assessment: String(item?.assessment || '').trim(),
      riskLevel: ['Low', 'Medium', 'High'].includes(String(item?.riskLevel || '').trim()) ? String(item.riskLevel).trim() : 'Medium',
    }))
    .filter(item => item.criterion && item.assessment)
    .slice(0, 10);

  ['caseCaption', 'eligibilityOutcome', 'overallAssessment', 'executiveSummary', 'legalStyleSummary', 'detailedAssessment', 'disclaimer'].forEach(key => {
...
async function runAssessmentAnalysis(submission) {
  ...
  const visaType = String(submission.visaType || 'Subclass 482');
  const visaSpecificFocus = [];
  if (/482/i.test(visaType)) {
    visaSpecificFocus.push(
      '- Subclass 482 focus areas: sponsorship approval, nomination approval, genuine position, occupation alignment, relevant work experience, skills assessment validity where applicable, market salary and TSMIT-style concerns, English, health, character, and timing risks.',
      '- Consider issues affecting the Core Skills stream, including whether the role, employer, and evidence package appear commercially credible on the available facts.'
    );
  }
  if (/186/i.test(visaType)) {
    visaSpecificFocus.push('- Consider subclass 186 issues such as TRT/direct entry pathway logic, age, skills, competent English, employment continuity, and nomination validity.');
  }
  if (/190|189|491/i.test(visaType)) {
    visaSpecificFocus.push('- Consider GSM issues such as points, invitation dependency, skills assessment, occupation list alignment, English level, state nomination where relevant, and claims evidence.');
  }

  const systemPrompt = [
    'You are a senior Australian migration case analyst drafting a detailed, professional preliminary visa assessment for Bircan Migration & Education.',
    'Use a law-firm style tone: measured, formal, strategic, and commercially valuable.',
    'Do not overstate certainty. Do not guarantee grant outcomes.',
    'Do not invent legislation, facts, or policy settings not supported by the supplied answers.',
    'Do not be generic. Explain why each strength or risk matters.',
    'Assess the case like a migration professional reviewing eligibility, evidence gaps, refusal exposure, and filing strategy.',
    ...
    '  "visaCriteriaBreakdown": [{ "criterion": string, "assessment": string, "riskLevel": "Low" | "Medium" | "High" }],',
    '  "strengths": string[],',
    '  "concerns": string[],',
    '  "risks": string[],',
    '  "refusalRisks": string[],',
  ...
  const userPrompt = [
    ...
    '- Provide detailed reasoning, not generic summaries.',
    '- Identify strengths, legal/practical risks, refusal exposure, evidence gaps, and filing strategy.',
    '- Explain WHY each issue matters.',
    '- Focus on visa-specific risks, refusal scenarios, compliance issues, and strategic advice for approval success.',
    '- Where the case appears viable, say "potentially eligible" or "appears to meet preliminary criteria", not guaranteed.',
    '- Distinguish between risks that can be fixed with evidence and risks that may affect eligibility itself.',
    ...visaSpecificFocus,
    '',
    'Applicant answers:',
...
function drawFirstPageHeader(doc) {
  ...
  if (fs.existsSync(letterheadPath)) {
    try {
      ...
      const cropTop = Math.max(0, scaledHeight - targetVisibleHeight);
      const drawY = visibleTopY - cropTop;
      doc.image(letterheadPath, startX, drawY, { width: contentWidth });
      drawLetterheadBorder(doc);
      doc.y = visibleTopY + targetVisibleHeight + 10;
    } catch (_) {
      doc.y = 72;
    }
  } else {
...
function addCriteriaBreakdown(doc, items = []) {
  const list = Array.isArray(items) && items.length ? items : [];
  if (!list.length) {
    addParagraph(doc, 'No visa-criteria breakdown was generated from the currently available information.');
    return;
  }

  list.forEach(item => {
    const criterion = String(item?.criterion || '').trim();
    const riskLevel = String(item?.riskLevel || 'Medium').trim();
    const assessment = String(item?.assessment || '').trim();

    const titleHeight = doc.heightOfString(`${criterion} (${riskLevel} risk)`, {
      width: 495,
      align: 'left'
    });

    const bodyHeight = doc.heightOfString(assessment, {
      width: 495,
      align: 'left',
      lineGap: 2
    });

    const boxHeight = Math.max(54, 10 + titleHeight + 6 + bodyHeight + 10);

    ensureSpace(doc, 760 - boxHeight);

    const boxY = doc.y;
    doc.roundedRect(40, boxY, 515, boxHeight, 6)
      .strokeColor('#d9e2ef')
      .lineWidth(1)
      .stroke();
    ...
    doc.font('Helvetica-Bold')
      .fontSize(10.5)
      .fillColor('#0f2747')
      .text(`${criterion} (${riskLevel} risk)`, 50, boxY + 10, {
        width: 495,
        align: 'left'
      });
    ...
    doc.font('Helvetica')
      .fontSize(10.2)
      .fillColor('#172033')
      .text(assessment, 50, boxY + 10 + titleHeight + 6, {
        width: 495,
        align: 'left',
        lineGap: 2
      });
...
function drawLetterheadBorder(doc) {
  const left = doc.page.margins.left;
  const top = 8;
  const width = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const height = 182;
  doc.save();
  doc.rect(left, top, width, height).lineWidth(1.2).strokeColor('#1d8f97').stroke();
  doc.restore();
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    addSectionTitle(doc, 'Detailed Eligibility Analysis');
    addParagraph(doc, analysis.detailedAssessment || analysis.overallAssessment || 'Detailed assessment not available.');

    addSectionTitle(doc, 'Visa Criteria Breakdown');
    addCriteriaBreakdown(doc, analysis.visaCriteriaBreakdown || []);
    ...
    addBulletList(doc, analysis.risks?.length ? analysis.risks : analysis.concerns, 'No material concerns were identified from the currently available information.');

    addSectionTitle(doc, 'Potential Refusal Risks');
    addBulletList(doc, analysis.refusalRisks, 'No refusal-specific risks were generated from the currently available information.');
    ...
    addBulletList(doc, analysis.nextSteps?.length ? analysis.nextSteps : analysis.recommendedNextSteps, 'No specific next steps were generated.');
