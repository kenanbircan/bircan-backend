...
const { normaliseDashboardPayload } = require('./utils/dashboard-normaliser');
...
function bmMarkGenerationState(submission = {}, status, extra = {}) { return { ...submission, pdfStatus: status, documentStatus: status, generationStatus: status, adviceLetterStatus: status, updatedAt: nowIso(), ...extra }; }
async function bmSaveGenerationState(submission = {}, status, extra = {}) { if (!submission || !submission.id) return null; const patched = await saveSubmission(bmMarkGenerationState(submission, status, extra)); if (typeof syncDbServiceDocumentFromSubmission === 'function') await syncDbServiceDocumentFromSubmission(patched).catch(err => console.error('[bm-pdf] db sync failed', err.message)); return patched; }
...
function bmEnqueueAdviceGeneration(submissionId, options = {}) {
  ...
  const job = { id, status: 'queued', attempts: 0, maxAttempts: Number(options.maxAttempts || BM_ADVICE_QUEUE_MAX_ATTEMPTS), force: Boolean(options.force), regenerate: Boolean(options.regenerate), createdAt: nowIso(), updatedAt: nowIso(), lastError: '' };
  BM_ADVICE_JOBS.set(id, job); BM_ADVICE_QUEUE.push(job);
  getSubmission(id).then(sub => { if (sub) return bmSaveGenerationState(sub, 'queued', { queuedAt: job.createdAt, queueAttempts: 0, pdfError: '' }); }).catch(err => console.error('[bm-queue] mark queued failed', id, err.message));
  setImmediate(bmProcessAdviceQueue);
  return job;
}
async function bmRunAdviceJob(job) {
  job.status = 'running'; job.attempts += 1; job.updatedAt = nowIso(); console.log(`[bm-queue] running ${job.id} attempt ${job.attempts}/${job.maxAttempts}`);
  try {
    ...
    job.status = 'completed'; job.completedAt = nowIso(); job.updatedAt = job.completedAt; job.lastError = ''; BM_ADVICE_JOBS.delete(job.id); console.log(`[bm-queue] completed ${job.id}`); return ready;
  } catch (error) {
    ...
    if (job.attempts < job.maxAttempts) {
      ...
      if (sub) await bmSaveGenerationState(sub, 'retrying', { queueAttempts: job.attempts, pdfError: job.lastError, nextRetryAt: new Date(Date.now() + BM_ADVICE_QUEUE_RETRY_DELAY_MS * job.attempts).toISOString() });
      await new Promise(resolve => setTimeout(resolve, BM_ADVICE_QUEUE_RETRY_DELAY_MS * job.attempts));
      BM_ADVICE_QUEUE.push(job);
    } else {
      job.status = 'failed'; job.failedAt = nowIso();
      const sub = await getSubmission(job.id).catch(() => null);
...
async function bmRecoverStaleAdviceJobs() { const all = await getAllSubmissions().catch(() => []); const stale = new Set(['queued','generating','retrying','processing']); const staleAfterMs = Number(process.env.ADVICE_QUEUE_STALE_AFTER_MS || 20 * 60 * 1000); const now = Date.now(); for (const sub of all) { const st = String(sub.generationStatus || sub.pdfStatus || sub.documentStatus || '').toLowerCase(); if (!stale.has(st)) continue; const updated = new Date(sub.updatedAt || sub.generationStartedAt || sub.createdAt || 0).getTime(); if (Number.isFinite(updated) && now - updated < staleAfterMs) continue; bmEnqueueAdviceGeneration(sub.id, { force: false, regenerate: false }); } }
...
app.post('/api/assessment/:submissionId/generate-pdf', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    submission = await ensureSubmissionPdfReady(submission);
    if (!submission || !submission.pdfPath || !fs.existsSync(submission.pdfPath)) {
    ...
    return res.json({
      ...
      pdfStatus: 'ready',
      documentStatus: 'ready',
      pdfUrl: submission.pdfUrl || `/api/assessment/${encodeURIComponent(submission.id)}/pdf`,
      downloadUrl: submission.downloadUrl || `/api/assessment/${encodeURIComponent(submission.id)}/pdf`,
    });
  } catch (error) {
    next(error);
  }
...
app.post('/api/assessment/generate-pdf', requireCitizenshipAuth, async (req, res) => {
  ...
  try {
    ...
    const userEmail = String(req.user?.email || '').trim().toLowerCase();
    const docEmails = [submission.accountEmail, submission.client?.email, submission.email, submission.stripeCustomerEmail].map(v => String(v || '').trim().toLowerCase()).filter(Boolean);
    if (userEmail && docEmails.length && !docEmails.includes(userEmail)) return res.status(403).json({ ok: false, ready: false, error: 'This assessment belongs to a different account.' });

    if (bmHasRealAdviceContent(submission) && submission.pdfStatus === 'ready' && submission.pdfUrl) {
      return res.json({ ok: true, ready: true, status: 'ready', assessmentId: submission.id, pdfStatus: submission.pdfStatus, documentStatus: submission.documentStatus, generationStatus: submission.generationStatus, pdfUrl: submission.pdfUrl, downloadUrl: submission.downloadUrl || submission.pdfUrl, documentUrl: submission.documentUrl || submission.pdfUrl, pdfIssuedAt: submission.pdfIssuedAt || submission.completedAt || null });
    }

    const job = bmEnqueueAdviceGeneration(submission.id, { force: Boolean(req.body?.force), regenerate: Boolean(req.body?.regenerate) });
    return res.status(202).json({ ok: true, ready: false, queued: true, status: job.status, assessmentId: submission.id, attempts: job.attempts, maxAttempts: job.maxAttempts, message: 'Advice-letter generation has been queued.' });
  } catch (error) {
    return res.status(error.statusCode || 500).json({ ok: false, ready: false, error: error.message || 'Advice-letter generation failed.' });
  }
});

app.get('/api/assessment/:submissionId/generation-status', requireCitizenshipAuth, async (req, res) => {
  ...
  const ready = Boolean(submission && bmHasRealAdviceContent(submission) && submission.pdfStatus === 'ready' && submission.pdfUrl);
  return res.json({ ok: true, ready, queued: Boolean(job && ['queued','running','retrying'].includes(job.status)), assessmentId: submission?.id || id, status: ready ? 'ready' : (job?.status || submission?.generationStatus || submission?.pdfStatus || submission?.documentStatus || 'processing'), attempts: job?.attempts || submission?.queueAttempts || 0, maxAttempts: job?.maxAttempts || BM_ADVICE_QUEUE_MAX_ATTEMPTS, error: job?.lastError || submission?.pdfError || '', pdfUrl: ready ? submission.pdfUrl : '', downloadUrl: ready ? (submission.downloadUrl || submission.pdfUrl) : '', documentUrl: ready ? (submission.documentUrl || submission.pdfUrl) : '', updatedAt: submission?.updatedAt || job?.updatedAt || null });
});
