...
function normaliseAnalysisShape(ai = {}, submission = {}) {
  ...
  ['caseCaption', 'eligibilityOutcome', 'outcomeRiskLevel', 'applicationReadiness', 'outcomeRationale', 'overallAssessment', 'executiveSummary', 'legalStyleSummary', 'disclaimer'].forEach(key => {
...
async function runAssessmentAnalysis(submission) {
  ...
  const systemPrompt = [
    'You are a senior Australian migration case analyst preparing a commercially valuable preliminary assessment for Bircan Migration & Education.',
    'Write like an experienced Registered Migration Agent: careful, specific, strategic, and professional.',
    'Do not be generic. Do not simply restate facts. Analyse what matters, why it matters, and what should happen next.',
    'Do not guarantee grant outcomes. Do not invent facts, legislation, or policy settings that are not supported by the supplied information.',
    'Where the case appears viable, state that it appears viable or potentially eligible, but pair that conclusion with a realistic risk profile and readiness position.',
    'Focus on: eligibility logic, evidentiary strength, refusal exposure, practical filing strategy, timing issues, sponsor or nomination dependencies, and documentary weaknesses.',
...
');
  ...
  const userPrompt = [
    ...
    '',
    'Instructions:',
    '- Take a position on the case, but stay measured.',
    '- Give a clear outcome, risk level, and application-readiness view.',
    '- Explain the primary reasons supporting the outcome.',
    '- Identify realistic refusal scenarios and why they matter.',
    '- Provide strategy advice that reduces refusal exposure and improves lodgement quality.',
    '- Distinguish between risks that are fatal, risks that are curable with evidence, and risks caused by timing or pending approvals.',
    '- If something is assumed rather than confirmed, say so clearly.',
...
// Highlight risk summary box
const riskY = doc.y;
doc.roundedRect(40, riskY, 515, 60, 6)
  .strokeColor('#f59e0b')
  .lineWidth(1)
  .stroke();
...
doc.font('Helvetica-Bold')
  .fontSize(11)
  .fillColor('#92400e')
  .text('KEY RISKS SUMMARY', 50, riskY + 10);

doc.font('Helvetica')
  .fontSize(10.3)
  .fillColor('#172033')
  .text('Critical risks should be addressed before lodgement to reduce refusal exposure.', 50, riskY + 30, { width: 495 });
...
addParagraph(doc, 'Professional Migration Advice & Representation');
