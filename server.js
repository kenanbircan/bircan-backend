...
async function handleAppealsAssessmentCreate(req, res) {
  ...
  const decisionFile = (fileFields.decisionFile || fileFields.decisionPdf || [])[0] || req.file || null;
...
async function generateAppealAdviceNow(appealId) {
  ...
  if (apiKey) {
    const prompt = `You are drafting a professional Australian migration refusal review advice letter for Bircan Migration & Education (www.bircanmigration.com.au). Analyse the uploaded refusal/decision material, refusal grounds, evidence gaps, deadline risk, review strategy and next steps. Do not mention AI, GPT, prompts or automation. Return only valid JSON with this shape: {"advice":{"title":"...","risk_level":"Low/Moderate/High/Very High","executive_summary":"...","refusal_grounds":["..."],"evidence_position":["..."],"strategy":"...","risk_assessment":"...","next_steps":["..."],"deadline_warning":"...","disclaimer":"..."}}.\n\nClient details:\nReference: ${assessment.id}\nApplicant: ${assessment.applicant_name || ''}\nEmail: ${assessment.client_email || ''}\nVisa subclass: ${assessment.visa_subclass || ''}\nDecision type: ${assessment.decision_type || ''}\nDecision date: ${assessment.decision_date || ''}\nReview deadline: ${assessment.tribunal_deadline || ''}\nClient appeal summary: ${assessment.appeal_grounds || ''}\nUrgency notes: ${assessment.urgency_notes || ''}\n\nUploaded document text:\n${combinedText}`;
    try {
