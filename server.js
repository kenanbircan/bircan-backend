...
// Full appeals list endpoint for account-dashboard.html.
// Keeps the appeals tab complete even when the unified service card feed is deduped.
app.get('/api/account/appeals/all', requireAuth, asyncRoute(async (req, res) => {
  ...
  res.json({ ok: true, count: rows.length, appealsAssessments: rows, appeals: rows, items: rows });
}));
