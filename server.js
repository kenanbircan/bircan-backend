...
async function getReusableOpenCheckoutSession(stripeSessionId, expected = {}) {
  ...
  try {
    ...
    if (session && session.url && session.status === 'open' && session.payment_status !== 'paid' && serviceOk && planOk && refOk) return session;
  } catch (err) {
...
function priceEnvNameForCommercialPlan(service, plan) {
  const p = safePlan(plan);
  const prefix = service === 'appeals_assessment' ? 'STRIPE_PRICE_APPEAL' : 'STRIPE_PRICE_VISA';
  if (p === 'instant') return `${prefix}_300`;
  if (p === '24h') return `${prefix}_250`;
  if (p === '3d') return `${prefix}_200`;
  throw Object.assign(new Error(`Invalid ${service} plan: ${plan}`), { statusCode: 400 });
}

function expectedCommercialAmountCents(plan) {
  const p = safePlan(plan);
  if (p === 'instant') return 30000;
  if (p === '24h') return 25000;
  if (p === '3d') return 20000;
  throw Object.assign(new Error(`Invalid commercial plan: ${plan}`), { statusCode: 400 });
}

function resolveVisaPriceId(_visaType, plan) {
  const p = safePlan(plan);
  if (p === 'instant') return envFirst('STRIPE_PRICE_VISA_300', 'STRIPE_PRICE_VISA_INSTANT', 'STRIPE_PRICE_VISA_INSTANT_TEST', 'STRIPE_PRICE_VISA_INSTANT_LIVE');
  if (p === '24h') return envFirst('STRIPE_PRICE_VISA_250', 'STRIPE_PRICE_VISA_24H', 'STRIPE_PRICE_VISA_24H_TEST', 'STRIPE_PRICE_VISA_24H_LIVE');
  if (p === '3d') return envFirst('STRIPE_PRICE_VISA_200', 'STRIPE_PRICE_VISA_3D', 'STRIPE_PRICE_VISA_3D_TEST', 'STRIPE_PRICE_VISA_3D_LIVE');
  return null;
}

function resolveAppealPriceId(plan) {
  const p = safePlan(plan);
  if (p === 'instant') return envFirst('STRIPE_PRICE_APPEAL_300', 'STRIPE_PRICE_APPEAL_INSTANT', 'STRIPE_PRICE_APPEAL_INSTANT_TEST', 'STRIPE_PRICE_APPEAL_INSTANT_LIVE');
  if (p === '24h') return envFirst('STRIPE_PRICE_APPEAL_250', 'STRIPE_PRICE_APPEAL_24H', 'STRIPE_PRICE_APPEAL_24H_TEST', 'STRIPE_PRICE_APPEAL_24H_LIVE');
  if (p === '3d') return envFirst('STRIPE_PRICE_APPEAL_200', 'STRIPE_PRICE_APPEAL_3D', 'STRIPE_PRICE_APPEAL_3D_TEST', 'STRIPE_PRICE_APPEAL_3D_LIVE');
  return null;
}
...
function normaliseCitizenshipPlan(plan) {
  ...
  const raw = lower.replace(/\s+/g, '').replace(/[-_]+/g, '').replace(/aud/g, '');

  // Citizenship plans are exam-count plans:
  // 20 tests -> $50, 50 tests -> $100, 150 tests -> $150.
  // Dollar labels are also accepted for older pages, but are converted into
  // the matching exam-count package before Stripe price lookup.
  if (/\$\s*50/.test(lower) || ['20', '20test', '20tests', '20exam', '20exams', 'starter', 'basic'].includes(raw.replace(/\$/g, ''))) return '20';
  if (/\$\s*100/.test(lower) || ['50', '50test', '50tests', '50exam', '50exams', 'standard', 'plus'].includes(raw.replace(/\$/g, ''))) return '50';
  if (/\$\s*150/.test(lower) || ['150', '150test', '150tests', '150exam', '150exams', 'premium', 'professional', 'complete', 'unlimited', 'unlimitedexams', 'unlimitedexam', 'all'].includes(raw.replace(/\$/g, ''))) return '150';

  // Backwards compatibility for previous dollar-plan field values.
  if (['50plan', '50payment', '50package'].includes(raw.replace(/\$/g, ''))) return '20';
  if (['100', '100plan', '100payment', '100package'].includes(raw.replace(/\$/g, ''))) return '50';
  if (['150plan', '150payment', '150package'].includes(raw.replace(/\$/g, ''))) return '150';

  return '20';
}

function requestedCitizenshipPlan(req, fallback = '50') {
  const sources = [
    req.body && req.body.plan,
    req.body && req.body.selectedPlan,
    req.body && req.body.selected_plan,
    req.body && req.body.planId,
    req.body && req.body.plan_id,
    req.body && req.body.pricePlan,
    req.body && req.body.price_plan,
    req.body && req.body.package,
    req.body && req.body.tier,
    req.body && req.body.amount,
    req.body && req.body.price,
    req.query && req.query.plan,
    req.query && req.query.selectedPlan,
    req.query && req.query.selected_plan,
    req.query && req.query.planId,
    req.query && req.query.pricePlan,
  ...
  for (const value of sources) {
    if (value !== undefined && value !== null && String(value).trim() !== '') {
      return normaliseCitizenshipPlan(value);
    }
  }
...
function expectedCitizenshipAmountCents(plan) {
  const p = normaliseCitizenshipPlan(plan);
  if (p === '20') return 5000;
  if (p === '50') return 10000;
  if (p === '150') return 15000;
  throw Object.assign(new Error(`Invalid citizenship plan: ${plan}`), { statusCode: 400 });
}

function resolveCitizenshipPriceId(plan) {
  const p = normaliseCitizenshipPlan(plan);
  if (p === '20') return envFirst('STRIPE_PRICE_CITIZENSHIP_50', 'STRIPE_PRICE_CITIZENSHIP_20');
  if (p === '50') return envFirst('STRIPE_PRICE_CITIZENSHIP_100', 'STRIPE_PRICE_CITIZENSHIP_50_TESTS');
  if (p === '150') return envFirst('STRIPE_PRICE_CITIZENSHIP_150');
  return null;
}

async function verifyStripePriceMatches(priceId, expectedAmountCents, context) {
  if (!stripe || !priceId) return;
  const price = await stripe.prices.retrieve(priceId);
  const actual = Number(price && price.unit_amount);
  const currency = String((price && price.currency) || '').toLowerCase();
  if (actual !== Number(expectedAmountCents) || currency !== 'aud') {
    const expectedDollars = (Number(expectedAmountCents) / 100).toFixed(2);
    const actualDollars = Number.isFinite(actual) ? (actual / 100).toFixed(2) : 'unknown';
    const err = new Error(`Stripe price mismatch blocked for ${context}: expected AUD $${expectedDollars}, but configured price ${priceId} is ${currency.toUpperCase() || 'UNKNOWN'} $${actualDollars}. Fix the Render Stripe price environment variable before checkout.`);
    err.statusCode = 500;
    throw err;
  }
}
...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.3.0-locked-stripe-routing-visa-appeals-citizenship',
    postgres: true,
    jsonFallback: false,
...
async function handleAppealsAssessmentCreate(req, res) {
  ...
  // Production duplicate guard: browsers can submit the appeals form twice during
  // login/payment handoff. If the same applicant, subclass, plan, grounds and
  // uploaded decision file arrive again within a short window, reuse the original
  // appeal record and service session instead of creating a second dashboard card.
  if (!publicDraftKey) {
    const duplicate = await findRecentDuplicateAppealSubmission({ email, visaSubclass, decisionType, appealGrounds, plan, decisionFile });
    if (duplicate) {
      const serviceSession = await upsertServiceSession({
        id: duplicate.service_session_id || null,
        serviceType: 'appeals_assessment',
        serviceRef: duplicate.id,
        email,
        plan,
        metadata: {
      ...
      return res.json({
        ...
        duplicate: true,
        plan,
        next: `/login.html?service=appeals&service_session_id=${encodeURIComponent(serviceSession.id)}&appeal_assessment_id=${encodeURIComponent(duplicate.id)}`
      });
    }
  }
...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'visa_assessment') {
    ...
    const checkoutPlan = requestedVisaPlan(req, serviceSession.selected_plan || assessment.active_plan || assessment.selected_plan || 'instant');
    const price = resolveVisaPriceId(assessment.visa_type, checkoutPlan);
    if (!price) return res.status(500).json({ ok: false, error: `Missing Stripe price for visa plan ${checkoutPlan}. Add ${priceEnvNameForCommercialPlan('visa_assessment', checkoutPlan)} in Render.` });
    try { await verifyStripePriceMatches(price, expectedCommercialAmountCents(checkoutPlan), `visa ${checkoutPlan}`); } catch (err) { return res.status(err.statusCode || 500).json({ ok: false, error: err.message }); }
    ...
    await query(
      `UPDATE assessments
       ...
       WHERE id=$5`,
      [stripeSession.id, checkoutPlan, stripeSession.amount_total || expectedCommercialAmountCents(checkoutPlan), stripeSession.currency || 'aud', assessment.id]
    );
  ...
  if (serviceSession.service_type === 'appeals_assessment') {
    ...
    const plan = safePlan(req.body.plan || req.body.selectedPlan || req.body.selected_plan || req.body.planId || serviceSession.selected_plan || assessment.selected_plan || 'instant');
    const price = resolveAppealPriceId(plan);
    if (!price) return res.status(500).json({ ok: false, error: `Missing Stripe price for appeals plan ${plan}. Add ${priceEnvNameForCommercialPlan('appeals_assessment', plan)} in Render.` });
    try { await verifyStripePriceMatches(price, expectedCommercialAmountCents(plan), `appeals ${plan}`); } catch (err) { return res.status(err.statusCode || 500).json({ ok: false, error: err.message }); }
  ...
  if (serviceSession.service_type === 'citizenship_test') {
    const plan = requestedCitizenshipPlan(req, serviceSession.selected_plan || '50');
    const price = resolveCitizenshipPriceId(plan);
    if (!price) return res.status(500).json({ ok: false, error: `Missing Stripe price for citizenship plan ${plan}. Expected 20 tests -> STRIPE_PRICE_CITIZENSHIP_50, 50 tests -> STRIPE_PRICE_CITIZENSHIP_100, 150 tests -> STRIPE_PRICE_CITIZENSHIP_150.` });
    try { await verifyStripePriceMatches(price, expectedCitizenshipAmountCents(plan), `citizenship ${plan} tests`); } catch (err) { return res.status(err.statusCode || 500).json({ ok: false, error: err.message }); }
    ...
    await query(`UPDATE citizenship_access SET stripe_session_id=$1, amount_cents=$2, currency=$3, raw_payload=$4, updated_at=now() WHERE id=$5`, [stripeSession.id, stripeSession.amount_total || expectedCitizenshipAmountCents(plan), stripeSession.currency || 'aud', stripeSession, accessId]);
    await markServiceSessionCheckoutCreated(serviceSession.id, stripeSession.id);
...
app.post('/api/service/start', asyncRoute(async (req, res) => {
  ...
  const plan = requestedCitizenshipPlan(req, '50');
  ...
  res.json({ ok: true, service: 'citizenship_test', serviceSessionId: serviceSession.id, service_session_id: serviceSession.id, plan, next: `/login.html?service=citizenship&service_session_id=${encodeURIComponent(serviceSession.id)}&plan=${encodeURIComponent(plan)}` });
}));
...
async function handleCitizenshipCheckoutSession(req, res) {
  ...
  await query(
    `UPDATE citizenship_access
     ...
     WHERE id=$5`,
    [session.id, session.amount_total || expectedCitizenshipAmountCents(plan), session.currency || 'aud', session, accessId]
  );
  ...
  await query(
    `UPDATE assessments
     SET stripe_session_id=$1,
         ...
         updated_at=now()
     WHERE id=$4`,
    [session.id, session.amount_total || expectedCommercialAmountCents(checkoutPlan), session.currency || 'aud', assessment.id, checkoutPlan]
  );
...
function canonicalDashboardCardKey(card) {
  ...
  const serviceType = String(card.serviceType || card.service_type || card.type || '').toLowerCase();
  const stripe = String(card.stripeSessionId || card.stripe_session_id || '').trim();
  if (stripe) return `${serviceType}:stripe:${stripe}`;
  const reference = String(card.reference || card.service_ref || card.assessmentId || card.assessment_id || card.id || '').trim();
  ...
  const { rows: appealAssessments } = await query(
    `WITH linked_appeals AS (
     ...
     SELECT id, 'appeals_assessment' AS service_type, visa_subclass, decision_type, applicant_email, applicant_name,
            COALESCE(session_selected_plan, selected_plan, payment_record_plan) AS selected_plan,
            COALESCE(active_plan, session_selected_plan, selected_plan, payment_record_plan) AS active_plan,
            CASE
...
app.get('/api/account/appeals/all', requireAuth, asyncRoute(async (req, res) => {
  const { rows } = await query(
    `WITH owned AS (
     ...
     SELECT id, 'appeals_assessment' AS service_type, visa_subclass, decision_type, applicant_email, applicant_name,
            COALESCE(session_selected_plan, selected_plan, payment_record_plan, 'instant') AS selected_plan,
            COALESCE(active_plan, session_selected_plan, selected_plan, payment_record_plan, 'instant') AS active_plan,
            CASE
  ...
  res.json({ ok: true, appeals: rows, appealAssessments: rows, count: rows.length });
}));
