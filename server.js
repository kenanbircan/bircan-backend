...
function buildVisaProfiles() {
  const protectionSections = [
    ...
    { title: 'Identity, Credibility, and Evidence Risks', type: 'bullets', key: 'protectionEvidenceGaps', fallback: 'No specific protection evidence gaps were identified.' },
  ];
  ...
  return {
    ...
    'Subclass 820': { stream: 'Partner (Onshore)', displayName: 'Subclass 820', aliasOf: 'Partner Visa' },
    'Subclass 801': { stream: 'Partner (Onshore Permanent)', displayName: 'Subclass 801', aliasOf: 'Partner Visa' },
    'Subclass 309': { stream: 'Partner (Offshore)', displayName: 'Subclass 309', aliasOf: 'Partner Visa' },
...
function normaliseAnalysisShape(ai = {}, submission = {}) {
  ...
  [
    ...
    'carerNeedAssessment', 'protectionClaimBasis', 'conventionGroundAssessment', 'complementaryProtectionAssessment', 'genuineStudentAssessment',
...
function parseNumberedAssessmentBlocks(text) {
  ...
  const matches = [...source.matchAll(/(?:^|\s)(\d+)\.\s+([^:]{2,120}):\s*([\s\S]*?)(?=(?:\s\d+\.\s+[^:]{2,120}:)|$)/g)];
