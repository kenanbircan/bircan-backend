...
function readSubmissions() {
  try {
    ...
    console.error('Failed to read submissions store', error);
    return [];
  }
}

function writeSubmissions(items) {
  fs.writeFileSync(SUBMISSIONS_PATH, JSON.stringify(items, null, 2), 'utf8');
}

function upsertSubmission(record) {
  const items = readSubmissions();
  const index = items.findIndex((item) => item.submissionId === record.submissionId);
  if (index >= 0) {
    items[index] = { ...items[index], ...record, updatedAt: new Date().toISOString() };
  } else {
    items.unshift({ ...record, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
  }
...
app.get('/api/assessment/status/:submissionId', (req, res) => {
  const items = readSubmissions();
  const item = items.find((entry) => entry.submissionId === req.params.submissionId);
  if (!item) {
...
app.post('/api/assessment/submit', async (req, res) => {
  ...
  try {
    ...
    const pdfUrl = `${PUBLIC_BASE_URL}/output/${path.basename(pdfPath)}`;
    ...
    upsertSubmission(finalRecord);
    return res.json({ ok: true, ...finalRecord });
  } catch (error) {
    ...
    upsertSubmission(failedRecord);
    return res.status(500).json({ ok: false, ...failedRecord });
  }
});
