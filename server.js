...
// Node 18+ provides global fetch on Render.
...
function drawFirstPageHeader(doc, submission, analysis) {
  const left = 50;
  const right = 545;

  doc.save();
  doc.rect(0, 0, doc.page.width, 130).fill('#ffffff');
  doc.roundedRect(left, 38, 265, 70, 12).fill(PDF_THEME.navy);
  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(18).text('Bircan Migration & Education', left + 16, 56, {
    width: 230,
    align: 'left',
    lineBreak: false,
  });
  doc.fillColor('#dbe7ff').font('Helvetica').fontSize(9.5).text(
    'Migration advice and professional visa assessment services',
    left + 16,
    80,
    { width: 235 }
  );

  doc.fillColor(PDF_THEME.navy).font('Helvetica-Bold').fontSize(13).text('Kenan Bircan JP', 340, 44, {
    width: 180,
    align: 'left',
    lineBreak: false,
  });
  doc.fillColor(PDF_THEME.text).font('Helvetica').fontSize(9.3).text('Registered Migration Agent', 340, 61, {
    width: 180,
    lineBreak: false,
  });
  doc.text('MARN: 1463685 | MMIA: 10497', 340, 76, { width: 180, lineBreak: false });
  doc.text('CMA: 20156912 | JP No: 218851', 340, 90, { width: 180, lineBreak: false });

  doc.moveTo(left, 122).lineTo(right, 122).strokeColor(PDF_THEME.light).lineWidth(1).stroke();
  doc.restore();

  doc.x = left;
  doc.y = 148;

  doc.fillColor(PDF_THEME.navy).font('Helvetica-Bold').fontSize(20).text(
    `${submission.visaType || 'Subclass 482'} Preliminary Assessment & Advice`,
    left,
    doc.y,
    { width: 495 }
  );
  doc.moveDown(0.35);
  doc.fillColor(PDF_THEME.text).font('Helvetica').fontSize(11).text(
    `Outcome: ${analysis.eligibilityOutcome || 'Needs professional review'}`,
    { width: 495 }
  );
  doc.moveDown(0.6);
}

function drawRunningHeader(doc, submission) {
  const left = 50;
  const right = 545;
  doc.save();
  doc.moveTo(left, 42).lineTo(right, 42).strokeColor(PDF_THEME.light).lineWidth(1).stroke();
  doc.fillColor(PDF_THEME.navy).font('Helvetica-Bold').fontSize(10).text(
    'Bircan Migration & Education',
    left,
    28,
    { width: 200, lineBreak: false }
  );
  doc.fillColor(PDF_THEME.muted).font('Helvetica').fontSize(9).text(
    `${submission.visaType || 'Visa'} Preliminary Assessment`,
    360,
    28,
...
function startNextPage(doc, submission) {
  doc.addPage();
  drawRunningHeader(doc, submission);
}

function ensureSpace(doc, submission, threshold = 730) {
  if (doc.y > threshold) {
    startNextPage(doc, submission);
  }
}

function addSectionTitle(doc, submission, title) {
  ensureSpace(doc, submission, 700);
  doc.moveDown(0.25);
  doc.fillColor(PDF_THEME.navy).font('Helvetica-Bold').fontSize(13).text(title, 50, doc.y, { width: 495 });
  const ruleY = doc.y + 3;
  doc.moveTo(50, ruleY).lineTo(545, ruleY).strokeColor(PDF_THEME.blue).lineWidth(1).stroke();
  doc.moveDown(0.55);
}

function addParagraph(doc, submission, text, opts = {}) {
  ensureSpace(doc, submission, 738);
  doc.fillColor(opts.color || PDF_THEME.text).font(opts.bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(opts.fontSize || 10.5).text(
    String(text || ''),
    50,
    doc.y,
    { width: opts.width || 495, lineGap: opts.lineGap || 3 }
  );
  doc.moveDown(opts.afterGap || 0.55);
}
...
function addBulletList(doc, submission, items, fallbackText = 'Not provided.') {
  ...
  for (const item of list) {
    ...
    const y = doc.y;
    doc.fillColor(PDF_THEME.blue).font('Helvetica-Bold').fontSize(12).text('•', 55, y + 1, { lineBreak: false });
    doc.fillColor(PDF_THEME.text).font('Helvetica').fontSize(10.5).text(String(item || ''), 70, y, {
      width: 470,
      lineGap: 3,
    });
    doc.moveDown(0.25);
...
function addMetaTable(doc, submission, rows) {
  const left = 50;
  const totalWidth = 495;
  const labelWidth = 120;
  const valueWidth = totalWidth - labelWidth;
  rows.forEach((row, index) => {
    ensureSpace(doc, submission, 740);
    const y = doc.y;
    doc.roundedRect(left, y - 1, totalWidth, 24, 4).fill(index % 2 === 0 ? '#f7f9fc' : '#ffffff');
    doc.fillColor(PDF_THEME.navy).font('Helvetica-Bold').fontSize(9.8).text(row[0], left + 10, y + 6, {
      width: labelWidth - 10,
      lineBreak: false,
    });
    doc.fillColor(PDF_THEME.text).font('Helvetica').fontSize(9.8).text(row[1], left + labelWidth, y + 6, {
      width: valueWidth - 10,
      lineBreak: false,
    });
    doc.y = y + 24;
...
function addClosing(doc, submission) {
  ensureSpace(doc, submission, 710);
  doc.moveDown(0.4);
  doc.fillColor(PDF_THEME.text).font('Helvetica').fontSize(10.5).text('Yours faithfully,', 50, doc.y, { width: 495 });
  doc.moveDown(1.0);
  doc.fillColor(PDF_THEME.navy).font('Helvetica-Bold').fontSize(11.5).text('Kenan Bircan JP', 50, doc.y, { width: 495 });
  doc.fillColor(PDF_THEME.text).font('Helvetica').fontSize(10.3).text('Registered Migration Agent | MARN: 1463685', 50, doc.y, { width: 495 });
  doc.text('Bircan Migration & Education', 50, doc.y, { width: 495 });
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.addPage();
    drawFirstPageHeader(doc, submission, analysis);

    addMetaTable(doc, submission, [
    ...
    addParagraph(doc, submission, analysis.disclaimer || 'This preliminary assessment is provided solely on the basis of the information supplied by the client and does not constitute legal advice or a guarantee of visa grant.');

    addClosing(doc, submission);
