...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'visa_assessment') {
    ...
    const checkoutPlan = safePlan(
      req.body.plan || req.body.selectedPlan || req.body.selected_plan || req.body.planId ||
      serviceSession.selected_plan || assessment.active_plan || assessment.selected_plan || 'instant'
    );
  ...
  if (serviceSession.service_type === 'citizenship_test') {
    const plan = normaliseCitizenshipPlan(req.body.plan || req.body.selectedPlan || req.body.selected_plan || req.body.planId || serviceSession.selected_plan || '20');
...
app.post('/api/assessment/create-checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  const reusableVisaCheckout = await getReusableOpenCheckoutSession(assessment.stripe_session_id, { serviceType: 'visa_assessment', serviceRef: assessment.id, plan: assessment.selected_plan });
  if (reusableVisaCheckout) {
    return res.json({ ok: true, reused: true, url: reusableVisaCheckout.url, sessionId: reusableVisaCheckout.id, assessmentId: assessment.id, assessment_id: assessment.id, plan: assessment.selected_plan });
  }
  ...
  }, 'visa-checkout', checkoutFingerprint({ serviceType: 'visa_assessment', serviceRef: assessment.id, plan: assessment.selected_plan, price, email: req.client.email }));
