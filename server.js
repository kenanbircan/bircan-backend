...
function normaliseAnalysisShape(ai = {}, submission = {}) {
  ...
  for (const key of ['strengths', 'concerns', 'missingInformation', 'recommendedNextSteps', 'documentChecklist']) {
    if (!Array.isArray(merged[key])) merged[key] = [];
    merged[key] = merged[key].map(item => String(item || '').trim()).filter(Boolean).slice(0, 12);
  }

  for (const key of ['eligibilityOutcome', 'overallAssessment', 'executiveSummary', 'disclaimer']) {
...
async function runAssessmentAnalysis(submission) {
  if (!OPENAI_API_KEY) {
    const analysis = normaliseAnalysisShape({
      ...
      overallAssessment: 'The submission was received successfully, but the server cannot generate an AI assessment because OPENAI_API_KEY is not configured.',
      executiveSummary: 'The questionnaire data was received. Configure OPENAI_API_KEY to generate an AI assessment letter.',
      concerns: ['OPENAI_API_KEY is not configured on the server.'],
      recommendedNextSteps: ['Add OPENAI_API_KEY in Render environment variables.', 'Redeploy the service.', 'Submit the test again.'],
    }, submission);
  ...
  const systemPrompt = [
    ...
    'Analyse the client questionnaire for a Subclass 482 style visa assessment.',
    'Be practical, careful, and professional.',
    'Do not claim grant certainty.',
    'Do not invent legal citations or policy rules that are not provided in the questionnaire.',
    'When information is incomplete, clearly identify missing information.',
  ...
  const userPrompt = [
    ...
    'Focus on strengths, risks, missing items, and next steps.',
    'Return JSON only.'
  ].join('\n');
  ...
  const analysis = normaliseAnalysisShape(parsedContent, submission);
...
function addSectionTitle(doc, title) {
  doc.moveDown(0.6);
  doc.font('Helvetica-Bold').fontSize(13).fillColor('#0b1f3a').text(title);
  doc.moveDown(0.25);
}

function addParagraph(doc, text) {
  doc.font('Helvetica').fontSize(10.5).fillColor('#172033').text(String(text || ''), {
    width: 515,
    lineGap: 3,
  });
  doc.moveDown(0.65);
...
function addBulletList(doc, items, fallbackText = 'Not provided.') {
  ...
  for (const item of list) {
    doc.font('Helvetica').fontSize(10.5).fillColor('#172033').text(`• ${String(item || '')}`, {
      width: 515,
      indent: 10,
      lineGap: 3,
    });
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(20).text('Bircan Migration & Education', 58, 56);
    doc.fillColor('#ffffff').font('Helvetica').fontSize(10).text('Preliminary Visa Assessment Letter', 58, 82);

    doc.fillColor('#172033');
    doc.y = 130;

    doc.font('Helvetica-Bold').fontSize(14).fillColor('#0b1f3a').text('Preliminary Assessment Letter');
    doc.moveDown(0.6);

    addParagraph(doc, `Date: ${todayHuman()}`);
    addParagraph(doc, `Submission ID: ${submission.id}`);
    addParagraph(doc, `Client: ${submission.client?.fullName || 'Client'}`);
    addParagraph(doc, `Email: ${submission.client?.email || 'Not provided'}`);
    addParagraph(doc, `Visa type: ${submission.visaType || 'Subclass 482'}`);
    addParagraph(doc, `Service plan: ${submission.plan?.label || 'Not provided'}`);
    addParagraph(doc, `Outcome: ${analysis.eligibilityOutcome || 'Needs professional review'}`);
    ...
    addBulletList(doc, answers.map(item => `${item.question}: ${item.answer || 'No answer provided'}`), 'No answers received.');
