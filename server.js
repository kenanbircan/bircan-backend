...
function safePlan(plan) {
  ...
  if (/(value|3\s*days|3\s*d|72\s*hours|200)/i.test(raw)) return '3d';
  return 'instant';
}
...
function findPlanInObject(value, depth = 0) {
  ...
  if (typeof value === 'string' || typeof value === 'number') {
    ...
    if (/(instant|fastest|24\s*h|24\s*hour|recommended|250|3\s*d|3\s*day|72\s*hour|value|200)/i.test(text)) return text;
    return null;
  }
...
async function attachVisaAssessmentToClientById(assessmentId, client) {
  ...
  // Production account-owner rule:
  // the portal login email owns the paid dashboard record. The applicant email
  // can be different and is preserved separately, so test/autofill applicant
  // emails no longer block checkout or hide cards from the dashboard.
  const updated = await query(
    `UPDATE assessments
     SET client_id=$1,
         client_email=$2,
         applicant_email=COALESCE(applicant_email, NULLIF($4,''), $2),
         active_plan=COALESCE(active_plan, selected_plan),
         updated_at=now()
     WHERE id=$3
     RETURNING *`,
    [client.id, client.email, assessment.id, assessmentEmail]
  );
