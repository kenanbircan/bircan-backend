...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '11.0.0-real-pdf-state-machine-single-source-of-truth',
    postgres: true,
    jsonFallback: false,
...
async function generateAssessmentPdfNow(assessmentId, accountEmail = null, options = {}) {
  ...
  return tx(async (client) => {
    ...
    if (assessment.pdf_bytes && !force) {
      if (assessment.status !== 'pdf_ready') await client.query(`UPDATE assessments SET status='pdf_ready', updated_at=now() WHERE id=$1`, [assessmentId]);
      await client.query(`UPDATE pdf_jobs SET status='completed', updated_at=now() WHERE assessment_id=$1`, [assessmentId]);
      return toPublicAssessment({ ...assessment, status: 'pdf_ready', has_pdf: true });
    }
...
function toPublicAssessment(a) {
  return {
    ...
    status: a.pdf_bytes || a.has_pdf ? 'pdf_ready' : a.status,
...
app.get('/api/account/dashboard', requireAuth, asyncRoute(async (req, res) => {
  const { rows: assessments } = await query(
    `SELECT id, visa_type, applicant_email, applicant_name, selected_plan, active_plan,
            CASE WHEN pdf_bytes IS NOT NULL THEN 'pdf_ready' ELSE status END AS status,
...
app.get('/api/assessment/:id/status', requireAuth, asyncRoute(async (req, res) => {
  const { rows } = await query(
    `SELECT id, visa_type, selected_plan, active_plan, CASE WHEN pdf_bytes IS NOT NULL THEN 'pdf_ready' ELSE status END AS status, payment_status, pdf_generated_at, pdf_filename, generation_error, CASE WHEN pdf_bytes IS NULL THEN false ELSE true END AS has_pdf
     FROM assessments WHERE id=$1 AND lower(client_email)=lower($2)`,
    [req.params.id, req.client.email]
  );
  if (!rows[0]) return res.status(404).json({ ok: false, error: 'Assessment was not found for this account.' });
...
app.post('/api/assessment/:id/email-pdf', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (!assessment) return res.status(404).json({ ok: false, error: 'Assessment was not found for this account.' });
  if (!assessment.pdf_bytes) return res.status(409).json({ ok: false, error: 'PDF not ready. The advice letter has not been issued yet.', status: assessment.status, generationError: assessment.generation_error || null });
