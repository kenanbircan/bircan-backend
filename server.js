...
async function getServiceSessionForCheckout(req) {
  ...
  if (normaliseEmail(session.client_email) !== normaliseEmail(req.client.email)) {
    throw Object.assign(new Error(`This service was started with ${session.client_email}, but you are logged in as ${req.client.email}. Use the same email address.`), { statusCode: 409 });
  }
  await query(`UPDATE service_sessions SET client_id=$1, client_email=$2, updated_at=now() WHERE id=$3`, [req.client.id, req.client.email, session.id]);
