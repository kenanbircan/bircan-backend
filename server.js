...
const KNOWLEDGEBASE_EXTENSIONS = new Set(['.docx', '.txt', '.md', '.json', '.csv', '.html', '.htm']);
const KNOWLEDGEBASE_MAX_CHARS_PER_CHUNK = Number(process.env.KNOWLEDGEBASE_MAX_CHARS_PER_CHUNK || 1800);
const KNOWLEDGEBASE_CHUNK_OVERLAP = Number(process.env.KNOWLEDGEBASE_CHUNK_OVERLAP || 200);
const KNOWLEDGEBASE_TOP_K = Number(process.env.KNOWLEDGEBASE_TOP_K || 6);
let knowledgebaseCache = { builtAt: null, files: [], chunks: [], stats: { fileCount: 0, chunkCount: 0, docxSupported: Boolean(mammoth) } };
...
function walkFilesRecursive(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) found.push(...walkFilesRecursive(fullPath));
...
async function readKnowledgebaseFileText(filePath) {
...
function inferKnowledgeTags(filePath) {
  const rel = path.relative(KNOWLEDGEBASE_DIR, filePath).replace(/\\/g, '/').toLowerCase();
  const parts = rel.split('/').filter(Boolean);
  const stem = path.basename(rel, path.extname(rel));
  const tokens = (parts.join(' ') + ' ' + stem)
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter(Boolean);
...
function looksLikeHeading(line) {
  ...
  if (/^[A-Z][A-Za-z0-9\s\-/(),]{2,100}$/.test(value) && value === value.toUpperCase()) return true;
  return /^[A-Z][A-Za-z0-9\s\-/(),]{2,100}$/.test(value) && !/[.;]$/.test(value);
}

function chunkKnowledgeText(text, meta = {}) {
  ...
  const flush = () => {
    ...
    if (!body) return;
    sections.push({ heading: currentHeading || 'Policy Guidance', body });
    buffer = [];
  };

  for (const line of lines) {
    if (looksLikeHeading(line) && buffer.length) {
      flush();
      currentHeading = line;
      continue;
    }
  ...
  flush();
  if (!sections.length) sections.push({ heading: 'Policy Guidance', body: raw });

  const chunks = [];
  for (const section of sections) {
    const combined = `${section.heading}\n${section.body}`.trim();
    if (combined.length <= KNOWLEDGEBASE_MAX_CHARS_PER_CHUNK) {
      chunks.push({ ...meta, heading: section.heading, text: combined });
      continue;
    }

    let start = 0;
    while (start < combined.length) {
      let end = Math.min(start + KNOWLEDGEBASE_MAX_CHARS_PER_CHUNK, combined.length);
      if (end < combined.length) {
        const boundary = combined.lastIndexOf(' ', end);
        if (boundary > start + 400) end = boundary;
      }
      const piece = combined.slice(start, end).trim();
      if (piece) chunks.push({ ...meta, heading: section.heading, text: piece });
      if (end >= combined.length) break;
      start = Math.max(end - KNOWLEDGEBASE_CHUNK_OVERLAP, start + 1);
    }
  }

  return chunks.map((chunk, index) => ({ ...chunk, chunkId: `${sanitizeFileName(chunk.source || 'kb')}_${index + 1}` }));
}

function extractSearchTerms(submission, profile = {}) {
  const baseTerms = [
    submission.visaType,
    profile.displayName,
    profile.stream,
  ...
  const terms = baseTerms.split(/[^a-z0-9]+/).filter(token => token && token.length > 2);
  const stop = new Set(['the','and','for','with','from','that','this','have','has','are','was','were','will','your','their','about','into','under','visa','subclass','application','client','assessment','migration','australia','australian']);
  const freq = new Map();
  for (const term of terms) {
    if (stop.has(term)) continue;
    freq.set(term, (freq.get(term) || 0) + 1);
  }
  return [...freq.entries()].sort((a,b)=>b[1]-a[1]).slice(0,40).map(([term])=>term);
}

function scoreKnowledgeChunk(chunk, terms = [], submission = {}, profile = {}) {
  const haystack = `${chunk.text} ${chunk.heading} ${(chunk.tags || []).join(' ')}`.toLowerCase();
  let score = 0;
  const visaBits = [String(submission.visaType || '').toLowerCase(), String(profile.displayName || '').toLowerCase(), String(profile.stream || '').toLowerCase()];
  for (const bit of visaBits) {
    if (!bit) continue;
    if (haystack.includes(bit)) score += 12;
  }
  for (const term of terms) {
    if (haystack.includes(term)) score += Math.min(5, 1 + term.length / 8);
  }
  if ((chunk.source || '').toLowerCase().includes(String(submission.visaType || '').replace(/\s+/g,'').toLowerCase())) score += 10;
  return score;
}

async function buildKnowledgebaseIndex(force = false) {
  if (!force && knowledgebaseCache.builtAt) return knowledgebaseCache;
  const files = walkFilesRecursive(KNOWLEDGEBASE_DIR)
    .filter(filePath => KNOWLEDGEBASE_EXTENSIONS.has(path.extname(filePath).toLowerCase()))
    .sort();
  ...
  for (const filePath of files) {
    try {
      const text = await readKnowledgebaseFileText(filePath);
      const relativePath = path.relative(KNOWLEDGEBASE_DIR, filePath).replace(/\\/g, '/');
      ...
      const entry = {
        ...
        tags: inferKnowledgeTags(filePath),
        textLength: text.length,
      };
      indexedFiles.push(entry);
      if (!text) continue;
      chunks.push(...chunkKnowledgeText(text, { source: relativePath, tags: entry.tags }));
    } catch (error) {
      indexedFiles.push({ path: path.relative(KNOWLEDGEBASE_DIR, filePath).replace(/\\/g, '/'), absolutePath: filePath, ext: path.extname(filePath).toLowerCase(), error: error.message, tags: inferKnowledgeTags(filePath), size: 0, textLength: 0 });
    }
  }
  ...
  knowledgebaseCache = {
    ...
    stats: {
      ...
      docxSupported: Boolean(mammoth),
    }
  };
  return knowledgebaseCache;
}

async function selectRelevantKnowledgeChunks(submission, profile = {}) {
  const index = await buildKnowledgebaseIndex();
  if (!index.chunks.length) return { selected: [], stats: index.stats };
  const terms = extractSearchTerms(submission, profile);
  const scored = index.chunks
    .map(chunk => ({ chunk, score: scoreKnowledgeChunk(chunk, terms, submission, profile) }))
  ...
  for (const item of scored) {
    const sourceCount = seenSources.get(item.chunk.source) || 0;
    if (sourceCount >= 2) continue;
    selected.push(item);
    seenSources.set(item.chunk.source, sourceCount + 1);
    if (selected.length >= KNOWLEDGEBASE_TOP_K) break;
  }

  return {
    selected: selected.map(item => ({ source: item.chunk.source, heading: item.chunk.heading, text: item.chunk.text, score: item.score })),
    stats: index.stats,
  };
}

function formatKnowledgebaseContext(selected = []) {
  if (!Array.isArray(selected) || !selected.length) return 'No local knowledgebase excerpts were selected for this assessment.';
  return selected.map((item, index) => {
    const excerpt = String(item.text || '').replace(/\s+/g, ' ').trim();
    return `[KB ${index + 1}] Source: ${item.source}${item.heading ? ` | Section: ${item.heading}` : ''}\n${excerpt}`;
  }).join('\n\n');
}
...
function normaliseAnalysisShape(ai = {}, submission = {}) {
  ...
  const merged = { ...fallback, ...ai, ratings: { ...fallback.ratings, ...(ai.ratings || {}) }, legalMetrics: (ai && typeof ai.legalMetrics === 'object' && !Array.isArray(ai.legalMetrics)) ? ai.legalMetrics : {} };
...
async function runAssessmentAnalysis(submission) {
  ...
  const systemPrompt = [
    ...
    'For protection visas, legalMetrics must reflect protection-law analysis such as validity/jurisdiction, Convention ground strength, credibility, corroboration and country information, state protection/internal relocation, complementary protection fallback, and lodgement timing.',
  ...
  const knowledgebaseResult = await selectRelevantKnowledgeChunks(submission, profile);
  const knowledgebaseContext = formatKnowledgebaseContext(knowledgebaseResult.selected);
  ...
  const userPrompt = [
    ...
    '- Use the knowledgebase excerpts below as internal case-officer-style policy guidance where relevant. Prefer the supplied facts if there is a conflict in factual detail.',
    ...visaFocus,
    '',
  ...
  return {
    ...
    knowledgebase: {
      ...
      usedSources: knowledgebaseResult.selected.map(item => ({ source: item.source, heading: item.heading, score: item.score })),
    },
  };
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    const metricPairs = buildMetricPairs(visaProfile, analysis);
    if (metricPairs.length) {
      addSectionTitle(doc, (visaProfile.legalMetrics && visaProfile.legalMetrics.title) || 'Assessment Ratings Snapshot');
      addLegalMetricsSection(doc, (visaProfile.legalMetrics && visaProfile.legalMetrics.title) || 'Assessment Ratings Snapshot', metricPairs);
    }
...
app.get('/api/meta/knowledgebase', async (_req, res, next) => {
  try {
    const index = await buildKnowledgebaseIndex();
    res.json({ ok: true, ...index.stats, builtAt: index.builtAt, files: index.files.map(file => ({ path: file.path, ext: file.ext, size: file.size, textLength: file.textLength || 0, tags: file.tags || [], error: file.error || null })) });
  } catch (error) {
    next(error);
  }
});
...
app.get('/api/health', async (_req, res) => {
  res.json({
    ...
    knowledgebase: (await buildKnowledgebaseIndex()).stats,
  });
});
...
app.get('/api/assessment/:submissionId/status', async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      knowledgebase: submission.analysis?.knowledgebase || null,
    });
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    res.json({
      ...
      items: submissions.slice(0, 200).map(item => ({
        ...
        knowledgebaseSourcesUsed: Array.isArray(item.analysis?.knowledgebase?.usedSources) ? item.analysis.knowledgebase.usedSources.length : 0,
      })),
    });
  } catch (error) {
    next(error);
...
app.listen(PORT, () => {
  ...
  buildKnowledgebaseIndex().then(index => {
    console.log(`[knowledgebase] files=${index.stats.fileCount} chunks=${index.stats.chunkCount} docxSupported=${index.stats.docxSupported}`);
  }).catch(error => {
    console.error('[knowledgebase-build-error]', error);
  });
});
