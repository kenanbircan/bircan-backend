...
function drawHeader(doc) {
  ...
  doc.roundedRect(40, 28, 515, 92, 14).fill('#0b1f3a');

  doc.fillColor('#ffffff');
  doc.font('Helvetica-Bold').fontSize(20).text('Bircan Migration & Education', 58, 48, {
    width: 290,
    lineBreak: false,
  });
  doc.font('Helvetica').fontSize(10).text('Preliminary Visa Assessment Letter', 58, 74, {
    width: 230,
    lineBreak: false,
  });

  doc.font('Helvetica').fontSize(8.5);
  doc.text('www.bircanmigration.com.au', 320, 44, { width: 185, align: 'right', lineBreak: false });
  doc.text('kenan@bircanmigration.com.au', 320, 58, { width: 185, align: 'right', lineBreak: false });
  doc.text('0421 618 522', 320, 72, { width: 185, align: 'right', lineBreak: false });
  doc.text('5-7 Northumberland Road Auburn NSW 2144', 290, 86, { width: 215, align: 'right' });

  doc.circle(522, 74, 18).lineWidth(2).strokeColor('#8fb4ff').stroke();
  doc.fillColor('#8fb4ff').font('Helvetica-Bold').fontSize(12).text('BM', 514, 69, { width: 16, align: 'center', lineBreak: false });
...
function drawFooterOnCurrentPage(doc, pageNumber, pageCount) {
  ...
  doc.save();
  doc.moveTo(40, footerY - 10).lineTo(555, footerY - 10).strokeColor('#dce4ef').stroke();
  ...
  doc.text(`Page ${pageNumber} of ${pageCount}`, 440, footerY, { width: 115, align: 'right', lineBreak: false });
  doc.restore();
  doc.fillColor('#172033');
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.on('pageAdded', () => {
      ...
      doc.save();
      doc.fillColor('#5f6b7d').font('Helvetica').fontSize(9);
      doc.text(`Page ${pageNumber}`, 470, doc.page.height - 28, { width: 70, align: 'right', lineBreak: false });
      doc.restore();
      doc.fillColor('#172033');
    });
    ...
    doc.save();
    doc.fillColor('#5f6b7d').font('Helvetica').fontSize(9);
    doc.text('Page 1', 470, doc.page.height - 28, { width: 70, align: 'right', lineBreak: false });
    doc.restore();
