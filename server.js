...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.1.1-citizenship-checkout-compatibility',
    postgres: true,
    jsonFallback: false,
...
async function handleCitizenshipCheckoutSession(req, res) {
  ...
  const successUrl = `${APP_BASE_URL}/payment-complete.html?session_id={CHECKOUT_SESSION_ID}&service=citizenship`;
...
app.post('/api/citizenship/verify-payment', requireAuth, asyncRoute(async (req, res) => {
  if (!stripe) return res.status(500).json({ ok: false, error: 'Stripe is not configured.' });
  ...
  res.json({ ok: true, status: 'paid', sessionId, accessId: result.accessId, plan: result.plan });
}));
