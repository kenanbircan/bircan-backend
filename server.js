...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '10.0.1-finalise-db-migration-safe',
    postgres: true,
    jsonFallback: false,
    stripeConfigured: Boolean(stripe),
    smtpConfigured: Boolean(process.env.SMTP_HOST),
    appBaseUrl: APP_BASE_URL,
    corsPatch: 'matched-frontend-backend-final',
...
async function attachPaidSession(session, options = {}) {
  ...
  await tx(async (client) => {
    ...
    await client.query(
      `INSERT INTO payments (client_id, client_email, service_type, service_ref, visa_type, plan, stripe_session_id, stripe_payment_intent, amount_cents, currency, status, raw_payload)
       ...
       ON CONFLICT (stripe_session_id) DO UPDATE SET status='paid', raw_payload=EXCLUDED.raw_payload, updated_at=now()`,
  ...
  return { attached: true, assessmentId, pdfReady: Boolean(pdfResult && pdfResult.has_pdf !== false), pdf: pdfResult };
}
