...
import {
  ...
  updateSubmissionByCheckoutSession,upsertDocument,getDocumentForSubmission,audit,attachSubmissionPdf,findSubmissionByCheckoutKey
} from "./storage.js";
...
async function markCheckoutPaid(session){
  ...
  if(updated){
    audit("payment.paid",{userId:updated.userId||updated.metadata?.userId,email,submissionId:updated.id,checkoutSessionId:session.id});
    updated=await ensurePdfForSubmission(updated);
  }
  return updated;
}
...
async function ensurePdfForSubmission(item) {
  if (!item) return null;
  const current = getSubmission(item.id) || item;
  const hasDoc = getDocumentForSubmission(current.id);
  const source = resolvePdfSource(current);
  if ((current.pdfStatus === "ready" && source.kind !== "missing") || hasDoc) return getSubmission(current.id) || current;
  updateSubmission(current.id, { pdfStatus: "generating" });
  try {
    const filePath = await generateAssessmentPdf(getSubmission(current.id) || current);
    attachSubmissionPdf(current.id, {
      status: "ready",
      filePath,
      filename: `visa-assessment-${current.id}.pdf`,
      userId: current.userId || current.metadata?.userId || "",
      email: current.client?.email || current.stripeCustomerEmail || ""
    });
    audit("pdf.generated", { submissionId: current.id, filePath });
    return getSubmission(current.id) || current;
  } catch (err) {
    console.error("PDF generation failed:", err);
    updateSubmission(current.id, { pdfStatus: "error", pdfError: err.message || String(err) });
    audit("pdf.failed", { submissionId: current.id, error: err.message || String(err) });
    return getSubmission(current.id) || current;
  }
}

function checkoutKeyFromRequest(req, applicantEmail, plan) {
  return String(req.body?.checkoutKey || req.body?.idempotencyKey || req.body?.assessmentPayload?.metadata?.checkoutKey || `${req.user?.sub || req.user?.id || ""}:${applicantEmail}:${req.body?.subclass || req.body?.assessmentPayload?.subclass || req.body?.assessmentPayload?.visaSubclass || ""}:${plan}`).trim();
}
...
app.post("/api/assessment/create-checkout-session",requireAuth,async(req,res)=>{
  ...
  const checkoutKey=checkoutKeyFromRequest(req, applicantEmail, plan);
  const existing=checkoutKey?findSubmissionByCheckoutKey(checkoutKey):null;
  if(existing){
    if(existing.paymentStatus==="paid"){
      const ready=await ensurePdfForSubmission(existing);
      return res.json({ok:true,alreadyPaid:true,redirectUrl:`${APP_BASE_URL}/account-dashboard.html`,submissionId:ready.id,plan,applicantEmail});
    }
    if(existing.checkoutSessionId && existing.checkoutUrl){
      audit("checkout.reused",{userId:req.user.sub,email:applicantEmail,submissionId:existing.id,checkoutSessionId:existing.checkoutSessionId,checkoutKey});
      return res.json({ok:true,url:existing.checkoutUrl,sessionId:existing.checkoutSessionId,plan,applicantEmail,priceId,reused:true,submissionId:existing.id});
    }
  }

  const fullName=extractApplicantName(assessmentPayload,req.body)||req.user.fullName||"";
  const submission=existing || createSubmission({...assessmentPayload,email:applicantEmail,fullName,userId:req.user.sub,checkoutKey,visaSubclass:req.body?.subclass||assessmentPayload?.subclass||assessmentPayload?.visaSubclass||"",selectedPlan:plan,metadata:{...(assessmentPayload?.metadata||{}),checkoutKey,userId:req.user.sub,accountEmail:normalizeEmail(req.user.email),clientEmail:applicantEmail,applicantEmail,selectedPlan:plan,stripeMode:STRIPE_MODE}});
  const session=await stripe.checkout.sessions.create({
    ...
    metadata:{product:"visa_assessment",checkoutKey,plan,selectedPlan:plan,userId:req.user.sub,email:applicantEmail,accountEmail:normalizeEmail(req.user.email),submissionId:submission.id,subclass:String(req.body?.subclass||assessmentPayload?.subclass||assessmentPayload?.visaSubclass||"")}
  });
  updateSubmission(submission.id,{checkoutSessionId:session.id,checkoutUrl:session.url,status:"checkout_created",paymentStatus:"pending",stripeCustomerEmail:applicantEmail,checkoutKey,metadata:{...(submission.metadata||{}),checkoutKey,userId:req.user.sub,accountEmail:normalizeEmail(req.user.email),clientEmail:applicantEmail,applicantEmail,selectedPlan:plan,stripeMode:STRIPE_MODE},visa:{...submission.visa,selectedPlan:plan}});
  audit("checkout.created",{userId:req.user.sub,email:applicantEmail,submissionId:submission.id,checkoutSessionId:session.id,checkoutKey,plan});
  res.json({ok:true,url:session.url,sessionId:session.id,plan,applicantEmail,priceId,submissionId:submission.id});
});
...
app.post("/api/assessment/:id/generate-pdf",requireAuth,async(req,res)=>{
  const item=getSubmission(req.params.id);
  if(!item)return res.status(404).json({ok:false,error:"Assessment was not found."});
  if(!ownsSubmission(req.user,item))return res.status(403).json({ok:false,error:"This assessment does not belong to the logged-in account."});
  if(item.paymentStatus!=="paid")return res.status(402).json({ok:false,error:"PDF can only be generated after payment is verified."});
  const updated=await ensurePdfForSubmission(item);
