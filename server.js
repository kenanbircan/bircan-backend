...
const APP_BASE_URL = (process.env.APP_BASE_URL || process.env.FRONTEND_URL || 'http://localhost:3000').replace(/\/$/, '');
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || `${APP_BASE_URL},http://localhost:3000,http://127.0.0.1:3000,null,file://`)
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
