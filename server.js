...
// ---- Appeals checkout route aliases ----
// These routes prevent old frontend login/payment flows from falling through to
// the generic citizenship checkout route. They create the appeals Stripe product
// using STRIPE_PRICE_APPEAL_* and the existing appeals_assessments record.
async function handleAppealsCheckoutSessionAlias(req, res) {
  if (!stripe) return res.status(500).json({ ok: false, error: 'Stripe is not configured.' });
  ...
  const serviceSessionId = String(req.body.serviceSessionId || req.body.service_session_id || '').trim();
  ...
  if (serviceSessionId) {
    ...
    serviceSession = serviceRows[0] || null;
    if (serviceSession && serviceSession.service_ref) finalAssessmentId = serviceSession.service_ref;
  }

  if (!finalAssessmentId) {
    return res.status(400).json({ ok: false, error: 'Missing appeals assessment ID or service session ID.' });
  }

  const { rows } = await query('SELECT * FROM appeals_assessments WHERE id=$1', [finalAssessmentId]);
  const assessment = rows[0];
  if (!assessment) return res.status(404).json({ ok: false, error: 'Appeals assessment was not found.' });
  ...
  const plan = safePlan(
    ...
    (serviceSession && serviceSession.selected_plan) ||
  ...
  const price = resolveAppealPriceId(plan);
  if (!price) {
    const key = plan === 'instant' ? 'INSTANT' : plan === '24h' ? '24H' : '3D';
    return res.status(500).json({ ok: false, error: `Missing Stripe price for appeals plan ${plan}. Add STRIPE_PRICE_APPEAL_${key} in Render.` });
  }
  ...
  await query(
    `UPDATE appeals_assessments
     ...
     WHERE id=$4`,
    [req.client.id, req.client.email, plan, finalAssessmentId]
  );
  ...
  if (serviceSession) {
    await upsertServiceSession({
      ...
      serviceType: 'appeals_assessment',
      serviceRef: finalAssessmentId,
      email: req.client.email,
      clientId: req.client.id,
      plan,
      status: 'draft_created',
      metadata: {
        original_started_email: (serviceSession.metadata && serviceSession.metadata.original_started_email) || assessment.applicant_email || assessment.client_email || null,
  ...
  const idempotencyKey = serviceSession
    ? `service-appeals-checkout-${serviceSession.id}-${plan}`
    : `appeals-checkout-${finalAssessmentId}-${plan}`;

  const stripeSession = await stripe.checkout.sessions.create({
    mode: 'payment',
    customer_email: req.client.email,
    client_reference_id: finalAssessmentId,
    line_items: [{ price, quantity: 1 }],
    success_url: `${APP_BASE_URL}/payment-complete.html?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${APP_BASE_URL}/appeals-assessment.html?cancelled=1${serviceSession ? `&service_session_id=${encodeURIComponent(serviceSession.id)}` : ''}&appeal_assessment_id=${encodeURIComponent(finalAssessmentId)}`,
    metadata: {
      service_type: 'appeals_assessment',
      service_session_id: serviceSession ? serviceSession.id : '',
      service_ref: finalAssessmentId,
      assessment_id: finalAssessmentId,
      appeal_assessment_id: finalAssessmentId,
      visa_type: assessment.visa_subclass || 'appeals',
      plan,
      client_email: req.client.email
    }
  ...
  await query(
    `UPDATE appeals_assessments
     SET stripe_session_id=$1, status='checkout_created', amount_cents=$2, currency=$3, updated_at=now()
     WHERE id=$4`,
    [stripeSession.id, stripeSession.amount_total || appealAmountCents(plan), stripeSession.currency || 'aud', finalAssessmentId]
  );

  if (serviceSession) await markServiceSessionCheckoutCreated(serviceSession.id, stripeSession.id);
  await recordAppealPaymentAuditSafe(finalAssessmentId, req.client.email, stripeSession);
  ...
  return res.json({
    ...
    serviceSessionId: serviceSession ? serviceSession.id : null,
    assessmentId: finalAssessmentId,
    appealAssessmentId: finalAssessmentId,
    plan
  });
}

app.post('/api/create-appeals-checkout-session', requireAuth, asyncRoute(handleAppealsCheckoutSessionAlias));
app.post('/create-appeals-checkout-session', requireAuth, asyncRoute(handleAppealsCheckoutSessionAlias));
app.post('/api/public/appeals/create-checkout-session', requireAuth, asyncRoute(handleAppealsCheckoutSessionAlias));
