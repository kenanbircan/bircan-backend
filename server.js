...
async function buildUnifiedDashboardPayload(user, verifiedPayment = null) {
  const visaAssessments = await getVisaAssessmentsForDashboard(user);
  const citizenshipProduct = publicCitizenshipProductForDashboard(user);
  const citizenshipTests = citizenshipProduct.entitlement ? [citizenshipProduct] : [];
  const appealsAssessments = [];
  return {
    ok: true,
    user: publicUser(user),
    verifiedPayment,
    services: [
      ...visaAssessments.map(item => ({ ...item, type: 'visa_assessment' })),
      ...citizenshipTests.map(item => ({ ...item, type: 'citizenship_test' })),
      ...appealsAssessments.map(item => ({ ...item, type: 'appeal_assessment' })),
    ],
    products: {
      visaAssessments,
      citizenshipTests,
