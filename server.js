...
const HARDCODED_ALLOWED_ORIGINS = [
  ...
  'https://bircan-migration-backend.onrender.com',
  'http://localhost:3000',
  'http://127.0.0.1:3000',
  'http://127.0.0.1:5500',
  'http://localhost:5500',
...
// CORS must run before API routes so bircanmigration.au can call the Render backend.
app.use(cors(corsOptionsDelegate));
app.options('*', cors(corsOptionsDelegate));

// Stripe webhook endpoints must receive the raw body BEFORE express.json().
// Keep both aliases because Stripe/Render may be configured with either URL.
...
// Lightweight diagnostic for browser/API checks.
app.get('/api/payments/finalise', (_req, res) => {
  res.json({ ok: true, route: '/api/payments/finalise', methodRequired: 'POST' });
});
