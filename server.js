...
// CORS must be registered before API routes so bircanmigration.au can call the Render backend.
...
app.post(['/create-visa-checkout-session', '/create-assessment-checkout-session', '/api/assessment/create-checkout-session', '/api/assessments/create-checkout-session', '/api/visa/create-checkout-session', '/api/checkout/visa'], async (req, res, next) => {
  try {
    ...
    // Production-safe payment completion flow:
    // Stripe returns to payment-complete.html, which finalises the session once,
    // then redirects to the dashboard. The dashboard no longer finalises payments.
    const dashboardReturnToken = encodeURIComponent(makeDashboardReturnToken(user));
    const successUrl = `${APP_BASE_URL}/payment-complete.html?session_id={CHECKOUT_SESSION_ID}&product=visa_assessment&auth_return=${dashboardReturnToken}`;
...
app.post('/create-checkout-session', async (req, res, next) => {
  try {
    ...
    if (!user) return res.status(401).json({ ok: false, error: 'Login required before Stripe checkout.' });
    const dashboardReturnToken = encodeURIComponent(makeDashboardReturnToken(user));
    const successUrl = `${APP_BASE_URL}/payment-complete.html?session_id={CHECKOUT_SESSION_ID}&product=citizenship_exam&plan=${encodeURIComponent(config.code)}&auth_return=${dashboardReturnToken}`;
...
// Dedicated payment completion endpoint. This is intentionally separate from
// the dashboard: payment-complete.html calls this once after Stripe redirects
// back, then redirects the logged-in user to /account-dashboard.html.
app.get(['/api/payments/finalise', '/api/payment/finalise', '/api/stripe/finalise-session'], (_req, res) => {
  return res.json({ ok: true, route: '/api/payments/finalise', method: 'POST required for payment finalisation', service: 'bircan-migration-backend', timestamp: nowIso() });
});

app.post(['/api/payments/finalise', '/api/payment/finalise', '/api/stripe/finalise-session'], requireCitizenshipAuth, async (req, res, next) => {

  try {
    assertStripeConfigured();
    assertStripeModeLooksConsistent();

    const sessionId = String(req.body?.sessionId || req.body?.session_id || req.query?.session_id || req.query?.sessionId || '').trim();
    ...
    const paid = session.payment_status === 'paid' || session.status === 'complete';
    if (!paid) return res.status(402).json({ ok: false, error: 'Payment has not been completed.' });

    const metadata = session.metadata || {};
    const product = String(metadata.product || req.body?.product || req.query?.product || '').trim();
    const sessionUserId = String(metadata.userId || '').trim();
    const sessionEmail = String(metadata.email || session.customer_details?.email || session.customer_email || '').trim().toLowerCase();
    ...
    if (sessionEmail && accountEmail && sessionEmail !== accountEmail) {
      return res.status(403).json({ ok: false, error: 'This payment email does not match the logged-in account.' });
    }

    if (product === 'visa_assessment') {
      const payloadToken = String(metadata.payloadToken || '').trim();
      const payload = payloadToken
        ? readJsonSafe(path.join(STORAGE_DIR, 'visa-checkout-payloads', `${sanitizeFileName(payloadToken)}.json`), null)
        : null;
      ...
      if (!submission) {
        return res.status(409).json({
          ok: false,
          error: 'Payment was confirmed, but the assessment payload was not found. Please contact admin with the Stripe session ID.',
          sessionId: session.id,
        });
      }
      ...
      return res.json({
        ...
        product: 'visa_assessment',
        sessionId: session.id,
        token: makeDashboardReturnToken(req.user),
        redirect: '/account-dashboard.html',
        assessment: publicDashboardAssessment(submission),
      });
    }

    if (product === 'citizenship_exam') {
      const entitlement = await upsertCitizenshipEntitlementFromSession(session);
      return res.json({
        ok: true,
        paid: true,
        product: 'citizenship_exam',
        sessionId: session.id,
        token: makeDashboardReturnToken(req.user),
        redirect: '/account-dashboard.html',
        entitlement: publicEntitlement(entitlement || findUserEntitlement(req.user.id)),
      });
    }

    return res.status(400).json({ ok: false, error: 'Unsupported Stripe product type.', product: product || null });
  } catch (error) {
    return next(error);
  }
});
...
app.get('/api/account/dashboard', requireCitizenshipAuth, async (req, res, next) => {
  try {
    // Production rule: dashboard only displays saved account data.
    // Payment verification/finalisation is handled by /api/payments/finalise and Stripe webhook.
    const entitlement = findUserEntitlement(req.user.id);
    const visaAssessments = await getVisaAssessmentsForDashboard(req.user);
    return res.json({ ok: true, user: publicUser(req.user), verifiedPayment: null, citizenship: { entitlement: publicEntitlement(entitlement) }, visaAssessments, assessments: visaAssessments });
  } catch (error) { return next(error); }
});
