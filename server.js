...
function normaliseCitizenshipPlan(plan) {
  ...
  if (['20', '20exam', '20exams', 'starter', 'basic', '50price', 'price50', 'amount50', '5000'].includes(compact)) return '20';
  if (['50', '50exam', '50exams', 'standard', '100price', 'price100', 'amount100', '10000'].includes(compact)) return '50';
  if (['100', '100exam', '100exams', 'premium', '150', '150price', 'price150', 'amount150', '15000', '3d', '3day', '3days', 'value'].includes(compact)) return '100';
  if (['unlimited', 'unlimitedexam', 'unlimitedexams', 'all', '200', '200price', 'price200', 'amount200', '20000'].includes(compact)) return 'unlimited';

  const label = rawOriginal.toLowerCase();
  if (label.includes('100 exams') || label.includes('100-exams') || label.includes('a$150') || label.includes('$150') || label.includes('150.00')) return '100';
  if (label.includes('50 exams') || label.includes('50-exams') || label.includes('a$100') || label.includes('$100') || label.includes('100.00')) return '50';
  if (label.includes('20 exams') || label.includes('20-exams') || label.includes('a$50') || label.includes('$50') || label.includes('50.00')) return '20';
  if (label.includes('unlimited')) return 'unlimited';

  return '20';
}

function getRequestedCitizenshipPlan(req, fallback = '20') {
  const b = req.body || {};
  const q = req.query || {};
  const nested = b.pendingAssessment || b.formData || b.payload || b.data || {};
  const candidates = [
    b.citizenshipPlan,
    b.citizenship_plan,
    b.examPlan,
    ...
    q.plan,
    q.amount,
    nested.citizenshipPlan,
    nested.citizenship_plan,
    nested.examPlan,
    nested.exam_plan,
    nested.packageId,
    nested.package_id,
  ...
  return normaliseCitizenshipPlan(candidates.length ? candidates[0] : fallback);
}
...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.2.2-citizenship-all-plan-fix',
    postgres: true,
    jsonFallback: false,
...
// Safe citizenship plan debug endpoint. No Stripe session is created.
app.post('/api/debug/citizenship-plan', (req, res) => {
  const plan = getRequestedCitizenshipPlan(req, '20');
  res.json({
    ok: true,
    input: req.body || {},
    normalisedCitizenshipPlan: plan,
    priceIdEnvExpected: `STRIPE_PRICE_CITIZENSHIP_${plan.toUpperCase()}`,
    mapping: {
      '20': 'A$50 / 20 exams',
      '50': 'A$100 / 50 exams',
      '100': 'A$150 / 100 exams',
...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'citizenship_test') {
    const plan = getRequestedCitizenshipPlan(req, serviceSession.selected_plan || '20');
...
app.post('/api/create-citizenship-checkout-session', requireAuth, asyncRoute(handleCitizenshipCheckoutSession));
app.post('/create-citizenship-checkout-session', requireAuth, asyncRoute(handleCitizenshipCheckoutSession));
app.post('/api/public/citizenship/create-checkout-session', requireAuth, asyncRoute(handleCitizenshipCheckoutSession));
