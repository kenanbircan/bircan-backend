...
function strictVisaOrAppealPlanFromRequest(req, fallback = '') {
  const body = (req && req.body) || {};
  const queryObj = (req && req.query) || {};
  const candidates = [
    body.plan, body.selectedPlan, body.selected_plan, body.planId, body.plan_id,
    body.assessmentPlan, body.assessment_plan, body.pricePlan, body.price_plan,
    body.paymentPlan, body.payment_plan, body.turnaround, body.serviceLevel, body.service_level,
    body.appealsPlan, body.appealPlan, body.reviewPlan,
    queryObj.plan, queryObj.selectedPlan, queryObj.selected_plan, queryObj.planId, queryObj.plan_id,
    queryObj.assessmentPlan, queryObj.assessment_plan, queryObj.pricePlan, queryObj.price_plan,
    queryObj.paymentPlan, queryObj.payment_plan, queryObj.turnaround, queryObj.serviceLevel, queryObj.service_level,
    queryObj.appealsPlan, queryObj.appealPlan, queryObj.reviewPlan,
    fallback
  ];
  for (const value of candidates) {
    if (value !== undefined && value !== null && String(value).trim() !== '') return safePlan(value);
  }
  const err = new Error('Payment plan is required. Checkout has been stopped because the system will not guess Instant/24H/3D.');
  err.statusCode = 400;
  throw err;
}

function serviceStartRedirect({ serviceType, serviceSession, serviceRef, plan, email }) {
  const service = normaliseServiceType(serviceType);
  const params = new URLSearchParams();
  params.set('service', service === 'visa_assessment' ? 'visa' : service === 'appeals_assessment' ? 'appeals' : 'citizenship');
  params.set('next', 'service-checkout');
  params.set('service_session_id', serviceSession.id);
  if (service === 'visa_assessment') params.set('assessment_id', serviceRef || serviceSession.service_ref || '');
  if (service === 'appeals_assessment') params.set('appeal_assessment_id', serviceRef || serviceSession.service_ref || '');
  if (service === 'citizenship_test' && serviceRef) params.set('citizenship_access_id', serviceRef);
  params.set('plan', service === 'citizenship_test' ? normaliseCitizenshipPlan(plan) : safePlan(plan));
  if (email) params.set('email', normaliseEmail(email));
...
async function getServiceSessionForCheckout(req) {
  const serviceSessionId = String(req.body.serviceSessionId || req.body.service_session_id || req.query.service_session_id || req.query.serviceSessionId || '').trim();
  const serviceType = normaliseServiceType(req.body.serviceType || req.body.service_type || req.body.service || req.query.service_type || req.query.service || '');
  const serviceRef = String(req.body.serviceRef || req.body.service_ref || req.body.assessmentId || req.body.assessment_id || req.body.appealAssessmentId || req.body.appeal_assessment_id || req.body.citizenshipAccessId || req.body.citizenship_access_id || req.query.assessment_id || req.query.appeal_assessment_id || req.query.citizenship_access_id || '').trim();
  ...
  if (serviceSessionId) {
    ...
    throw Object.assign(new Error('Checkout handoff ID is required. The system will not guess a previous unpaid checkout session.'), { statusCode: 400, code: 'SERVICE_SESSION_ID_REQUIRED' });
  }

  const session = rows && rows[0];
  if (!session) {
    throw Object.assign(new Error('Checkout handoff was not found. The assessment/payment record was not saved before login, so Stripe has been stopped.'), { statusCode: 404, code: 'SERVICE_SESSION_NOT_FOUND' });
  }
  if (session.payment_status === 'paid') {
    throw Object.assign(new Error('This checkout handoff has already been paid. Start a new assessment or package for a new payment.'), { statusCode: 409, code: 'SERVICE_SESSION_ALREADY_PAID' });
  }
...
async function handleAppealsAssessmentCreate(req, res) {
  let plan;
  try { plan = strictVisaOrAppealPlanFromRequest(req); }
  catch (err) { return res.status(err.statusCode || 400).json({ ok: false, code: 'PLAN_REQUIRED', error: err.message }); }
  ...
  if (!publicDraftKey) {
    ...
    if (duplicate) {
      ...
      return res.json({
        ...
        duplicate: true,
        plan,
        next: `/login.html?service=appeals&next=service-checkout&service_session_id=${encodeURIComponent(serviceSession.id)}&appeal_assessment_id=${encodeURIComponent(duplicate.id)}&plan=${encodeURIComponent(plan)}&email=${encodeURIComponent(email)}`
      });
    }
  }
...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'appeals_assessment') {
    ...
    const plan = strictVisaOrAppealPlanFromRequest(req, serviceSession.selected_plan || assessment.active_plan || assessment.selected_plan);
...
async function handleUnifiedServiceStart(req, res) {
  const serviceType = normaliseServiceType(req.body.serviceType || req.body.service_type || req.body.service || req.query.service || 'citizenship_test');
  if (!serviceType) return res.status(400).json({ ok: false, code: 'SERVICE_REQUIRED', error: 'Service type is required.' });

  if (serviceType !== 'citizenship_test') {
    return res.status(400).json({
      ok: false,
      code: 'SERVICE_FORM_RECORD_REQUIRED',
      error: 'Visa and appeals must first save their full form/evidence records through their dedicated start endpoints before login/payment.'
    });
  }

  const email = normaliseEmail(req.body.email || req.body.client_email || req.body.clientEmail || req.body.applicantEmail || req.body.applicant_email || req.query.email);
  if (!email || !email.includes('@')) return res.status(400).json({ ok: false, code: 'EMAIL_REQUIRED', error: 'Valid client email is required before checkout.' });
  const plan = requestedCitizenshipPlan(req, '');
  if (!['20','50','100'].includes(plan)) return res.status(400).json({ ok: false, code: 'PLAN_REQUIRED', error: 'Citizenship test package must be 20, 50, or 100.' });
  ...
  const serviceSession = await upsertServiceSession({
    ...
    metadata: {
      ...
      require_fresh_login: true,
      login_required_before_payment: true,
      source: 'unified_service_start'
    }
  });
  const next = serviceStartRedirect({ serviceType: 'citizenship_test', serviceSession, serviceRef: serviceSession.service_ref, plan, email });
  res.json({ ok: true, service: 'citizenship_test', serviceType: 'citizenship_test', service_type: 'citizenship_test', serviceSessionId: serviceSession.id, service_session_id: serviceSession.id, plan, next });
}
app.post('/api/service/start', asyncRoute(handleUnifiedServiceStart));
app.post('/api/public/service/start', asyncRoute(handleUnifiedServiceStart));
app.post('/api/unified/service/start', asyncRoute(handleUnifiedServiceStart));
app.post('/api/citizenship/start', asyncRoute(handleUnifiedServiceStart));
app.post('/api/public/citizenship/start', asyncRoute(handleUnifiedServiceStart));
...
app.post('/api/appeals/create-checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  const plan = strictVisaOrAppealPlanFromRequest(req, (linkedServiceSession && linkedServiceSession.selected_plan) || assessment.active_plan || assessment.selected_plan || requestedPlan);
  const price = resolveAppealPriceId(plan);
...
app.get('/api/account/visa/all', requireAuth, asyncRoute(async (req, res) => {
  ...
  res.json({ ok: true, visa: rows, visaAssessments: rows, assessments: rows, count: rows.length });
}));
