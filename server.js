...
async function sendIssuedAssessmentPdf(req, res, rawId) {
  // This is the canonical issued advice-letter responder.
  // It intentionally returns a real application/pdf body, not a legacy/template URL JSON payload.
  const generated = await generateAssessmentPdfNow(rawId, req.client.email);
  const { rows } = await query(
    'SELECT * FROM assessments WHERE id=$1 AND lower(client_email)=lower($2)',
    [generated.id, req.client.email]
  );
  const assessment = rows[0];
  if (!assessment) return res.status(404).json({ ok: false, error: 'Assessment was not found for this account.' });
  if (!assessment.pdf_bytes) {
    return res.status(409).json({
      ok: false,
      error: 'PDF not ready. The advice letter has not been generated yet.',
      status: assessment.status,
      generationError: assessment.generation_error || null
    });
  }

  const filename = assessment.pdf_filename || `Bircan-${assessment.visa_type || 'assessment'}-${assessment.id}.pdf`;
  res.status(200);
  ...
  res.setHeader('X-Bircan-PDF-Route', 'final-advice-letter');
  res.setHeader('X-Bircan-Assessment-Id', assessment.id);
  res.send(assessment.pdf_bytes);
}

async function generatePdfResponse(req, res, assessmentId) {
  // Production behaviour: Generate PDF now and return the issued PDF binary directly.
  // Do not return /pdf or legacy/template endpoint URLs from this route.
  await sendIssuedAssessmentPdf(req, res, assessmentId);
}
...
async function sendAssessmentPdf(req, res, rawId) {
  // Backwards-compatible wrapper, but still returns the issued final advice-letter PDF.
  await sendIssuedAssessmentPdf(req, res, rawId);
}
