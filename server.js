...
// Backwards-compatible appeals checkout aliases for older frontend login/payment flows.
// These routes intentionally create ONLY the appeals Stripe product. They must never
// fall back to citizenship or the generic checkout product.
async function handleAppealsCheckoutAlias(req, res) {
  if (!stripe) return res.status(500).json({ ok: false, error: 'Stripe is not configured.' });

  const body = req.body || {};
  const pending = isPlainObject(body.pendingAssessment) ? body.pendingAssessment : {};
  const formData = isPlainObject(body.formData) ? body.formData : isPlainObject(pending.formData) ? pending.formData : {};

  let serviceSessionId = String(
    body.serviceSessionId || body.service_session_id ||
    pending.serviceSessionId || pending.service_session_id ||
    formData.serviceSessionId || formData.service_session_id ||
    ''
  ).trim();

  let assessmentId = String(
    body.appealAssessmentId || body.appeal_assessment_id ||
    body.assessmentId || body.assessment_id || body.id ||
    pending.appealAssessmentId || pending.appeal_assessment_id ||
    pending.assessmentId || pending.assessment_id || pending.id ||
    formData.appealAssessmentId || formData.appeal_assessment_id ||
    formData.assessmentId || formData.assessment_id || formData.id ||
    ''
  ).trim();
  ...
  if (serviceSessionId) {
    const sessionRows = (await query(
      `SELECT * FROM service_sessions WHERE id=$1 AND service_type='appeals_assessment' LIMIT 1`,
      [serviceSessionId]
    )).rows;
    serviceSession = sessionRows[0] || null;
    if (serviceSession && serviceSession.service_ref) assessmentId = serviceSession.service_ref;
  }

  if (!assessmentId && normaliseEmail(req.client.email)) {
    const sessionRows = (await query(
      `SELECT * FROM service_sessions
       WHERE service_type='appeals_assessment'
         AND lower(client_email)=lower($1)
    ...
    if (serviceSession) {
      serviceSessionId = serviceSession.id;
      assessmentId = serviceSession.service_ref;
    }
  }
  ...
  if (!assessmentId) {
    return res.status(400).json({
      ok: false,
      error: 'Missing appeals assessment ID. Start the appeals assessment first, then log in and continue to payment.'
    });
  }
  ...
  if (!serviceSession) {
    ...
    serviceSession = sessionRows[0] || null;
    if (serviceSession) serviceSessionId = serviceSession.id;
  }

  if (!serviceSession) {
    serviceSession = await upsertServiceSession({
      serviceType: 'appeals_assessment',
      serviceRef: assessmentId,
      email: req.client.email,
      clientId: req.client.id,
      plan: safePlan(body.plan || body.selectedPlan || assessment.selected_plan || 'instant'),
      status: 'draft_created',
      paymentStatus: 'unpaid',
      metadata: {
        created_by: 'appeals_checkout_alias',
        original_started_email: normaliseEmail(assessment.applicant_email || assessment.client_email) || null,
        portal_login_email: normaliseEmail(req.client.email),
        portal_login_confirmed_at: new Date().toISOString(),
        fresh_login_confirmed: true
      }
    });
    ...
    const metadata = {
      ...(serviceSession.metadata || {}),
      original_started_email: (serviceSession.metadata && serviceSession.metadata.original_started_email) || normaliseEmail(assessment.applicant_email || assessment.client_email) || null,
      portal_login_email: normaliseEmail(req.client.email),
      portal_login_confirmed_at: new Date().toISOString(),
      fresh_login_confirmed: true
    };
    await query(
      `UPDATE service_sessions
       SET client_id=$1, client_email=$2, metadata=COALESCE(metadata, '{}'::jsonb) || $3::jsonb, updated_at=now()
       WHERE id=$4`,
      [req.client.id, req.client.email, JSON.stringify(metadata), serviceSession.id]
    );
    serviceSession.client_id = req.client.id;
    serviceSession.client_email = req.client.email;
    serviceSession.metadata = metadata;
  }

  const plan = safePlan(body.plan || body.selectedPlan || body.selected_plan || serviceSession.selected_plan || assessment.selected_plan || 'instant');
  const price = resolveAppealPriceId(plan);
  if (!price) {
    return res.status(500).json({
      ok: false,
      error: `Missing Stripe price for appeals plan ${plan}. Add STRIPE_PRICE_APPEAL_${plan === 'instant' ? 'INSTANT' : plan === '24h' ? '24H' : '3D'} in Render.`
    });
  }

  await query(
    `UPDATE appeals_assessments
     SET client_id=$1, client_email=$2, selected_plan=$3, active_plan=$3,
         release_at=${appealReleaseAtSql(plan)}, updated_at=now()
     WHERE id=$4`,
    [req.client.id, req.client.email, plan, assessmentId]
  );

  await upsertServiceSession({
    id: serviceSessionId,
    serviceType: 'appeals_assessment',
    serviceRef: assessmentId,
    email: req.client.email,
  ...
  const stripeSession = await stripe.checkout.sessions.create({
    ...
    line_items: [{ price, quantity: 1 }],
    success_url: `${APP_BASE_URL}/payment-complete.html?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${APP_BASE_URL}/appeals-assessment.html?cancelled=1&service_session_id=${encodeURIComponent(serviceSessionId)}&appeal_assessment_id=${encodeURIComponent(assessmentId)}`,
    metadata: {
      service_type: 'appeals_assessment',
      ...
      plan,
      client_email: req.client.email,
      route_alias: req.path
    }
  }, { idempotencyKey: `appeals-alias-checkout-${serviceSessionId || assessmentId}-${plan}` });

  await query(
    `UPDATE appeals_assessments
     SET stripe_session_id=$1, status='checkout_created', amount_cents=$2, currency=$3, updated_at=now()
     WHERE id=$4`,
    [stripeSession.id, stripeSession.amount_total || appealAmountCents(plan), stripeSession.currency || 'aud', assessmentId]
  );
  await markServiceSessionCheckoutCreated(serviceSessionId, stripeSession.id);
  await recordAppealPaymentAuditSafe(assessmentId, req.client.email, stripeSession);
  ...
  return res.json({
    ...
    service_session_id: serviceSessionId,
    assessmentId,
    appealAssessmentId: assessmentId,
    plan
  });
}

app.post('/api/create-appeals-checkout-session', requireAuth, asyncRoute(handleAppealsCheckoutAlias));
app.post('/create-appeals-checkout-session', requireAuth, asyncRoute(handleAppealsCheckoutAlias));
app.post('/api/public/appeals/create-checkout-session', requireAuth, asyncRoute(handleAppealsCheckoutAlias));
app.post('/api/checkout/appeals/create-session', requireAuth, asyncRoute(handleAppealsCheckoutAlias));
