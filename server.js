...
// -----------------------------------------------------------------------------
// Advice-letter finality validation + production queue
// -----------------------------------------------------------------------------
// This block ensures paid instant matters are processed asynchronously and
// template PDFs are never marked as issued advice letters.

function extractAdviceAnalysis(submission = {}) {
  const a = submission.analysis || submission.aiAnalysis || submission.assessmentAnalysis || submission.result || {};
  if (a && a.analysis && typeof a.analysis === 'object') return a.analysis;
  if (a && typeof a === 'object') return a;
  return {};
}

function stringifyAdviceValue(value) {
  if (!value) return '';
...
function hasRealAdviceContent(submission = {}) {
  const analysis = extractAdviceAnalysis(submission);
  const fields = [
  ...
  const joined = fields.map(stringifyAdviceValue).join('\n').trim();

  const listCount =
    (Array.isArray(analysis.criterionFramework) ? analysis.criterionFramework.length : 0) +
    (Array.isArray(analysis.departmentStyleDecisionFramework) ? analysis.departmentStyleDecisionFramework.length : 0) +
    ...
    (Array.isArray(analysis.criticalEvidenceGaps) ? analysis.criticalEvidenceGaps.length : 0) +
  ...
  const placeholderPhrases = [
    ...
    /no structured/i,
    /not provided/i,
    /no material concerns were identified from the currently available information/i,
    /no material strengths were identified from the currently available information/i,
  ];
...
function assertRealAdviceContent(submission = {}) {
  if (!hasRealAdviceContent(submission)) {
    const err = new Error('Advice letter content has not been generated. Template PDF generation blocked.');
    err.statusCode = 409;
    throw err;
  }
}

function markAdviceGenerationState(submission = {}, status, extra = {}) {
  return {
    ...submission,
    pdfStatus: status,
    documentStatus: status,
    generationStatus: status,
    adviceLetterStatus: status,
    updatedAt: nowIso(),
    ...extra,
  };
}

function buildFullAdvicePrompt(submission = {}) {
  const visaType = normaliseVisaType(submission.visaType || submission.subclass || submission.visaSubclass || submission.assessmentType || '');
  const answers = normalizeAnswers(submission.answers || submission.formData?.answers || submission.responses || []);
  const answerText = answers.map((a, i) => `${i + 1}. ${a.question || 'Question'}\nAnswer: ${a.answer || ''}`).join('\n\n');
  const facts = {
    id: submission.id,
    visaType,
    clientEmail: submission.clientEmail || submission.email || submission.applicantEmail || '',
    applicant: submission.applicantName || submission.fullName || submission.client?.fullName || '',
    plan: submission.plan || submission.assessmentPlan || submission.selectedPlan || '',
    formData: submission.formData || null
  };

  return `
You are preparing a professional preliminary Australian migration advice letter for Bircan Migration & Education.

Use Australian migration law decision-writing style. Return STRICT JSON only. Do not use markdown.

Matter facts:
${JSON.stringify(facts, null, 2)}

Assessment answers:
${answerText || JSON.stringify(submission, null, 2)}
...
Required JSON shape:
{
  ...
  "evidencePosition": "minimum 250 words",
  "detailedAssessment": "minimum 500 words",
  "strategyAdvice": "minimum 300 words",
  "refusalRiskSummary": "minimum 250 words",
  "criterionFramework": [
    {"criterion":"criterion name","assessment":"criterion-specific analysis","risk":"low/moderate/high","evidenceRequired":"evidence required"}
  ],
  "refusalExposure": ["specific refusal exposure 1", "specific refusal exposure 2"],
  "criticalEvidenceGaps": ["gap 1", "gap 2"],
  "curableIssues": ["issue 1", "issue 2"],
  "thresholdBlockers": ["blocker if any"],
...
- Do not say "not generated", "not available", or "not provided" except where describing a missing document as an evidence gap.
- Give criterion-by-criterion reasoning.
- If facts are missing, identify the evidence gap and explain its legal effect.
`.trim();
}

async function callOpenAiForFullAdvice(submission = {}) {
  if (!OPENAI_API_KEY) {
  ...
  const response = await fetch(`${OPENAI_BASE_URL}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      response_format: { type: 'json_object' },
      messages: [
        { role: 'system', content: 'You are a senior Australian migration advice writer. Return complete JSON only.' },
        { role: 'user', content: buildFullAdvicePrompt(submission) }
      ],
    }),
  });
  ...
  if (!response.ok) {
    const err = new Error(data.error?.message || `OpenAI advice generation failed with HTTP ${response.status}`);
    err.statusCode = 502;
    throw err;
  }

  const content = data.choices?.[0]?.message?.content || '';
  if (!content || content.length < 900) {
    const err = new Error('OpenAI returned insufficient advice-letter content.');
    err.statusCode = 502;
    throw err;
  }
  ...
  try {
    ...
    const err = new Error('OpenAI advice-letter JSON could not be parsed.');
    err.statusCode = 502;
    throw err;
  }
  ...
  const next = {
    ...submission,
    analysis: {
      ...(submission.analysis && typeof submission.analysis === 'object' ? submission.analysis : {}),
      analysis: parsed,
      generatedAt: nowIso(),
      model: OPENAI_MODEL_ANALYSIS || 'gpt-4.1',
    },
...
async function ensureFullAdviceLetterReady(submissionId, options = {}) {
  const cleanId = typeof normaliseDocumentSubmissionId === 'function'
    ? normaliseDocumentSubmissionId(submissionId)
    : String(submissionId || '').trim();

  let submission = await getSubmission(cleanId);
  if (!submission) {
  ...
  submission = await saveSubmission(markAdviceGenerationState(submission, 'generating', {
    pdfError: '',
    generationStartedAt: submission.generationStartedAt || nowIso(),
  }));

  try {
    if (options.force || options.regenerate || !hasRealAdviceContent(submission)) {
      submission = await callOpenAiForFullAdvice(submission);
    }

    assertRealAdviceContent(submission);
    ...
    if (size < 25000) {
      const err = new Error('Generated PDF appears to be a template or incomplete file.');
      err.statusCode = 422;
      throw err;
    }

    const pdfUrl = generated?.pdfUrl || generated?.downloadUrl || submission.pdfUrl || `/pdfs/${path.basename(pdfPath)}`;
    ...
    const failed = await saveSubmission(markAdviceGenerationState(submission, 'failed', {
...
// Simple production-safe in-process queue for Render single-worker deployments.
// For multi-instance deployments, move this to a DB-backed queue.
const ADVICE_JOB_QUEUE = [];
const ADVICE_JOB_INDEX = new Map();
let ADVICE_QUEUE_RUNNING = false;

const ADVICE_QUEUE_CONFIG = {
  maxAttempts: Number(process.env.ADVICE_QUEUE_MAX_ATTEMPTS || 3),
  retryDelayMs: Number(process.env.ADVICE_QUEUE_RETRY_DELAY_MS || 5000),
  staleAfterMs: Number(process.env.ADVICE_QUEUE_STALE_AFTER_MS || 20 * 60 * 1000),
};

function cleanQueueId(value) {
  return typeof normaliseDocumentSubmissionId === 'function'
    ? normaliseDocumentSubmissionId(value)
    : String(value || '').trim();
}

function getAdviceJob(id) {
  const cleanId = cleanQueueId(id);
  return ADVICE_JOB_INDEX.get(cleanId) || null;
}

async function updateAdviceJobSubmissionState(id, status, extra = {}) {
  const cleanId = cleanQueueId(id);
  const submission = await getSubmission(cleanId);
  if (!submission) return null;
  const next = await saveSubmission(markAdviceGenerationState(submission, status, extra));
  if (typeof syncDbServiceDocumentFromSubmission === 'function') {
    await syncDbServiceDocumentFromSubmission(next).catch(() => null);
  }
  return next;
}

function enqueueAdviceGeneration(submissionId, options = {}) {
  const id = cleanQueueId(submissionId);
  if (!id) {
    const err = new Error('Missing assessment ID.');
    err.statusCode = 400;
    throw err;
  }

  const existing = ADVICE_JOB_INDEX.get(id);
  if (existing && ['queued', 'running', 'retrying'].includes(existing.status)) {
    return existing;
  }
  ...
  const job = {
    ...
    status: 'queued',
    attempts: 0,
    maxAttempts: Number(options.maxAttempts || ADVICE_QUEUE_CONFIG.maxAttempts),
  ...
  ADVICE_JOB_INDEX.set(id, job);
  ADVICE_JOB_QUEUE.push(job);

  updateAdviceJobSubmissionState(id, 'queued', {
    pdfError: '',
    queuedAt: job.createdAt,
    queueAttempts: 0,
  }).catch(err => console.error('[advice-queue] failed to mark queued', id, err.message));

  setImmediate(processAdviceQueue);
  return job;
}

function adviceQueueSnapshot() {
  return {
    running: ADVICE_QUEUE_RUNNING,
    queued: ADVICE_JOB_QUEUE.length,
    jobs: Array.from(ADVICE_JOB_INDEX.values()).map(job => ({
      id: job.id,
      status: job.status,
...
async function runAdviceJob(job) {
  job.status = 'running';
  job.attempts += 1;
  job.updatedAt = nowIso();

  await updateAdviceJobSubmissionState(job.id, 'generating', {
    queueAttempts: job.attempts,
    generationStartedAt: nowIso(),
    pdfError: '',
  });

  console.log(`[advice-queue] running ${job.id} attempt ${job.attempts}/${job.maxAttempts}`);

  try {
    const submission = await ensureFullAdviceLetterReady(job.id, {
      force: job.force,
    ...
    ADVICE_JOB_INDEX.delete(job.id);

    console.log(`[advice-queue] completed ${job.id}`);
    return submission;
  } catch (error) {
    job.lastError = error.message || 'Advice generation failed.';
    job.updatedAt = nowIso();

    console.error(`[advice-queue] failed ${job.id} attempt ${job.attempts}: ${job.lastError}`);

    if (job.attempts < job.maxAttempts) {
      job.status = 'retrying';
      await updateAdviceJobSubmissionState(job.id, 'retrying', {
        queueAttempts: job.attempts,
        pdfError: job.lastError,
        nextRetryAt: new Date(Date.now() + ADVICE_QUEUE_CONFIG.retryDelayMs * job.attempts).toISOString(),
      });

      await new Promise(resolve => setTimeout(resolve, ADVICE_QUEUE_CONFIG.retryDelayMs * job.attempts));
      ADVICE_JOB_QUEUE.push(job);
    } else {
      job.status = 'failed';
      job.failedAt = nowIso();
      await updateAdviceJobSubmissionState(job.id, 'failed', {
        queueAttempts: job.attempts,
        pdfError: job.lastError,
        generationFailedAt: job.failedAt,
      });
      // Keep failed jobs visible for diagnostics.
    }
  }
}

async function processAdviceQueue() {
  if (ADVICE_QUEUE_RUNNING) return;
  ADVICE_QUEUE_RUNNING = true;

  try {
    while (ADVICE_JOB_QUEUE.length) {
      const job = ADVICE_JOB_QUEUE.shift();
      if (!job || job.status === 'completed') continue;
      await runAdviceJob(job);
    }
  } finally {
    ADVICE_QUEUE_RUNNING = false;
  }
}

async function recoverStaleAdviceJobs() {
  const all = await getAllSubmissions().catch(() => []);
  const staleStatuses = new Set(['queued', 'generating', 'retrying', 'processing']);
  const now = Date.now();
  for (const submission of all) {
    const status = String(submission.generationStatus || submission.pdfStatus || submission.documentStatus || '').toLowerCase();
    if (!staleStatuses.has(status)) continue;
    const updated = new Date(submission.updatedAt || submission.generationStartedAt || submission.createdAt || 0).getTime();
    if (Number.isFinite(updated) && now - updated < ADVICE_QUEUE_CONFIG.staleAfterMs) continue;
    enqueueAdviceGeneration(submission.id, { force: false, regenerate: false });
  }
}

setTimeout(() => {
  recoverStaleAdviceJobs().catch(err => console.error('[advice-queue] stale recovery failed', err.message));
}, 5000);
...
async function ensureSubmissionPdfReady(submission) {
  ...
  // If the physical PDF is missing, generate it on demand from the stored
  // submission/analysis data and then persist the resulting path and URL.
  try {
    const generated = await generateProfessionalPdf(submission);
    if (generated?.pdfPath && fs.existsSync(generated.pdfPath)) {
      const patched = {
        ...
        documentStatus: 'ready',
        completedAt: submission.completedAt || submission.processedAt || nowIso(),
        pdfIssuedAt: submission.pdfIssuedAt || nowIso(),
      };
      await saveSubmission(patched);
      await syncDbServiceDocumentFromSubmission(patched).catch(() => null);
      return patched;
    ...
    console.error('[pdf] on-demand generation failed', submission.id, error);
  }
...
async function resolveReadyAssessmentPdf(rawId) {
  ...
  // Last-resort file-system lookup. This fixes cases where the dashboard has a
  // ready matter id but the JSON record has not yet been updated with pdfPath.
  const safe = sanitizeFileName(id);
  const files = await fsp.readdir(PDF_DIR).catch(() => []);
  const match = files.find(name => name.endsWith('.pdf') && (name.includes(safe) || safe.includes(name.replace(/\.pdf$/i, ''))));
  if (match) {
    const pdfPath = path.join(PDF_DIR, match);
    const patched = {
      ...(submission || {}),
      id,
      pdfPath,
      pdfFileName: match,
      pdfUrl: `/api/assessment/${encodeURIComponent(id)}/pdf`,
      downloadUrl: `/api/assessment/${encodeURIComponent(id)}/pdf`,
      pdfStatus: 'ready',
      documentStatus: 'ready',
      pdfIssuedAt: submission?.pdfIssuedAt || nowIso(),
      completedAt: submission?.completedAt || nowIso(),
    };
    await saveSubmission(patched).catch(() => null);
    await syncDbServiceDocumentFromSubmission(patched).catch(() => null);
    return { submission: patched, pdfPath };
  }
...
// -----------------------------------------------------------------------------
// Queue-based advice-letter generation API
// -----------------------------------------------------------------------------
app.post(['/api/assessment/:id/generate-pdf', '/api/assessment/:id/generate', '/api/assessments/:id/generate-pdf'], async (req, res) => {
  const id = cleanQueueId(req.params.id || req.body?.assessmentId || req.body?.submissionId || req.body?.id);
  ...
  try {
    ...
    if (hasRealAdviceContent(submission) && submission.pdfUrl && submission.pdfStatus === 'ready') {
      return res.json({
        ok: true,
        ready: true,
        status: 'ready',
        ...
        downloadUrl: submission.downloadUrl || submission.pdfUrl,
        documentUrl: submission.documentUrl || submission.pdfUrl,
        pdfIssuedAt: submission.pdfIssuedAt || submission.completedAt || null,
      });
    }

    const job = enqueueAdviceGeneration(id, {
    ...
    return res.status(202).json({
      ...
      attempts: job.attempts,
      maxAttempts: job.maxAttempts,
      message: 'Advice-letter generation has been queued.',
    });
  } catch (error) {
    return res.status(error.statusCode || 500).json({
      ok: false,
      ready: false,
      error: error.message || 'Advice-letter generation could not be queued.',
    });
  }
});

app.post('/api/assessment/generate-pdf', async (req, res) => {
  const id = cleanQueueId(req.body?.assessmentId || req.body?.submissionId || req.body?.id);
  if (!id) return res.status(400).json({ ok: false, ready: false, error: 'Missing assessment ID.' });
...
app.get('/api/assessment/:id/generation-status', async (req, res) => {
  const id = cleanQueueId(req.params.id);
  const submission = await getSubmission(id);
  const job = getAdviceJob(id);

  if (!submission && !job) {
    return res.status(404).json({ ok: false, ready: false, error: 'Assessment submission not found.' });
  }

  const ready = Boolean(submission && hasRealAdviceContent(submission) && submission.pdfStatus === 'ready' && submission.pdfUrl);
  return res.json({
    ok: true,
    ready,
    queued: Boolean(job && ['queued', 'running', 'retrying'].includes(job.status)),
    assessmentId: id,
    status: ready ? 'ready' : (job?.status || submission?.generationStatus || submission?.pdfStatus || submission?.documentStatus || 'processing'),
    attempts: job?.attempts || submission?.queueAttempts || 0,
    maxAttempts: job?.maxAttempts || ADVICE_QUEUE_CONFIG.maxAttempts,
    error: job?.lastError || submission?.pdfError || '',
    pdfUrl: ready ? submission.pdfUrl : '',
    downloadUrl: ready ? (submission.downloadUrl || submission.pdfUrl) : '',
    documentUrl: ready ? (submission.documentUrl || submission.pdfUrl) : '',
    updatedAt: submission?.updatedAt || job?.updatedAt || null,
  });
});

app.get('/api/admin/advice-queue', (req, res) => {
  if (ADMIN_TOKEN && req.headers.authorization !== `Bearer ${ADMIN_TOKEN}` && req.query.token !== ADMIN_TOKEN) {
    return res.status(401).json({ ok: false, error: 'Unauthorised.' });
  }
  return res.json({ ok: true, ...adviceQueueSnapshot() });
});
