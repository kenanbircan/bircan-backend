...
const OPENAI_MODEL_ANALYSIS = String(process.env.OPENAI_MODEL_ANALYSIS || process.env.OPENAI_MODEL || '').trim();
const OPENAI_MODEL_HELPER = String(process.env.OPENAI_MODEL_HELPER || process.env.OPENAI_MODEL || '').trim();
const OPENAI_LEGACY_MODEL = String(process.env.OPENAI_MODEL || '').trim();
...
function buildLegalMetricsInstruction(profile = {}) {
  const items = Array.isArray(profile?.legalMetrics?.items) ? profile.legalMetrics.items : [];
  if (!items.length) return 'Populate legalMetrics with the most relevant visa-specific legal metrics for this matter.';
  return `Populate legalMetrics using these exact metric labels where they genuinely apply: ${items.map(item => item.label).join('; ')}.`;
}

function enforceKnowledgebaseCitations(analysis = {}, kbContext = {}) {
  const allowed = new Set((Array.isArray(kbContext?.selectedChunks) ? kbContext.selectedChunks : []).map(chunk => `[KB: ${chunk.fileName} > ${chunk.section}]`));
  const citationPattern = /\[KB:\s*([^\]]+)\]/g;

  const cleanText = (value) => String(value || '').replace(citationPattern, (full) => allowed.has(full) ? full : '').replace(/\s{2,}/g, ' ').trim();
  const cleanList = (list) => Array.isArray(list) ? list.map(item => cleanText(item)).filter(Boolean) : [];

  const next = { ...analysis };
  [
    'outcomeRationale', 'professionalOpinion', 'overallAssessment', 'executiveSummary', 'legalStyleSummary', 'detailedAssessment',
    ...
    'carerNeedAssessment', 'protectionClaimBasis', 'conventionGroundAssessment', 'complementaryProtectionAssessment', 'identityCredibilityAssessment',
    'countryRiskAssessment', 'genuineStudentAssessment', 'studyPlanAssessment', 'graduateAssessment', 'graduateStreamAssessment',
    'temporaryEntrantAssessment', 'trainingPlanAssessment', 'temporaryActivityAssessment', 'businessBackgroundAssessment', 'statusVisaAssessment',
    'residenceAssessment'
  ].forEach((key) => {
    if (key in next) next[key] = cleanText(next[key]);
  });

  [
    'strengths', 'refusalScenarios', 'strategyAdvice', 'missingInformation', 'recommendedNextSteps', 'documentChecklist', 'factualAssumptions',
    'nominationRisks', 'invitationRisks', 'regionalRisks', 'legacyRisks', 'relationshipRisks', 'parentRisks', 'childRisks', 'relativeRisks',
    'carerRisks', 'protectionEvidenceGaps', 'genuineStudentRisks', 'temporaryEntrantRisks', 'trainingRisks', 'temporaryActivityRisks',
    'businessRisks', 'statusVisaRisks', 'thresholdIssues', 'timingRisks'
  ].forEach((key) => {
    if (key in next) next[key] = cleanList(next[key]);
  });

  if (Array.isArray(next.concerns)) {
    next.concerns = next.concerns.map(item => ({
      issue: cleanText(item?.issue || ''),
      impact: cleanText(item?.impact || ''),
      requiredAction: cleanText(item?.requiredAction || ''),
    })).filter(item => item.issue || item.impact || item.requiredAction);
  }

  next.knowledgebaseCitations = Array.from(new Set(Array.from(allowed).filter(citation => JSON.stringify(next).includes(citation))));
  return next;
}
...
async function selectKnowledgebaseContext(submission = {}) {
  ...
  const scored = (state.chunks || []).map(chunk => ({ ...chunk, score: scoreKnowledgebaseChunk(chunk, keywords, profile, submission) }))
...
function normaliseAnalysisShape(ai = {}, submission = {}) {
  ...
  const merged = { ...fallback, ...ai, ratings: { ...fallback.ratings, ...(ai.ratings || {}) }, legalMetrics: (ai && typeof ai.legalMetrics === 'object' && !Array.isArray(ai.legalMetrics)) ? ai.legalMetrics : {}, knowledgebaseUsed: Array.isArray(ai?.knowledgebaseUsed) ? ai.knowledgebaseUsed : [], knowledgebaseCitations: Array.isArray(ai?.knowledgebaseCitations) ? ai.knowledgebaseCitations : [] };
...
async function runAssessmentAnalysis(submission) {
  ...
  const systemPrompt = [
    ...
    buildLegalMetricsInstruction(profile),
    ...
    'When knowledgebase excerpts are provided, use them as primary internal policy guidance and cite them inline in outcomeRationale, professionalOpinion, detailedAssessment, strategyAdvice, and any visa-specific reasoning sections using the exact format [KB: file > section].',
    'Do not invent knowledgebase citations. Only cite a knowledgebase section if it was included in the supplied excerpts.',
    'Every conclusion must state why. Distinguish curable evidentiary gaps from substantive ineligibility.',
  ...
  const response = await fetch(`${OPENAI_BASE_URL}/chat/completions`, {
    ...
    body: JSON.stringify({
      model: OPENAI_MODEL_ANALYSIS || OPENAI_LEGACY_MODEL || 'gpt-4.1-mini',
      temperature: 0.1,
  ...
  let parsed;
  try {
    parsed = parseJsonObjectStrict(content);
  } catch (e) {
    console.error('[openai-raw-content]', content);
    throw e;
  }

  const analysis = enforceKnowledgebaseCitations(normaliseAnalysisShape(parsed, submission), kbContext);
  ...
  analysis.knowledgebaseCitations = kbContext.citations;
  return {
    status: 'completed',
    model: OPENAI_MODEL_ANALYSIS || OPENAI_LEGACY_MODEL || 'gpt-4.1-mini',
    helperModel: OPENAI_MODEL_HELPER || OPENAI_LEGACY_MODEL || null,
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    addSectionTitle(doc, 'Knowledgebase Sources Referenced');
    addBulletList(
      doc,
      (analysis.knowledgebaseUsed || []).map(item => `${item.file} — ${item.section}`),
      'No knowledgebase sources were referenced.'
    );
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    helperModel: OPENAI_MODEL_HELPER || OPENAI_LEGACY_MODEL || null,
    configuredModels: {
      analysis: OPENAI_MODEL_ANALYSIS || OPENAI_LEGACY_MODEL || null,
      helper: OPENAI_MODEL_HELPER || OPENAI_LEGACY_MODEL || null,
      legacy: OPENAI_LEGACY_MODEL || null,
    },
