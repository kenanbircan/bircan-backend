...
app.get('/api/citizenship/my-entitlement', requireCitizenshipAuth, (req, res) => { const entitlement = findUserEntitlement(req.user.id); if (!entitlement) return res.status(404).json({ ok: false, error: 'No paid exam pack is attached to this account.' }); return res.json({ ok: true, entitlement: publicEntitlement(entitlement) }); });
