...
// -----------------------------------------------------------------------------
// Advice-letter generation finality guard
// -----------------------------------------------------------------------------
// These helpers prevent template/placeholder PDFs from being marked ready.
// A PDF is only "ready" when the GPT analysis contains real substantive advice.
...
async function ensureFullAdviceLetterReady(submissionId, options = {}) {
  const cleanId = normaliseDocumentSubmissionId ? normaliseDocumentSubmissionId(submissionId) : String(submissionId || '').trim();
...
// -----------------------------------------------------------------------------
// Hardened advice-letter PDF generation routes
// -----------------------------------------------------------------------------
// These routes override old behaviour: no template PDF is returned as "ready".
app.post(['/api/assessment/:id/generate-pdf', '/api/assessment/:id/generate', '/api/assessments/:id/generate-pdf'], async (req, res) => {
  const id = normaliseDocumentSubmissionId(req.params.id || req.body?.assessmentId || req.body?.submissionId || req.body?.id);
