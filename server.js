...
app.post(['/create-visa-checkout-session', '/api/assessment/create-checkout-session'], async (req, res, next) => {
  try {
...
app.get(['/checkout/verify-visa-session', '/api/assessment/verify-checkout-session'], async (req, res, next) => {
  try {
    assertStripeConfigured();
    assertStripeModeLooksConsistent();
    const sessionId = String(req.query?.session_id || req.query?.sessionId || '').trim();
