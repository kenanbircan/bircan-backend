...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.2.14-payment-finalise-identity-first-v7',
    postgres: true,
    jsonFallback: false,
...
// ---- v7 permanent fast payment finalisation ----
...
// Rule: finalise-return must return client identity/dashboard token fast. Paid record
// attachment and advice queue repair run in the background and are picked up by polling.
const PAYMENT_FINALISE_FAST_VERSION = 'payment-finalise-fast-identity-first-v7';
...
function buildPaymentFinaliseResponsePayload({ context, sessionId, background, stripeChecked }) {
  ...
  return {
    ...
    redirectUrl,
    message: 'Payment return accepted. Dashboard identity is returned immediately; paid service attachment continues in the background if required.'
  };
}
...
async function finaliseStripePayment(req, res) {
  ...
  // 2) If local context is missing, allow a short Stripe lookup only for identity/metadata.
  let stripeContext = null;
  if (!context.client || !context.clientEmail || !context.assessmentId) {
  ...
  // 3) Always queue the real paid-session attach. Never wait for it in the browser request.
  const background = schedulePaymentFinaliseBackground(sessionId);

  // 4) If we found the client, set cookie/token immediately. This prevents blank dashboard.
  if (context.client) {
