...
async function syncDbServiceDocumentFromSubmission(submission) {
  try {
    ...
    const documentStatus = pdfReady ? 'ready' : (String(submission.pdfStatus || '').toLowerCase() === 'failed' ? 'failed' : (submission.pdfStatus || 'generating'));
    const documentUrl = pdfReady ? (submission.pdfUrl || `/api/assessment/${encodeURIComponent(submission.id)}/pdf`) : (submission.pdfUrl || null);
    const metaPatch = {
      submissionId: submission.id,
      pdfPath: submission.pdfPath || null,
      pdfUrl: documentUrl,
    ...
    const params = [
      ...
      String(submission.stripeSessionId || ''),
...
function publicDashboardAssessment(submission = {}) {
  ...
  return {
    ...
    pdfUrl: submission.pdfUrl || submission.downloadUrl || (submission.pdfFileName ? `/pdfs/${submission.pdfFileName}` : null),
...
app.post('/api/admin/submissions/:submissionId/regenerate-pdf', adminGuard, async (req, res, next) => {
  try {
    ...
    const updated = await updateSubmission(submission.id, {
      ...
      pdfPath: pdfResult?.pdfPath || null,
      pdfUrl: pdfResult?.pdfUrl || null,
      documentStatus: pdfResult?.pdfPath ? 'ready' : 'failed',
      updatedAt: nowIso(),
    });
