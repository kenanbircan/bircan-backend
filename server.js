...
async function ensurePaidVisaAssessmentReadyForDashboard(submissionId, context = {}) {
  let submission = await getSubmission(submissionId);
  if (!submission) throw new Error(`Submission not found for ${submissionId}`);

  const stripeSessionId = context.stripeSessionId || submission.stripeSessionId || null;
  const paymentIntentId = context.stripePaymentIntentId || submission.stripePaymentIntentId || null;
  const accountEmail = String(context.accountEmail || submission.accountEmail || submission.stripeCustomerEmail || submission.customerEmail || submission.clientEmail || submission.email || (submission.client && submission.client.email) || '').trim().toLowerCase();
  const accountUserId = String(context.userId || submission.userId || submission.accountUserId || '').trim() || null;
  const plan = normalizeVisaCheckoutPlan(context.plan || submission.plan || submission.assessmentPlan || submission.delivery || '24h');

  submission = await markSubmissionStage(submissionId, {
    paymentStatus: 'paid',
    paid: true,
    paidAt: submission.paidAt || nowIso(),
    productType: submission.productType || 'visa_assessment',
    stripeSessionId,
    stripePaymentIntentId: paymentIntentId,
    accountEmail: accountEmail || submission.accountEmail || null,
    userId: accountUserId || submission.userId || null,
    plan,
    status: submission.status === 'failed' ? 'paid' : (submission.status || 'paid'),
    processingError: null,
  });

  const pdfExists = Boolean(submission.pdfPath && fs.existsSync(submission.pdfPath));
  if (!pdfExists) {
    const pdfResult = await generateProfessionalPdf(submission);
    submission = await markSubmissionStage(submissionId, {
      pdfStatus: 'generated',
      ...
      status: 'completed',
      ...buildReleasePatch({ ...submission, plan }),
    });
  } else if (submission.pdfStatus !== 'generated' || submission.status !== 'completed') {
    submission = await markSubmissionStage(submissionId, {
      pdfStatus: 'generated',
...
app.get(['/api/stripe/verify-session', '/checkout/verify-visa-session', '/api/assessment/verify-session', '/api/assessment/verify-checkout-session'], async (req, res, next) => {
  try {
    ...
    if (verified) {
      ...
      submission = await createVisaAssessmentFromPaidSession(session, payload, { finalize: false, background: false });
      if (!submission) {
        return res.status(404).json({ ok: false, error: 'Paid Stripe session was verified, but no assessment payload/submission could be attached. Check Stripe metadata payloadToken/submissionId.', paid, verified, sessionId: session.id });
      }
      submission = await ensurePaidVisaAssessmentReadyForDashboard(submission.id, {
        stripeSessionId: session.id,
        stripePaymentIntentId: typeof session.payment_intent === 'string' ? session.payment_intent : (session.payment_intent?.id || null),
        accountEmail: session.customer_details?.email || session.customer_email || metadata.email || '',
...
function publicDashboardAssessment(submission = {}) {
  ...
  return {
    ...
    pdfStatus: hasPdfOnDisk ? 'generated' : (submission.pdfStatus || null),
...
app.get('/api/assessment/:submissionId/pdf', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    if (!readySubmission.pdfPath || !fs.existsSync(readySubmission.pdfPath)) {
      readySubmission = await ensurePaidVisaAssessmentReadyForDashboard(readySubmission.id, {
    ...
    if (!readySubmission.pdfPath || !fs.existsSync(readySubmission.pdfPath)) {
      return res.status(409).json({ ok: false, error: 'PDF could not be generated for this paid assessment. Please check backend logs.' });
    }
