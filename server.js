...
async function findSubmissionByStripeSessionId(sessionId) {
  const cleanSessionId = String(sessionId || '').trim();
  if (!cleanSessionId) return null;
  const all = await getAllSubmissions();
  return all.find(item => String(item.stripeSessionId || '').trim() === cleanSessionId) || null;
}
...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null, options = {}) {
  ...
  const existingBySession = await findSubmissionByStripeSessionId(session.id);
  if (existingBySession && String(existingBySession.paymentStatus || '').toLowerCase() === 'paid') {
    if (options.finalize === false) {
      if (options.background !== false && (!existingBySession.pdfPath || !fs.existsSync(existingBySession.pdfPath))) startPaidVisaSubmissionProcessing(existingBySession.id);
      return existingBySession;
    }
    return finalizePaidVisaSubmissionForDashboard(existingBySession.id);
  }
...
app.get(['/api/stripe/verify-session', '/checkout/verify-visa-session', '/api/assessment/verify-session', '/api/assessment/verify-checkout-session'], async (req, res, next) => {
  try {
    ...
    const usersForSession = getUsersStore();
    const sessionUser = sessionUserId ? usersForSession.users[sessionUserId] : null;
    ...
    return res.json({
      ...
      token: (user || sessionUser) ? makeDashboardReturnToken(user || sessionUser) : null,
      user: (user || sessionUser) ? publicUser(user || sessionUser) : null,
...
app.get('/api/assessment/:submissionId/pdf', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    if (!readySubmission.pdfPath || !fs.existsSync(readySubmission.pdfPath)) {
      try {
        readySubmission = await finalizePaidVisaSubmissionForDashboard(submission.id);
      } catch (generationError) {
        readySubmission = (await getSubmission(submission.id)) || submission;
      }
    }
