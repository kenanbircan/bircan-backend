...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.0.0-subclass190-production-legal-engine',
    postgres: true,
    jsonFallback: false,
...
function buildLegalEnginePdfInputs(assessment) {
  // Universal delegate-simulator engine for supported subclasses.
  // It produces an adviceBundle compatible with pdf.js and prevents GPT from
  // controlling risk, lodgement position, validity or criterion outcomes.
  return buildDelegateSimulatorPdfInputs(assessment);
}
  ...
  return tx(async (client) => {
    ...
    try {
      ...
      if (legalEngineInputs) {
        pdf = await buildAssessmentPdfBuffer(legalEngineInputs.assessmentForPdf, legalEngineInputs.adviceBundle);
      } else {
