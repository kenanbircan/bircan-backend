...
function buildLegalEnginePdfInputs(assessment) {
  if (String(assessment.visa_type || '').replace(/[^0-9]/g, '') !== '190') return null;
  const decision = runSubclass190LegalEngine(assessment);
  const adviceBundle = sanitizeLegalEngineBundle(buildSubclass190LegalAdviceBundle(decision, assessment));
