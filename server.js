...
async function ensurePaidVisaSubmissionProcessing(submission, options = {}) {
  ...
  const pdfReady = Boolean(submission.pdfPath && fs.existsSync(submission.pdfPath));
  if (pdfReady) {
    await syncDbServiceDocumentFromSubmission(submission);
    return { ok: true, ready: true, submissionId: submission.id };
  }

  const startedAt = new Date(submission.processingStartedAt || submission.updatedAt || submission.createdAt || 0).getTime();
  const ageMs = Number.isFinite(startedAt) ? Date.now() - startedAt : Number.POSITIVE_INFINITY;
  const staleAfterMs = Number(process.env.PDF_GENERATION_STALE_MS || 2 * 60 * 1000);
  const statusBlob = [submission.analysisStatus, submission.pdfStatus, submission.status].map(v => String(v || '').toLowerCase()).join(' ');
  const processing = /running|generating|queued|processing/.test(statusBlob);
  const failed = /failed|error/.test(statusBlob);
  const stale = processing && ageMs > staleAfterMs;
  ...
  if (!processing || failed || stale || force) {
    ...
    const queued = await getSubmission(submission.id);
    if (queued) await syncDbServiceDocumentFromSubmission(queued);

    processSubmission(submission.id).catch(async (error) => {
      console.error('[visa-paid-processing-error]', submission.id, error);
      await updateSubmission(submission.id, {
      ...
      const failedSubmission = await getSubmission(submission.id);
      if (failedSubmission) await syncDbServiceDocumentFromSubmission(failedSubmission);
    });
    return { ok: true, queued: true, stale, forced: force, submissionId: submission.id };
  }

  await syncDbServiceDocumentFromSubmission(submission);
  return { ok: true, queued: false, processing: true, submissionId: submission.id };
}
...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null) {
  ...
  if (existingSubmission && existingSubmission.paymentStatus === 'paid') {
    ...
    await ensurePaidVisaSubmissionProcessing(updatedExisting);
...
async function getVisaAssessmentsForDashboard(user) {
  ...
  for (const item of matched) {
    if (item.paymentStatus === 'paid') {
      try { await ensurePaidVisaSubmissionProcessing(item); } catch (error) { console.error('[dashboard-pdf-repair-error]', item.id, error); }
    }
  }
