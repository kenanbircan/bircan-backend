...
const STRIPE_SUCCESS_PATH = String(process.env.STRIPE_SUCCESS_PATH || '/citizenship-test-stripe-wired.html').trim() || '/citizenship-test-stripe-wired.html';
const STRIPE_CANCEL_PATH = String(process.env.STRIPE_CANCEL_PATH || '/citizenship-test-stripe-wired.html').trim() || '/citizenship-test-stripe-wired.html';
...
function pickCitizenshipExamQuestions() {
  const bank = getCitizenshipBank(); const values = bank.filter(q => q.isValue); const general = bank.filter(q => !q.isValue); const shuffleLocal = arr => [...arr].sort(() => Math.random() - 0.5);
  const selected = shuffleLocal([...shuffleLocal(values).slice(0, 5), ...shuffleLocal(general).slice(0, 15)]).slice(0, 20);
  if (selected.length < 20) throw new Error('Citizenship question bank must contain at least 20 usable questions.'); return selected;
}
...
function getEntitlementsStore() { const store = readCitizenshipStore(CITIZENSHIP_ENTITLEMENTS_PATH, { entitlements: {} }); if (!store.entitlements) store.entitlements = {}; return store; }
...
async function upsertCitizenshipEntitlementFromSession(session) {
  ...
  const token = makeCitizenshipAccessToken(session.id); const store = getEntitlementsStore(); const existing = store.entitlements[token] || {}; const total = Number(config.attempts || 0); const used = Number(existing.used || 0);
  store.entitlements[token] = { token, plan: config.code, planLabel: config.label, total, used: Math.min(used, total), remaining: Math.max(0, total - used), sessionId: session.id, customerEmail: session.customer_details?.email || session.customer_email || existing.customerEmail || null, amountTotal: session.amount_total || existing.amountTotal || null, currency: session.currency || existing.currency || null, paymentStatus: session.payment_status || existing.paymentStatus || null, createdAt: existing.createdAt || nowIso(), updatedAt: nowIso(), mode: STRIPE_MODE, results: Array.isArray(existing.results) ? existing.results : [] };
  await saveEntitlementsStore(store); return store.entitlements[token];
}
function publicEntitlement(ent) { if (!ent) return null; return { plan: ent.plan, planLabel: ent.planLabel, total: ent.total, used: ent.used, remaining: Math.max(0, Number(ent.total || 0) - Number(ent.used || 0)), customerEmail: ent.customerEmail || null, createdAt: ent.createdAt, updatedAt: ent.updatedAt, results: Array.isArray(ent.results) ? ent.results.slice(-10) : [] }; }
...
app.post('/create-checkout-session', async (req, res, next) => {
  try {
    ...
    // IMPORTANT: Do not pass {CHECKOUT_SESSION_ID} through URLSearchParams.
    // Stripe must receive the placeholder literally so it can replace it with a real cs_test_/cs_live_ session ID.
    const successBaseUrl = buildStripeReturnUrl(STRIPE_SUCCESS_PATH, {
      plan: config.code,
      paid: '1'
    });
    const successUrl = `${successBaseUrl}${successBaseUrl.includes('?') ? '&' : '?'}session_id={CHECKOUT_SESSION_ID}`;
    const cancelUrl = buildStripeReturnUrl(STRIPE_CANCEL_PATH, {
      canceled: '1'
    });
...
app.get('/checkout/verify-session', async (req, res, next) => {
  try {
    ...
    return res.json({ ok: verified, paid, verified, plan: metadataPlan || requestedPlan || null, accessToken: entitlement ? entitlement.token : null, entitlement: publicEntitlement(entitlement), sessionId: session.id, customerEmail: session.customer_details?.email || session.customer_email || null, amountTotal: session.amount_total || null, currency: session.currency || null, paymentStatus: session.payment_status || null, status: session.status || null });
  } catch (error) {
    return next(error);
  }
});
