...
const corsOptions = {
  origin(origin, cb) {
    // Allow server-to-server, same-origin, and local file testing where Origin may be null.
    if (!origin || origin === 'null') return cb(null, true);

    // Allow exact configured production domains.
    if (allowedOrigins.includes(origin)) return cb(null, true);

    // Allow common local development / preview origins.
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      corsPatch: 'matched-frontend-backend-v3',
      allowedOrigins
    });
  } catch (err) {
