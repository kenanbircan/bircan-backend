...
const app = express();
// All routes must be registered after this line. Do not place app.get/app.post above const app = express().
...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.2.1-appeals-checkout-aliases-no-citizenship-fallback',
    postgres: true,
    jsonFallback: false,
...
const handleAppealsCheckoutSession = asyncRoute(async (req, res) => {
  if (!stripe) return res.status(500).json({ ok: false, error: 'Stripe is not configured.' });

  let assessmentId = String(
    req.body.assessmentId ||
    req.body.assessment_id ||
    req.body.appealAssessmentId ||
    req.body.appeal_assessment_id ||
    req.body.serviceRef ||
    req.body.service_ref ||
    ''
  ).trim();

  const serviceSessionId = String(
    req.body.serviceSessionId ||
    req.body.service_session_id ||
    req.query.serviceSessionId ||
    req.query.service_session_id ||
    ''
  ).trim();

  let linkedServiceSession = null;
  ...
  if (serviceSessionId) {
    ...
    linkedServiceSession = serviceRows[0] || null;
    if (linkedServiceSession && !assessmentId) assessmentId = linkedServiceSession.service_ref;
  }

  if (!linkedServiceSession && assessmentId) {
    const serviceRows = (await query(
      `SELECT * FROM service_sessions WHERE service_type='appeals_assessment' AND service_ref=$1 ORDER BY created_at DESC LIMIT 1`,
      [assessmentId]
    )).rows;
  ...
  // Last-resort recovery for older frontend payloads: use the latest unpaid appeal
  // service session for the logged-in portal account. This prevents the frontend
  // from falling back into citizenship checkout just because an alias route was missing.
  if (!assessmentId) {
    const recentRows = (await query(
      `SELECT * FROM service_sessions
       WHERE service_type='appeals_assessment'
         AND payment_status <> 'paid'
         AND (lower(client_email)=lower($1) OR client_id=$2)
       ORDER BY created_at DESC
       LIMIT 1`,
      [req.client.email, req.client.id]
    )).rows;
    linkedServiceSession = recentRows[0] || linkedServiceSession;
    if (linkedServiceSession) assessmentId = linkedServiceSession.service_ref;
  }

  if (!assessmentId) {
    return res.status(400).json({
      ok: false,
      error: 'Missing appeals assessment ID or serviceSessionId. Start the appeals assessment first, then continue to payment.'
    });
  }
  ...
  const requestedPlan = req.body.plan || req.body.selectedPlan || req.body.selected_plan || (linkedServiceSession && linkedServiceSession.selected_plan);
  ...
  if (!linkedServiceSession) {
    linkedServiceSession = await upsertServiceSession({
      serviceType: 'appeals_assessment',
      ...
      metadata: {
        visa_subclass: assessment.visa_subclass || null,
        created_by_checkout_alias: true,
        portal_login_confirmed_at: new Date().toISOString(),
        portal_login_email: req.client.email
      }
    });
  } else {
    await query(
      `UPDATE service_sessions
       SET client_id=$1,
           ...
           updated_at=now()
       WHERE id=$5`,
      [req.client.id, req.client.email, plan, JSON.stringify({
        portal_login_confirmed_at: new Date().toISOString(),
        portal_login_email: req.client.email,
        checkout_alias_confirmed: true
      }), linkedServiceSession.id]
    );
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{ price, quantity: 1 }],
    success_url: `${APP_BASE_URL}/payment-complete.html?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${APP_BASE_URL}/appeals-assessment.html?cancelled=1&service_session_id=${encodeURIComponent(linkedServiceSession.id)}&appeal_assessment_id=${encodeURIComponent(assessmentId)}`,
    metadata: {
      service_type: 'appeals_assessment',
      service_session_id: linkedServiceSession.id,
  ...
  }, { idempotencyKey: `appeals-checkout-${linkedServiceSession.id}-${assessmentId}-${plan}` });

  await query(
    `UPDATE appeals_assessments
     SET stripe_session_id=$1, status='checkout_created', amount_cents=$2, currency=$3, updated_at=now()
     WHERE id=$4`,
    [session.id, session.amount_total || appealAmountCents(plan), session.currency || 'aud', assessmentId]
  );

  await markServiceSessionCheckoutCreated(linkedServiceSession.id, session.id);
  ...
  return res.json({
    ...
    serviceSessionId: linkedServiceSession.id,
    service_session_id: linkedServiceSession.id,
    assessmentId,
    assessment_id: assessmentId,
...
// Backward-compatible aliases used by older login/payment flow scripts.
// These aliases are deliberately appeals-only so they can never create the Citizenship Test product.
app.post('/api/appeals/create-checkout-session', requireAuth, handleAppealsCheckoutSession);
app.post('/api/create-appeals-checkout-session', requireAuth, handleAppealsCheckoutSession);
app.post('/create-appeals-checkout-session', requireAuth, handleAppealsCheckoutSession);
app.post('/api/public/appeals/create-checkout-session', requireAuth, handleAppealsCheckoutSession);
app.post('/api/appeals/checkout', requireAuth, handleAppealsCheckoutSession);
...
async function handleVisaCheckoutSession(req, res) {
  if (!stripe) return res.status(500).json({ ok: false, error: 'Stripe is not configured.' });
...
app.post('/api/assessment/create-checkout-session', requireAuth, asyncRoute(handleVisaCheckoutSession));
app.post('/api/public/visa-assessment/checkout', requireAuth, asyncRoute(handleVisaCheckoutSession));
app.post('/api/visa-assessment/checkout', requireAuth, asyncRoute(handleVisaCheckoutSession));
app.post('/api/public/visa-assessment/create-checkout-session', requireAuth, asyncRoute(handleVisaCheckoutSession));
