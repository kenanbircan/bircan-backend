...
function drawHeader(doc, options = {}) {
  ...
  if (compact) {
    if (hasLogo) {
      doc.image(logoPath, 40, 28, { fit: [84, 40], align: 'left', valign: 'center' });
    }
    const titleX = hasLogo ? 136 : 40;
    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(11).text('Bircan Migration & Education', titleX, 34, {
      width: 260,
      lineBreak: false,
    });
    doc.fillColor('#5f6b7d').font('Helvetica').fontSize(10).text('Preliminary assessment letter', titleX, 49, {
      width: 220,
      lineBreak: false,
    });
  ...
  doc.roundedRect(40, 32, 515, 88, 14).fill('#0b1f3a');

  if (hasLogo) {
    doc.image(logoPath, 54, 46, { fit: [120, 60], align: 'left', valign: 'center' });
  }

  const textX = hasLogo ? 190 : 58;
  const textWidth = hasLogo ? 280 : 390;

  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(20).text('Bircan Migration & Education', textX, 50, {
    width: textWidth,
    lineBreak: false,
  });
  doc.fillColor('#d8e6ff').font('Helvetica').fontSize(10).text('Migration advice and professional visa assessment services', textX, 77, {
    width: 300,
  });

  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(11).text('Kenan Bircan JP', 400, 48, {
    width: 130,
    align: 'right',
    lineBreak: false,
  });
  doc.fillColor('#d8e6ff').font('Helvetica').fontSize(8.8).text(
    'Registered Migration Agent
MARN: 1463685 | MMIA: 10497
CMA: 20156912 | JP No: 218851',
    380,
    64,
...
function drawFooterOnCurrentPage(doc, pageNumber) {
  ...
  doc.text('Bircan Migration & Education | Preliminary assessment letter', 40, footerY, { width: 350, lineBreak: false });
  doc.text(`Page ${pageNumber}`, 470, footerY, { width: 85, align: 'right', lineBreak: false });
  doc.restore();
  doc.fillColor('#172033');
}

function ensureSpace(doc, threshold = 710) {
  if (doc.y > threshold) {
    drawFooterOnCurrentPage(doc, doc._pageNumber || 1);
    doc.addPage();
    doc._pageNumber = (doc._pageNumber || 1) + 1;
...
function addSectionTitle(doc, title) {
  ...
  doc.moveTo(40, doc.y + 2).lineTo(555, doc.y + 2).strokeColor('#2f6fed').stroke();
  doc.moveDown(0.5);
}
...
function addMetaGrid(doc, rows) {
  rows.forEach((row, index) => {
    ensureSpace(doc, 730);
    const y = doc.y;
    doc.roundedRect(40, y - 2, 515, 24, 4).fill(index % 2 === 0 ? '#f6f9fc' : '#ffffff');
    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(10).text(row[0], 50, y + 5, { width: 145, lineBreak: false });
    doc.fillColor('#172033').font('Helvetica').fontSize(10).text(row[1], 200, y + 5, { width: 340 });
    doc.y = y + 26;
  });
  doc.moveDown(0.4);
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica-Bold').fontSize(14).fillColor('#0b1f3a').text(analysis.caseCaption || 'Preliminary Migration Assessment');
    doc.moveDown(0.5);

    addParagraph(doc, `Date: ${todayHuman()}`);
    addParagraph(doc, `Client: ${submission.client?.fullName || 'Client'}`);
    addParagraph(doc, `Matter: ${submission.visaType || 'Subclass 482'} preliminary advice`);
    ...
    addSectionTitle(doc, 'Factual Assumptions Used for this Preliminary View');
    addBulletList(doc, analysis.factualAssumptions, 'No specific factual assumptions were listed.');
    ...
    addSectionTitle(doc, 'Questionnaire Record');
