...
async function processPaidSubmission(submissionId) {
  ...
  if (existing.analysisStatus === "completed" && existing.pdfStatus === "generated" && existing.emailStatus === "sent") {
    return existing;
  }
...
app.post("/api/assessment/submit", (req, res) => {
  try {
    const { fullName, email, phone, visaSubclass, selectedPlan, deliveryLabel, amount, answers, metadata } = req.body || {};
...
app.post("/api/payments/create-checkout-session", async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      customer_email: submission.client.email,
      ...
      line_items: [
        {
          ...
          price_data: {
            ...
            product_data: {
              name: `Bircan Migration Visa Assessment - ${submission.visa.subclass}`,
      ...
      success_url: `${process.env.FRONTEND_URL}/success.html?submissionId=${submission.id}&session_id={CHECKOUT_SESSION_ID}`,
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || "Stripe session could not be created."
    });
  }
});
