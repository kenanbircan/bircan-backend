...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '10.0.3-payment-reconcile-before-pdf',
    postgres: true,
    jsonFallback: false,
    stripeConfigured: Boolean(stripe),
    smtpConfigured: Boolean(process.env.SMTP_HOST),
    appBaseUrl: APP_BASE_URL,
    corsPatch: 'matched-frontend-backend-final-x-auth-token-final-pdf-route',
...
async function reconcileAssessmentPaymentFromStripe(assessmentId, accountEmail = null) {
  if (!stripe || !assessmentId) return { reconciled: false, reason: 'stripe_not_configured_or_missing_id' };

  const { rows } = await query(
    `SELECT id, client_email, payment_status, stripe_session_id
     FROM assessments
     WHERE id=$1 ${accountEmail ? 'AND lower(client_email)=lower($2)' : ''}
     LIMIT 1`,
    accountEmail ? [assessmentId, accountEmail] : [assessmentId]
  );
  const assessment = rows[0];
  if (!assessment) return { reconciled: false, reason: 'assessment_not_found' };
  if (assessment.payment_status === 'paid') return { reconciled: true, alreadyPaid: true };
  if (!assessment.stripe_session_id) return { reconciled: false, reason: 'no_stripe_session_id' };

  const session = await stripe.checkout.sessions.retrieve(assessment.stripe_session_id);
  const paid = session.payment_status === 'paid' || session.status === 'complete';
  if (!paid) return { reconciled: false, reason: `stripe_not_paid:${session.payment_status || session.status || 'unknown'}` };

  await attachPaidSession(session, { triggerGeneration: false, waitForPdf: false });
  return { reconciled: true, sessionId: session.id };
}
...
async function generateAssessmentPdfNow(assessmentId, accountEmail = null) {
  ...
  // Reconcile a genuinely paid Stripe session before refusing PDF generation.
  // This fixes cases where the browser reaches the dashboard before webhook/finalise
  // updates payment_status. It never generates a PDF for an unpaid Stripe session.
  let preRows = (await query(
    `SELECT id, payment_status, stripe_session_id FROM assessments WHERE id=$1 ${accountEmail ? 'AND lower(client_email)=lower($2)' : ''} LIMIT 1`,
    accountEmail ? [requestedId, accountEmail] : [requestedId]
  )).rows;
  if (!preRows[0] && /^sub_\d+_[a-z0-9]+$/i.test(requestedId)) {
    const likePattern = requestedId.replace(/([%_\\])/g, '\\$1') + '\_%';
    preRows = (await query(
      `SELECT id, payment_status, stripe_session_id FROM assessments WHERE id LIKE $1 ESCAPE '\\' ${accountEmail ? 'AND lower(client_email)=lower($2)' : ''} ORDER BY created_at DESC LIMIT 1`,
      accountEmail ? [likePattern, accountEmail] : [likePattern]
    )).rows;
  }
  if (preRows[0] && preRows[0].payment_status !== 'paid' && preRows[0].stripe_session_id) {
    await reconcileAssessmentPaymentFromStripe(preRows[0].id, accountEmail);
  }
  ...
  return tx(async (client) => {
    ...
    if (assessment.payment_status !== 'paid') throw new Error('Payment is not verified for this assessment. Complete Stripe payment or finalise the Stripe session before generating the advice letter.');
