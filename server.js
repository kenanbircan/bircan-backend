...
const STRIPE_SUCCESS_PATH = String(process.env.STRIPE_SUCCESS_PATH || '/paid-citizenship-exam.html').trim() || '/paid-citizenship-exam.html';
const STRIPE_CANCEL_PATH = String(process.env.STRIPE_CANCEL_PATH || '/citizenship-test.html').trim() || '/citizenship-test.html';
...
const ASSESSMENT_PAYMENTS_PATH = path.join(STORAGE_DIR, 'assessment-payments.json');
...
function getBearerToken(req) { const header = String(req.headers.authorization || ''); return header.startsWith('Bearer ') ? header.slice(7).trim() : ''; }
...
function getAssessmentPaymentsStore() {
  const store = readJsonSafe(ASSESSMENT_PAYMENTS_PATH, { bySessionId: {}, bySubmissionId: {} }) || { bySessionId: {}, bySubmissionId: {} };
  if (!store.bySessionId) store.bySessionId = {};
  if (!store.bySubmissionId) store.bySubmissionId = {};
  return store;
}
async function saveAssessmentPaymentsStore(store) { await writeJson(ASSESSMENT_PAYMENTS_PATH, store); }
function publicAssessmentSubmission(item = {}) {
  const release = getReleaseSnapshot(item);
  return {
    id: item.id,
    submissionId: item.id,
    visaType: item.visaType || item.visa || null,
    plan: item.plan || null,
    clientName: item.client?.name || item.clientName || null,
    clientEmail: item.client?.email || item.clientEmail || null,
    createdAt: item.createdAt || null,
    updatedAt: item.updatedAt || null,
    paidAt: item.paidAt || null,
    paymentStatus: item.paymentStatus || 'pending',
    status: item.status || 'created',
    analysisStatus: item.analysisStatus || 'not_started',
    pdfStatus: item.pdfStatus || 'not_generated',
    emailStatus: item.emailStatus || 'not_sent',
    releaseAt: release.releaseAt,
    releaseStatus: release.releaseStatus,
    pdfUrl: item.pdfUrl || null,
  };
}
async function startPaidAssessmentProcessing(submissionId) {
  const submission = await getSubmission(submissionId);
  if (!submission) return null;
  if (submission.paymentStatus !== 'paid') return submission;
  if (['processing', 'completed'].includes(String(submission.status || '').toLowerCase()) || submission.processingStartedAt) return submission;
  await updateSubmission(submissionId, {
    status: 'queued',
    analysisStatus: submission.analysisStatus && submission.analysisStatus !== 'not_started' ? submission.analysisStatus : 'queued',
    pdfStatus: submission.pdfStatus && submission.pdfStatus !== 'not_generated' ? submission.pdfStatus : 'queued',
...
async function markAssessmentSessionPaid(session) {
  const metadata = session.metadata || {};
  if (String(metadata.product || '') !== 'visa_assessment') return null;
  const paid = session.payment_status === 'paid' || session.status === 'complete';
  if (!paid) return null;
  const submissionId = String(metadata.submissionId || '').trim();
  if (!submissionId) return null;
  const submission = await getSubmission(submissionId);
  if (!submission) return null;
  const releaseAt = submission.releaseAt || calculateReleaseAt(submission.plan, nowIso());
  const patch = {
    paymentStatus: 'paid',
    paidAt: submission.paidAt || nowIso(),
    stripeSessionId: session.id,
    stripePaymentIntentId: typeof session.payment_intent === 'string' ? session.payment_intent : (session.payment_intent?.id || null),
    stripeAmountTotal: session.amount_total || null,
    stripeCurrency: session.currency || 'aud',
    userId: metadata.userId || submission.userId || null,
    accountEmail: metadata.email || session.customer_details?.email || session.customer_email || submission.accountEmail || null,
    releaseAt,
    emailReleaseAt: releaseAt,
    scheduledEmailAt: releaseAt,
    releaseStatus: normalizePlan(submission.plan).code === 'instant' ? 'ready' : 'scheduled',
    releaseCountdownMs: Math.max(0, new Date(releaseAt).getTime() - Date.now()),
  };
  await updateSubmission(submissionId, patch);
  const store = getAssessmentPaymentsStore();
  store.bySessionId[session.id] = { sessionId: session.id, submissionId, userId: patch.userId, email: patch.accountEmail, paidAt: patch.paidAt, amountTotal: patch.stripeAmountTotal, currency: patch.stripeCurrency, mode: STRIPE_MODE };
  store.bySubmissionId[submissionId] = session.id;
  await saveAssessmentPaymentsStore(store);
  return startPaidAssessmentProcessing(submissionId);
}
...
app.post('/create-checkout-session', async (req, res, next) => {
  try {
    ...
    const successUrl = `${successBaseUrl}${successBaseUrl.includes('?') ? '&' : '?'}session_id={CHECKOUT_SESSION_ID}`;
...
app.get('/api/citizenship/debug-session', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    return res.json({
      ...
      entitlement: publicEntitlement(findUserEntitlement(req.user.id)),
    });
  } catch (error) { return next(error); }
});
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (String(session.metadata?.product || '') === 'visa_assessment') {
        await markAssessmentSessionPaid(session);
      } else {
...
app.post('/api/assessment/submit', async (req, res, next) => {
  try {
    ...
    const submission = {
      ...
      emailReleaseAt: releaseAt,
      scheduledEmailAt: releaseAt,
      releaseStatus: 'payment_required',
      releaseCountdownMs: Math.max(0, new Date(releaseAt).getTime() - Date.now()),
      emailReleased: false,
    ...
    res.status(201).json({
      ...
      nextStep: 'login_then_payment',
      loginUrl: `/assessment-login.html?submissionId=${encodeURIComponent(submissionId)}`,
      message: 'Assessment saved. The client must login or create an account before Stripe payment is started.',
    });
  } catch (error) {
    next(error);
  }
...
app.post('/api/assessment/create-checkout-session', requireCitizenshipAuth, async (req, res, next) => {
  try {
    assertStripeConfigured();
    assertStripeModeLooksConsistent();
    const submissionId = String(req.body?.submissionId || '').trim();
    if (!submissionId) return res.status(400).json({ ok: false, error: 'Submission ID is required.' });
    const submission = await getSubmission(submissionId);
    if (!submission) return res.status(404).json({ ok: false, error: 'Assessment submission was not found.' });
    if (submission.paymentStatus === 'paid') return res.status(409).json({ ok: false, error: 'This assessment has already been paid.' });
    // Attach the submitted assessment to the authenticated account that continues to checkout.
    // The form email may differ from the account email where a client creates an account after submission.
    const originalClientEmail = String(submission.client?.email || submission.clientEmail || "").trim().toLowerCase();
    const plan = normalizePlan(submission.plan);
    const visaType = normaliseVisaType(submission.visaType || submission.visa || 'Visa assessment');
    const successUrl = buildStripeReturnUrl('/account-dashboard.html', { assessment_paid: '1', submissionId, session_id: '{CHECKOUT_SESSION_ID}' });
    const cancelUrl = buildStripeReturnUrl('/assessment-login.html', { submissionId, payment_cancelled: '1' });
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        ...
        price_data: {
          ...
          product_data: { name: `${visaType} - ${plan.label} assessment`, description: 'Professional visa assessment payment' }
        }
      }],
      success_url: successUrl,
      cancel_url: cancelUrl,
      metadata: { product: 'visa_assessment', submissionId, visaType, plan: plan.code, planLabel: plan.label, userId: req.user.id, email: req.user.email },
      payment_intent_data: { metadata: { product: 'visa_assessment', submissionId, visaType, plan: plan.code, planLabel: plan.label, userId: req.user.id, email: req.user.email } }
    });
    await updateSubmission(submissionId, { userId: req.user.id, accountEmail: req.user.email, originalClientEmail: originalClientEmail || submission.originalClientEmail || null, stripeSessionId: session.id, paymentStatus: 'checkout_created', status: 'awaiting_payment' });
    return res.json({ ok: true, url: session.url, sessionId: session.id, submissionId });
  } catch (error) { next(error); }
});

app.get('/api/assessment/verify-session', requireCitizenshipAuth, async (req, res, next) => {
  try {
    assertStripeConfigured();
    assertStripeModeLooksConsistent();
    const sessionId = String(req.query?.session_id || req.query?.sessionId || '').trim();
    if (!sessionId) return res.status(400).json({ ok: false, error: 'Missing Stripe session ID.' });
    const session = await stripe.checkout.sessions.retrieve(sessionId, { expand: ['line_items.data.price', 'payment_intent'] });
    const metadata = session.metadata || {};
    if (String(metadata.product || '') !== 'visa_assessment') return res.status(400).json({ ok: false, error: 'This Stripe session is not a visa assessment payment.' });
    const sameUser = String(metadata.userId || '') === req.user.id || String(metadata.email || '').toLowerCase() === String(req.user.email || '').toLowerCase();
    if (!sameUser) return res.status(403).json({ ok: false, error: 'This Stripe payment belongs to a different account.' });
    const paid = session.payment_status === 'paid' || session.status === 'complete';
    if (!paid) return res.status(402).json({ ok: false, error: 'Stripe payment has not been completed.' });
    const submission = await markAssessmentSessionPaid(session);
    return res.json({ ok: true, verified: true, paid: true, sessionId: session.id, assessment: publicAssessmentSubmission(submission), user: publicUser(req.user) });
  } catch (error) { next(error); }
});
...
app.get('/api/account/dashboard', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    const assessments = all
      .filter(item => String(item.userId || '') === req.user.id || String(item.accountEmail || item.client?.email || '').toLowerCase() === email)
      .map(publicAssessmentSubmission);
    const entitlement = publicEntitlement(findUserEntitlement(req.user.id));
    const citizenshipHistory = typeof getUserCitizenshipExamHistory === 'function' ? getUserCitizenshipExamHistory(req.user.id).slice(0, 20) : [];
    return res.json({ ok: true, user: publicUser(req.user), assessments, citizenship: { entitlement, history: citizenshipHistory } });
  } catch (error) { next(error); }
});
