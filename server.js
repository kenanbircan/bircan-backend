...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '10.2.0-final-issued-pdf-route-no-legacy-template',
    postgres: true,
    jsonFallback: false,
    ...
    pdfMode: 'final-issued-advice-letter-only-no-legacy-template',
    allowedOrigins
  });
}));
...
async function handleAssessmentSubmit(req, res) {
  ...
  await query(
    `INSERT INTO assessments (id, client_id, client_email, applicant_email, applicant_name, visa_type, selected_plan, active_plan, status, payment_status, form_payload, pdf_bytes, pdf_generated_at, generation_error)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$7,'payment_pending','unpaid',$8,NULL,NULL,NULL)`,
    [id, req.client.id, req.client.email, applicantEmail, applicantName || null, visaType, plan, built]
  );
  res.json({ ok: true, assessmentId: id, status: 'payment_pending', plan, payloadSaved: true, answerCount: payloadAnswerCount(built) });
}
...
app.post('/api/assessment/retry-generation', requireAuth, asyncRoute(async (req, res) => {
  ...
  await query(`UPDATE assessments SET status='preparing', pdf_bytes=NULL, pdf_mime=NULL, pdf_filename=NULL, pdf_sha256=NULL, pdf_generated_at=NULL, generation_error=NULL, updated_at=now() WHERE id=$1`, [assessmentId]);
...
async function generateAssessmentPdfNow(assessmentId, accountEmail = null, options = {}) {
  ...
  return tx(async (client) => {
    ...
    if (!payloadLooksUsable(assessment.form_payload)) {
      ...
      await client.query(`UPDATE assessments SET status='failed', generation_error=$1, updated_at=now() WHERE id=$2`, [msg, assessmentId]);
...
app.get('/api/assessment/:id/payload-status', requireAuth, asyncRoute(async (req, res) => {
  const { rows } = await query(
    `SELECT id, visa_type, status, payment_status, form_payload, pdf_generated_at, generation_error, CASE WHEN pdf_bytes IS NULL THEN false ELSE true END AS has_pdf
