...
function addCriteriaBreakdown(doc, items = []) {
  ...
  list.forEach(item => {
    ensureSpace(doc, 720);
    doc.roundedRect(40, doc.y, 515, 48, 6).strokeColor('#d9e2ef').lineWidth(1).stroke();
    doc.font('Helvetica-Bold').fontSize(10.5).fillColor('#0f2747')
      .text(`${item.criterion} (${item.riskLevel} risk)`, 50, doc.y - 42 + 10, { width: 495 });
    doc.font('Helvetica').fontSize(10.2).fillColor('#172033')
      .text(item.assessment, 50, doc.y - 42 + 25, { width: 495, lineGap: 2 });
    doc.moveDown(2.9);
  });
}
