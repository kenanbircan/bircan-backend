...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'visa_assessment') {
    ...
    }, { idempotencyKey: `service-visa-checkout-${serviceSession.id}-${checkoutPlan}` });
    await query(
      `UPDATE assessments
...
async function attachPaidAppealsSession(session) {
  ...
  const assessmentEmail = normaliseEmail(assessment.client_email);
  if (assessmentEmail && assessmentEmail !== email) {
    console.warn('Appeals Stripe email differs from stored appeal client_email; attaching paid appeal to Stripe customer email for dashboard visibility.', { assessmentId, stored: assessmentEmail, paidEmail: email });
  }
  ...
  await query(
    `UPDATE appeals_assessments
     SET status=$1, payment_status='paid', stripe_session_id=$2, stripe_payment_intent=$3,
         ...
         client_email=$8, release_at=${appealReleaseAtSql(plan)}, updated_at=now()
     WHERE id=$7`,
    [plan === 'instant' ? 'advice_preparing' : 'release_scheduled', session.id, session.payment_intent || null, session.amount_total || appealAmountCents(plan), session.currency || 'aud', plan, assessmentId, email]
  );
  await query(
    `UPDATE service_sessions
     SET status='paid', payment_status='paid', stripe_session_id=$1, client_email=$2, selected_plan=$3, updated_at=now()
     WHERE service_type='appeals_assessment' AND (service_ref=$4 OR id=$5)`,
    [session.id, email, plan, assessmentId, md.service_session_id || null]
  ).catch(err => console.warn('Appeals service session paid sync skipped:', err.message));
...
app.get('/api/account/dashboard', requireAuth, asyncRoute(async (req, res) => {
  ...
  const { rows: appealAssessments } = await query(
    `SELECT id, 'appeals_assessment' AS service_type, visa_subclass, decision_type, applicant_email, applicant_name, selected_plan, active_plan,
     ...
     WHERE lower(client_email)=lower($1)
        OR id IN (SELECT service_ref FROM payments WHERE service_type='appeals_assessment' AND lower(client_email)=lower($1))
        OR id IN (SELECT service_ref FROM service_sessions WHERE service_type='appeals_assessment' AND lower(client_email)=lower($1))
     ORDER BY created_at DESC`,
    [req.client.email]
  );
