...
async function getReusableOpenCheckoutSession(_stripeSessionId, _expected = {}) {
  // Zero-fallback mode: never reuse stale open Stripe checkout sessions.
  return null;
}
...
// Zero-fallback commercial payment lock helpers.
function normaliseCommercialServicePlanStrict(plan) {
  const rawInput = String(plan || '').trim();
  const p = rawInput.toLowerCase().replace(/\s+/g, '').replace(/[-_]+/g, '').replace(/\$/g, '').replace(/aud/g, '');
  if (['instant','fastest','sameday','priority','300','30000'].includes(p)) return 'instant';
  if (['24h','24hr','24hour','24hours','24','recommended','250','25000'].includes(p)) return '24h';
  if (['3d','3day','3days','3','72h','72hr','72hour','72hours','value','200','20000'].includes(p)) return '3d';
  throw Object.assign(new Error(`Invalid or missing assessment payment plan: ${rawInput || '(empty)'}. Allowed plans are instant, 24h, or 3d.`), { statusCode: 400 });
}
function explicitCommercialPlanFromRequest(req) {
  ...
  for (const value of candidates) {
    if (value !== undefined && value !== null && String(value).trim() !== '') return normaliseCommercialServicePlanStrict(value);
  }
  throw Object.assign(new Error('Payment plan is required. Start checkout again from the selected plan button.'), { statusCode: 400 });
}
...
function strictServicePlanFromRequest(req, _fallback = '') {
  return explicitCommercialPlanFromRequest(req);
}
...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'visa_assessment') {
    ...
    const checkoutPlan = strictServicePlanFromRequest(req);
  ...
  if (serviceSession.service_type === 'appeals_assessment') {
    ...
    const plan = strictServicePlanFromRequest(req);
...
app.get('/api/account/dashboard', requireAuth, asyncRoute(async (req, res) => {
  const { rows: assessments } = await query(
    `SELECT a.id, 'visa_assessment' AS service_type, a.visa_type, a.applicant_email, a.applicant_name,
            COALESCE(p.plan, a.active_plan, a.selected_plan) AS selected_plan,
            COALESCE(p.plan, a.active_plan, a.selected_plan) AS active_plan,
            CASE WHEN a.pdf_bytes IS NOT NULL AND octet_length(pdf_bytes) > 1024 THEN 'pdf_ready' ELSE a.status END AS status,
            CASE WHEN a.payment_status='paid' OR p.status='paid' THEN 'paid' ELSE COALESCE(a.payment_status,'unpaid') END AS payment_status,
            COALESCE(p.amount_cents, a.amount_cents) AS amount_cents,
            COALESCE(p.currency, a.currency, 'aud') AS currency,
            COALESCE(p.stripe_session_id, a.stripe_session_id) AS stripe_session_id, a.created_at, a.updated_at,
            COALESCE(a.release_at, a.created_at) AS release_at,
            a.pdf_generated_at, a.pdf_filename, a.generation_error,
            CASE WHEN a.pdf_bytes IS NOT NULL AND octet_length(pdf_bytes) > 1024 THEN true ELSE false END AS has_pdf,
            CASE WHEN (a.payment_status='paid' OR p.status='paid') AND now() >= COALESCE(a.release_at, now()) THEN true ELSE false END AS release_ready,
            GREATEST(0, EXTRACT(EPOCH FROM (COALESCE(a.release_at, now()) - now())))::integer AS release_seconds_remaining
     FROM assessments a
     LEFT JOIN LATERAL (
       SELECT * FROM payments pay
       WHERE pay.service_type='visa_assessment' AND (pay.service_ref=a.id OR pay.stripe_session_id=a.stripe_session_id)
       ORDER BY COALESCE(pay.paid_at, pay.stripe_created_at, pay.created_at) DESC NULLS LAST
       LIMIT 1
     ) p ON true
     WHERE lower(a.client_email)=lower($1) OR lower(COALESCE(a.applicant_email,''))=lower($1) OR a.client_id=$2
     ORDER BY a.created_at DESC`,
  ...
  const { rows: appealAssessments } = await query(
    `WITH linked_appeals AS (
     ...
     SELECT id, 'appeals_assessment' AS service_type, visa_subclass, decision_type, applicant_email, applicant_name,
            COALESCE(payment_record_plan, active_plan, selected_plan, session_selected_plan) AS selected_plan,
            COALESCE(payment_record_plan, active_plan, selected_plan, session_selected_plan) AS active_plan,
            CASE
...
app.get('/api/account/visa/all', requireAuth, asyncRoute(async (req, res) => {
  const { rows } = await query(
    `SELECT a.id, 'visa_assessment' AS service_type, a.visa_type, a.applicant_email, a.applicant_name,
     ...
     WHERE lower(a.client_email)=lower($1) OR lower(COALESCE(a.applicant_email,''))=lower($1) OR a.client_id=$2
     ORDER BY COALESCE(a.created_at, a.updated_at) DESC`,
    [req.client.email, req.client.id]
  );
...
app.get('/api/account/appeals/all', requireAuth, asyncRoute(async (req, res) => {
  const { rows } = await query(
    `WITH owned AS (
     ...
     SELECT id, 'appeals_assessment' AS service_type, visa_subclass, decision_type, applicant_email, applicant_name,
            COALESCE(payment_record_plan, active_plan, selected_plan, session_selected_plan, 'instant') AS selected_plan,
            COALESCE(payment_record_plan, active_plan, selected_plan, session_selected_plan, 'instant') AS active_plan,
            CASE
