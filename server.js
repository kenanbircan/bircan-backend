...
function rebuildUsersStoreIndexes(store) {
  ...
  Object.entries(store.users).forEach(([id, user]) => {
    ...
    const emailKey = normaliseLoginKey(user.email || user.emailAddress || user.userEmail);
    const usernameKey = normaliseUsernameKey(user.username || user.userName || user.name);

    if (emailKey) {
      user.email = emailKey;
      store.byEmail[emailKey] = user.id;
    }
    if (usernameKey) {
      user.username = usernameKey;
      store.byUsername[usernameKey] = user.id;
    }
  });
...
// Allows the paid exam page to restore a logged-in account after Stripe redirects
// back with ?session_id=cs_... even if the browser auth token was lost.
// This verifies the Checkout Session with Stripe, confirms it is paid, binds the
// entitlement server-side, then issues a fresh citizenship JWT for the matched user.
// -----------------------------------------------------------------------------
