...
// Worker timeout fix v5: do not run long PDF generation inside the HTTP request.
// Render/proxy timeouts can kill the request while leaving the assessment at 70%.
// This in-memory guard prevents duplicate background runs per assessment in one process.
const BM_BACKGROUND_ADVICE_WORKERS = new Set();
...
// ---- v8 permanent payment finalisation: identity + paid-service attach before dashboard ----
// Problem fixed: the old finalise route retrieved Stripe and ran attachPaidSession()
// inline before sending any dashboard identity back to the browser. If Stripe/DB/PDF
// queue work was slow, payment-complete timed out and account-dashboard opened blank.
// Rule: finalise-return must return client identity/dashboard token fast AND synchronously
// attach the paid service record where Stripe confirms paid. Advice/PDF generation remains background-only.
const PAYMENT_FINALISE_FAST_VERSION = 'payment-finalise-identity-plus-paid-attach-v8';
const PAYMENT_FINALISE_INLINE_TIMEOUT_MS = Math.max(2500, Number(process.env.PAYMENT_FINALISE_INLINE_TIMEOUT_MS || 6500));
const paymentFinaliseReturnLocks = new Map();

function withSoftTimeout(promise, ms, fallback, label) {
  let settled = false;
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      console.warn(`${label || 'operation'} exceeded ${ms}ms; returning fallback.`);
      resolve(fallback);
    }, ms);
    Promise.resolve(promise)
      ...
      .catch((err) => {
        ...
        settled = true;
        clearTimeout(timer);
        console.warn(`${label || 'operation'} failed:`, err && err.message ? err.message : err);
        resolve(fallback);
      });
  });
}

function cleanStripeSessionIdValue(value) {
  const sid = String(value || '').trim();
  return /^cs_(test|live)_/i.test(sid) && !sid.includes('{CHECKOUT_SESSION_ID}') ? sid : '';
}

async function findFastPaymentReturnContext(sessionId, req = null) {
  const sid = cleanStripeSessionIdValue(sessionId);
  const assessmentFromRequest = String(
    (req && req.body && (req.body.assessment_id || req.body.assessmentId)) ||
    (req && req.query && (req.query.assessment_id || req.query.assessmentId)) ||
    ''
  ).trim();
  ...
  const empty = {
    ...
    client: null,
    clientEmail: '',
    assessmentId: assessmentFromRequest || '',
    serviceSessionId: '',
  ...
  if (!sid && !assessmentFromRequest) return empty;

  const candidates = [];

  async function pushRows(sql, params, source) {
    try {
      const result = await query(sql, params);
      for (const row of result.rows || []) candidates.push({ ...row, __source: source });
    } catch (err) {
      console.warn(`Fast finalise local lookup skipped (${source}):`, err && err.message ? err.message : err);
    }
  }

  if (sid) {
    await pushRows(
      `SELECT client_id, client_email, service_type, service_ref AS assessment_id,
              selected_plan AS plan, id AS service_session_id, payment_status, stripe_session_id
       FROM service_sessions
       WHERE stripe_session_id=$1
       ORDER BY updated_at DESC NULLS LAST, created_at DESC NULLS LAST
       LIMIT 3`,
      [sid],
      'service_sessions.stripe_session_id'
    );

    await pushRows(
      `SELECT client_id, client_email, service_type, service_ref AS assessment_id,
              plan, NULL::text AS service_session_id, status AS payment_status, stripe_session_id
       FROM payments
       WHERE stripe_session_id=$1
    ...
    await pushRows(
      `SELECT client_id, COALESCE(client_email, applicant_email) AS client_email,
              'visa_assessment'::text AS service_type, id AS assessment_id,
              COALESCE(selected_plan, active_plan) AS plan,
              NULL::text AS service_session_id, payment_status, stripe_session_id
       FROM assessments
       WHERE stripe_session_id=$1
       ORDER BY updated_at DESC NULLS LAST, created_at DESC NULLS LAST
       LIMIT 3`,
      [sid],
      'assessments.stripe_session_id'
    );
  }

  if (assessmentFromRequest) {
    await pushRows(
      `SELECT client_id, COALESCE(client_email, applicant_email) AS client_email,
              'visa_assessment'::text AS service_type, id AS assessment_id,
              COALESCE(selected_plan, active_plan) AS plan,
  ...
  let chosen = candidates.find(r => normaliseEmail(r.client_email) && r.client_id)
    || candidates.find(r => normaliseEmail(r.client_email))
    || candidates[0];
  ...
  if (chosen.client_id) {
    try {
      client = (await query('SELECT id, email, name FROM clients WHERE id=$1 LIMIT 1', [chosen.client_id])).rows[0] || null;
    } catch (err) {
      console.warn('Fast finalise client lookup by id failed:', err.message);
    }
  }
  const email = normaliseEmail((client && client.email) || chosen.client_email);
  if (!client && email) {
    try {
      client = (await query('SELECT id, email, name FROM clients WHERE lower(email)=lower($1) LIMIT 1', [email])).rows[0] || null;
    } catch (err) {
      console.warn('Fast finalise client lookup by email failed:', err.message);
    }
  }

  return {
    sessionId: sid || chosen.stripe_session_id || '',
    client,
    clientEmail: email,
    assessmentId: chosen.assessment_id || assessmentFromRequest || '',
    serviceSessionId: chosen.service_session_id || '',
    serviceType: normaliseServiceType(chosen.service_type || 'visa_assessment'),
    plan: chosen.plan || null,
    paymentStatus: chosen.payment_status || '',
    source: chosen.__source || 'local'
  };
}

async function contextFromStripeForFastFinalise(sessionId, req = null) {
  ...
  try {
    ...
    console.warn('Fast finalise Stripe lookup failed:', err && err.message ? err.message : err);
    return null;
  }

  const md = session.metadata || {};
  const email = normaliseEmail(md.client_email || md.email || session.customer_email);
  let client = null;
  if (email) {
    try {
      client = (await query('SELECT id, email, name FROM clients WHERE lower(email)=lower($1) LIMIT 1', [email])).rows[0] || null;
    } catch (err) {
      console.warn('Fast finalise Stripe client lookup failed:', err.message);
    }
  }

  // Preserve local service session hints where possible.
  const local = await findFastPaymentReturnContext(sid, req);
  return {
    session,
    sessionId: sid,
    client: client || local.client || null,
    clientEmail: normaliseEmail((client && client.email) || email || local.clientEmail),
    assessmentId: md.assessment_id || md.service_ref || local.assessmentId || '',
    serviceSessionId: md.service_session_id || local.serviceSessionId || '',
    serviceType: normaliseServiceType(md.service_type || local.serviceType || 'visa_assessment'),
    plan: md.plan || local.plan || null,
    paymentStatus: (session.payment_status === 'paid' || session.status === 'complete') ? 'paid' : (local.paymentStatus || ''),
    source: 'stripe'
  };
}

function schedulePaymentFinaliseBackground(sessionId) {
  const sid = cleanStripeSessionIdValue(sessionId);
  if (!sid || !stripe) return { queued: false, status: 'not_required' };

  const now = Date.now();
  const lockedAt = Number(paymentFinaliseReturnLocks.get(sid) || 0);
  if (lockedAt && now - lockedAt < 90000) {
    return { queued: false, status: 'already_queued', sessionId: sid };
  }

  paymentFinaliseReturnLocks.set(sid, now);
  setImmediate(async () => {
    try {
      ...
      const paid = session.payment_status === 'paid' || session.status === 'complete';
      if (!paid) {
        console.warn('Fast finalise background attach skipped because Stripe session is not paid:', sid);
        paymentFinaliseReturnLocks.delete(sid);
        return;
      }
      await attachPaidSession(session, { triggerGeneration: true, waitForPdf: false });
      paymentFinaliseReturnLocks.delete(sid);
    } catch (err) {
      console.warn('Fast finalise background attach failed:', sid, err && err.message ? err.message : err);
      paymentFinaliseReturnLocks.delete(sid);
    }
  });

  return { queued: true, status: 'background_queued', sessionId: sid };
}

function buildPaymentFinaliseResponsePayload({ context, sessionId, background, stripeChecked }) {
  const serviceType = normaliseServiceType(context.serviceType || 'visa_assessment');
  const isCitizenship = serviceType === 'citizenship_test' || serviceType === 'citizenship';
  const assessmentId = context.assessmentId || '';
  const accessId = context.accessId || '';
  const redirectUrl = isCitizenship
    ? `${APP_BASE_URL}/account-dashboard.html?payment=verified&service=citizenship&citizenship=active&access_id=${encodeURIComponent(accessId || assessmentId)}&session_id=${encodeURIComponent(sessionId)}`
    : `${APP_BASE_URL}/account-dashboard.html?payment=verified&service=visa_assessment&assessment_id=${encodeURIComponent(assessmentId)}&session_id=${encodeURIComponent(sessionId)}`;

  const dashboardAccessToken = context.client ? signDashboardAccessToken(context.client) : null;
  return {
    ok: true,
    version: PAYMENT_FINALISE_FAST_VERSION,
    status: context.paymentStatus === 'paid' ? 'paid' : 'pending_verification',
    paymentLinked: context.paymentStatus === 'paid',
    finalisationQueued: Boolean(background && background.queued),
    background,
    stripeChecked: Boolean(stripeChecked),
    service: isCitizenship ? 'citizenship' : serviceType,
    sessionId,
    assessmentId,
    serviceSessionId: context.serviceSessionId || null,
    accessId: accessId || null,
    citizenshipAccessId: accessId || null,
    plan: context.plan || null,
    clientEmail: context.clientEmail || (context.client && context.client.email) || null,
    client: context.client ? { id: context.client.id, email: context.client.email, name: context.client.name || null } : null,
    dashboardAccessToken,
    accessToken: dashboardAccessToken,
    redirectUrl,
    message: 'Payment return accepted. Dashboard identity and paid service attachment are completed before dashboard release where Stripe confirms paid; advice generation remains background-only.'
  };
}

async function finaliseStripePayment(req, res) {
  const sessionId = cleanStripeSessionIdValue(
    (req.body && (req.body.sessionId || req.body.session_id || req.body.checkoutSessionId)) ||
  ...
  // 1) Resolve account identity from local DB first. This is fast and avoids Stripe latency.
  let context = await withSoftTimeout(
    findFastPaymentReturnContext(sessionId, req),
    2500,
    { sessionId, client: null, clientEmail: '', assessmentId: '', serviceType: 'visa_assessment', paymentStatus: '', source: 'timeout' },
    'fast payment local context lookup'
  );

  // 2) If local context is missing OR not yet marked paid, perform a bounded Stripe lookup.
  // v8 correction: v7 could restore identity from an unpaid service_session and skip Stripe,
  // so the dashboard opened with the right email but no paid service card. Finalisation must
  // confirm/attach the paid matter before the dashboard is treated as ready.
  let stripeContext = null;
  if (!context.client || !context.clientEmail || !context.assessmentId || context.paymentStatus !== 'paid') {
    stripeContext = await withSoftTimeout(
      contextFromStripeForFastFinalise(sessionId, req),
      PAYMENT_FINALISE_INLINE_TIMEOUT_MS,
      null,
      'fast payment Stripe identity lookup'
    );
    if (stripeContext) {
      context = {
        ...context,
        ...stripeContext,
        client: stripeContext.client || context.client,
        clientEmail: stripeContext.clientEmail || context.clientEmail,
        assessmentId: stripeContext.assessmentId || context.assessmentId,
        serviceSessionId: stripeContext.serviceSessionId || context.serviceSessionId,
        serviceType: stripeContext.serviceType || context.serviceType,
        plan: stripeContext.plan || context.plan,
        paymentStatus: stripeContext.paymentStatus || context.paymentStatus
      };
    }
  }

  // 3) If Stripe says paid, synchronously attach the paid assessment/service record.
  // This is database/payment linking only. Advice/PDF generation is still background-only.
  let inlineAttach = null;
  if (stripeContext && stripeContext.session && stripeContext.paymentStatus === 'paid') {
    inlineAttach = await withSoftTimeout(
      attachPaidSession(stripeContext.session, { triggerGeneration: true, waitForPdf: false }),
      Math.max(6000, Number(process.env.PAYMENT_FINALISE_ATTACH_TIMEOUT_MS || 12000)),
      { timedOut: true },
      'payment finalise paid-service attach'
    );

    if (inlineAttach && !inlineAttach.timedOut && inlineAttach.attached !== false) {
      context = {
        ...context,
        paymentStatus: 'paid',
        assessmentId: inlineAttach.assessmentId || context.assessmentId,
        accessId: inlineAttach.accessId || context.accessId,
        serviceType: inlineAttach.type || context.serviceType,
        plan: inlineAttach.plan || context.plan,
        source: 'stripe_inline_paid_attach'
      };
    }
  }

  // 4) Queue background repair only when the inline paid attach did not complete.
  const background = (inlineAttach && !inlineAttach.timedOut && inlineAttach.attached !== false)
...
function buildCriterionFindingFromProfile(profile, context) {
  ...
  const criterionKey = normaliseKey(profile.key || profile.criterion || '');

  function aliasKeysForProfile(p) {
    const key = normaliseKey(p.key || '');
    const base = Array.isArray(p.answerKeys) ? [...p.answerKeys] : [];
    const aliases = {
      stream: ['selectedStream','selected_stream','nominationStream','stream','pathway'],
      directentry: ['skillsAssessment','skills_assessment','skills-assessment','qualification','qualifications','registration','licensing','occupation','nominatedOccupation'],
      direct_entry: ['skillsAssessment','skills_assessment','skills-assessment','qualification','qualifications','registration','licensing','occupation','nominatedOccupation'],
      labouragreement: ['labourAgreement','labour_agreement','labour-agreement','agreementConcession','concession','selectedStream','stream'],
      labour_agreement: ['labourAgreement','labour_agreement','labour-agreement','agreementConcession','concession','selectedStream','stream'],
      trt: ['trtStartDate','trt_start_date','continuousWork','continuous_work','previousSponsoredVisas','sponsorContinuity'],
      sponsor: ['employerName','employer_name','employer-name','sponsorName','sponsor_name','businessName','business_name','currentEmployer','current_employer'],
      genuineposition: ['roleOngoing','role_ongoing','roleFullTime','role_full_time','businessNeed','business_need','employeeCount','employee_count'],
      genuine_position: ['roleOngoing','role_ongoing','roleFullTime','role_full_time','businessNeed','business_need','employeeCount','employee_count'],
      occupation: ['nominatedOccupation','nominated_occupation','occupation','anzsco','jobTitle','job_title','dailyDuties','daily_duties','duties'],
      employment: ['continuousWork','continuous_work','trtStartDate','trt_start_date','employmentStartDate','employment_start_date','weeklyHours','weekly_hours'],
      salary: ['salaryOffered','salary_offered','annualSalary','annual_salary','salary','remuneration','baseSalary','base_salary'],
      english: ['englishEvidence','english_evidence','englishTestType','english_test_type','englishScore','english_score','englishGrade','english_grade','englishReading','englishWriting','englishSpeaking','englishListening','pte','ielts','oet','toefl'],
      validity: ['passportAvailable','passport_available','passport-available','identityDocsConsistent','identity_docs_consistent','currentLocation','current_location','currentVisaStatus','current_visa_status'],
      health: ['seriousMedical','serious_medical','healthIssues','health_issues','health'],
      character: ['criminalHistory','criminal_history','characterIssues','character_issues','pic4020','falseInformation','false_information'],
      migrationhistory: ['visaRefused','visa_refused','visaCancelled','visa_cancelled','unlawfulStatus','unlawful_status','section48Mentioned','section48','8503'],
      migration_history: ['visaRefused','visa_refused','visaCancelled','visa_cancelled','unlawfulStatus','unlawful_status','section48Mentioned','section48','8503']
    };
    return [...(aliases[key] || []), ...base];
  }

  function getAny(keys) {
    const wanted = (keys || []).map(normaliseKey).filter(Boolean);
    const entries = Object.entries(flat || {}).map(([k, v]) => [k, normaliseKey(k), v]);

    // First pass: exact key match only.
    for (const w of wanted) {
      for (const [rawKey, nk, v] of entries) {
        if (nk === w) {
    ...
    // Second pass: safe suffix match for long, specific field names only.
    for (const w of wanted.filter(x => x.length >= 8)) {
      for (const [rawKey, nk, v] of entries) {
        if (nk.endsWith(w) || w.endsWith(nk)) {
          const text = String(v ?? '').replace(/\s+/g, ' ').trim();
          if (text) return text;
        }
      }
  ...
  const answer = getAny(aliasKeysForProfile(profile));
  ...
  let finding = answer
    ...
    : 'The available questionnaire material does not finally establish this criterion. It must be assessed against the original evidence before lodgement-ready advice is issued.';

  let legal_consequence = status.tone === 'risk'
  ...
  if (criterionKey === 'english') {
    if (!answer || /\b(no|none|not available|not yet|missing|unknown|false)\b/i.test(lowerAnswer)) {
      finding = 'English evidence has not been verified. The file should not proceed on assumed English eligibility unless a valid test result, eligible passport evidence, exemption or lawful concession is identified and checked against the current legal threshold.';
      ...
      finding = `English evidence has been disclosed as "${answer}", but the original test report, test type, component scores or grade, validity date, eligible passport evidence, exemption basis or lawful concession must be verified before lodgement-ready advice is issued.`;
      legal_consequence = 'The English position may be supportable only if the evidence is valid, current and meets the applicable threshold, exemption or concession for the selected stream.';
      delegateRisk = 'Moderate Delegate Risk';
    }
  } else if (criterionKey === 'salary') {
    finding = answer
      ? `The salary/remuneration figure recorded is ${answer}. It must be reconciled against the nomination, employment contract, payroll, superannuation, tax records, market salary evidence and any applicable concession.`
      : 'The salary/remuneration position has not been sufficiently identified and must be reconciled against nomination and employment records before lodgement-ready advice is issued.';
    ...
    delegateRisk = 'Moderate Delegate Risk';
  } else if (criterionKey === 'labouragreement' || criterionKey === 'labour_agreement') {
    finding = 'The Labour Agreement pathway must be assessed against the executed agreement itself. The critical issue is whether the agreement covers the employer, nominated occupation, nomination limits, salary settings, English or age concessions, and any conditions attached to the agreement.';
  ...
  } else if (criterionKey === 'directentry' || criterionKey === 'direct_entry') {
    finding = 'For the Direct Entry stream, the assessment should focus on skills assessment, occupation eligibility, qualifications, employment history, registration or licensing requirements and the fit between the applicant’s background and the nominated position. It should not be treated as a TRT qualifying-employment analysis unless the selected stream changes.';
    legal_consequence = 'If the skills assessment, occupation eligibility, registration/licensing or qualification evidence cannot be verified, the Direct Entry pathway may not be lodgement ready.';
    delegateRisk = 'Moderate Delegate Risk';
  } else if (criterionKey === 'trt') {
    finding = 'For the Temporary Residence Transition stream, the qualifying employment period, sponsor continuity, nominated occupation continuity and visa/work-rights history must be reconstructed from objective records.';
    legal_consequence = 'If qualifying employment, sponsor continuity or occupation continuity cannot be verified, the TRT pathway may become unsuitable or exposed to refusal risk.';
    delegateRisk = 'Moderate Delegate Risk';
  } else if (criterionKey === 'sponsor' || criterionKey === 'genuineposition' || criterionKey === 'genuine_position') {
    finding = 'The nomination and employer material must show a coherent connection between the business operations, the nominated position, the applicant’s actual duties and the ongoing need for the role. A delegate is likely to test the employer’s structure, payroll capacity, trading activity and organisational chart rather than relying only on broad employer assertions.';
    legal_consequence = 'If the nomination, genuine-position or employer-capacity evidence is inconsistent or incomplete, the nomination may become a primary refusal risk.';
    delegateRisk = 'Moderate Delegate Risk';
  } else if (criterionKey === 'occupation') {
    finding = 'The occupation analysis should not be limited to the job title. The actual duties, seniority level, technical responsibilities, qualifications and employment history must support the nominated occupation.';
    legal_consequence = 'If the duties and evidence do not align with the nominated occupation, the occupation or nomination position may become a primary refusal risk.';
    delegateRisk = 'Moderate Delegate Risk';
  } else if (criterionKey === 'health') {
    finding = /yes|issue|condition|medical/i.test(lowerAnswer)
      ? 'A health issue has been disclosed or may require clarification. The matter should be reviewed against Departmental health requirements and any family-member health considerations.'
      : 'No health issue has been disclosed in the questionnaire. The applicant must still satisfy standard health requirements, and the position remains subject to Departmental health examinations and any family-member health considerations.';
    ...
    delegateRisk = /yes|issue|condition|medical/i.test(lowerAnswer) ? 'Moderate Delegate Risk' : 'Standard Requirement';
  } else if (criterionKey === 'character' || criterionKey === 'migrationhistory' || criterionKey === 'migration_history') {
    finding = /yes|refus|cancel|criminal|court|false|misleading|unlawful|section|8503/i.test(lowerAnswer)
      ? 'An adverse character, integrity or immigration-history issue has been disclosed or may require clarification. The issue should be reviewed against Departmental records before lodgement strategy is finalised.'
      : 'No character, integrity or adverse immigration-history issue has been disclosed in the assessment response. This remains subject to police clearances, Departmental records and document-consistency checks.';
    legal_consequence = 'If character, integrity or immigration-history records are inconsistent with the instructions, the matter may require strategy before lodgement.';
...
async function resetStuckAdviceForAssessment(assessmentId, reason = 'Advice generation was reset after becoming stuck.') {
  if (!assessmentId) return null;
  await query(
    `UPDATE assessments
     ...
     WHERE id=$1
       AND payment_status='paid'
       AND status IN ('pdf_generating','processing','advice_generating','pdf_failed','manual_review_required')`,
    [assessmentId, reason]
  ).catch(() => null);

  await query(
    `INSERT INTO pdf_jobs (assessment_id, status, run_after, locked_at, last_error, created_at, updated_at)
     VALUES ($1,'queued',now(),NULL,$2,now(),now())
  ...
  return { assessmentId, reset: true };
}

function runAdviceWorkerInBackground({ assessmentId, limit }) {
  const key = assessmentId || `limit:${limit || 1}`;
  if (BM_BACKGROUND_ADVICE_WORKERS.has(key)) {
    return { started: false, alreadyRunning: true, key };
  }

  BM_BACKGROUND_ADVICE_WORKERS.add(key);

  setImmediate(async () => {
    try {
      await runDueAdviceJobs({
        generateAssessmentPdfNow,
        limit: Math.max(1, Math.min(10, Number(limit || 1))),
        assessmentId: assessmentId || undefined
      });
    } catch (err) {
      const message = String(err && err.message ? err.message : err || 'Background advice worker failed.');
      console.error('Background advice worker failed:', assessmentId || key, message);
      if (assessmentId) {
        await markPdfGenerationFailed(assessmentId, message).catch(() => null);
      }
    } finally {
      BM_BACKGROUND_ADVICE_WORKERS.delete(key);
    }
  });

  return { started: true, alreadyRunning: false, key };
}

async function resetStuckAdviceHttp(req, res) {
  const assessmentId = await resolveAdviceAssessmentId(req);
  if (!assessmentId) {
    return res.status(400).json({
      ok: false,
      error: 'assessment_id or session_id is required to reset a stuck advice job.'
    });
  }

  const result = await resetStuckAdviceForAssessment(
    assessmentId,
    'Advice generation was manually reset because it was stuck at the generating stage.'
  );
...
async function runAdviceWorkerHttp(req, res) {
  ...
  const assessmentId = await resolveAdviceAssessmentId(req);
  const wait = /^(1|true|yes)$/i.test(String((req.body && req.body.wait) || req.query.wait || ''));

  // If a specific assessment/session is supplied, align the job table first.
  // This clears stuck processing/failed job states and lets the worker pick it up again.
  if (assessmentId) {
    await createPaidAdviceJob({ assessmentId, runAfter: null, resetFailure: true });
  }

  // Default behaviour is now non-blocking, because PDF generation can exceed Render's
  // HTTP timeout and leave the dashboard stuck at "Advice letter generating · 70%".
  if (!wait) {
    const background = runAdviceWorkerInBackground({ assessmentId, limit });
    return res.status(202).json({
      ok: true,
      worker: 'universal-paid-advice-pipeline-v5-background-no-http-timeout',
      mode: 'background',
      assessmentId: assessmentId || null,
      limit,
      ...background,
      message: background.alreadyRunning
        ? 'Advice worker is already running for this matter.'
        : 'Advice worker started in the background. Refresh the dashboard shortly.'
    });
  }
  ...
  res.json({
    ok: true,
    worker: 'universal-paid-advice-pipeline-v5-wait-mode',
    mode: 'wait',
    assessmentId: assessmentId || null,
    results
  });
...
app.post('/api/admin/reset-stuck-advice', asyncRoute(resetStuckAdviceHttp));
app.get('/api/admin/reset-stuck-advice', asyncRoute(resetStuckAdviceHttp));
