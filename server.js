...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null) {
  ...
  if (existingByStripeSession && existingByStripeSession.paymentStatus === 'paid') return existingByStripeSession;
...
const effectiveUser = user || (metadata.userId ? getUsersStore().users[String(metadata.userId || '').trim()] : null);
const visaAssessments = effectiveUser ? await getVisaAssessmentsForDashboard(effectiveUser) : (submission ? [publicDashboardAssessment(submission)] : []);
return res.json({
  ok: verified,
  paid,
  verified,
  ...
  verifiedPayment: { product: 'visa_assessment', paid, sessionId: session.id, assessment: submission ? publicDashboardAssessment(submission) : null },
  user: effectiveUser ? publicUser(effectiveUser) : null,
  visaAssessments,
