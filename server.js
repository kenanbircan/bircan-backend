...
function normaliseCitizenshipPlan(plan) {
  ...
  const raw = rawInput.toLowerCase().replace(/\s+/g, '').replace(/-/g, '').replace(/\$/g, '').replace(/aud/g, '');

  // Production citizenship pricing fix:
  // The public page uses dollar plans ($50, $100, $150). Older backend code treated
  // citizenship values as exam-count plans and silently fell back to the default plan,
  // which could send every selection to the same Stripe Price. These aliases preserve
  // the selected commercial plan all the way into Stripe metadata and price lookup.
  if (['50', '50plan', '50payment', '50package', 'standard', 'basic', 'starter'].includes(raw)) return '50';
  if (['100', '100plan', '100payment', '100package', 'premium', 'plus'].includes(raw)) return '100';
  if (['150', '150plan', '150payment', '150package', 'professional', 'complete', 'unlimited', 'unlimitedexams', 'unlimitedexam', 'all'].includes(raw)) return '150';

  // Backwards compatibility for old exam-count labels. Keep them from accidentally
  // overriding the paid $50/$100/$150 options unless the frontend explicitly sends them.
  if (['20', '20exam', '20exams'].includes(raw)) return '20';
  return '50';
}
