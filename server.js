...
const corsOptions = {
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'Accept', 'X-Requested-With', 'X-Auth-Token', 'X-Admin-Token', 'X-Admin-Password'],
...
app.use('/api/admin', (req, res, next) => {
  ...
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, Accept, X-Requested-With, X-Auth-Token, X-Admin-Token, X-Admin-Password');
...
// ---------- Paid citizenship exam access API ----------
// The paid exam frontend calls these routes. They always return a stable response
// shape with a questions array, so older frontend files cannot crash on
// `questions.length` when the backend is missing a field.
...
async function getActivePaidCitizenshipAccess(req) {
  ...
  const params = [req.client.id, normaliseEmail(req.client.email)];
  let where = `(client_id=$1 OR lower(client_email)=lower($2)) AND (payment_status='paid' OR status='active' OR status='paid')`;
  if (requestedAccessId) {
    params.push(requestedAccessId);
    where = `id=$3 AND ${where}`;
  }
  const { rows } = await query(
...
function calculateCitizenshipResultFromPayload(questions, answers) {
  ...
  const review = qs.map((q, i) => {
    ...
    return {
      ...
      correct,
      correctIndex,
      selectedText: selected === null || !Array.isArray(q.options) ? 'No answer selected' : (q.options[selected] || 'No answer selected'),
      correctText: Array.isArray(q.options) ? (q.options[correctIndex] || '') : '',
      explanation: q.explanation || '',
      category: q.category || (q.isValue ? 'Australian values' : 'General')
    };
  });
  const score = review.filter(r => r.correct).length;
  const valueIndexes = qs.map((q, i) => q.isValue ? i : null).filter(i => i !== null).slice(0, 5);
  ...
  review.forEach(r => {
    ...
    categories[k] = categories[k] || { total: 0, correct: 0 };
  ...
  return { score, valuesCorrect, valuesTotal, passed, reasons, categories, review };
}
...
async function handlePaidCitizenshipExamSubmit(req, res) {
  ...
  const row = updated.rows[0] || access;
  const plan = normaliseCitizenshipPlan(row.active_plan || row.selected_plan || access.active_plan || access.selected_plan || '20');
  ...
  return res.json({ ok: true, examId: req.body && req.body.examId, result, entitlement, attempts: { used, remaining: remainingAfter, total: allowance } });
}
...
async function handleFreeCitizenshipExamSubmit(req, res) {
  ...
  const answers = Array.isArray(req.body && req.body.answers) ? req.body.answers : [];
  const result = calculateCitizenshipResultFromPayload(questions, answers);
  return res.json({ ok: true, examId: req.body && req.body.examId, result, attempts: { used: 1, remaining: 2, total: 3 } });
}

app.post('/api/exam/start', requireAuth, asyncRoute(handlePaidCitizenshipExamStart));
app.get('/api/exam/start', requireAuth, asyncRoute(handlePaidCitizenshipExamStart));
app.post('/api/citizenship/exam/start', requireAuth, asyncRoute(handlePaidCitizenshipExamStart));
app.post('/api/citizenship-test/exam/start', requireAuth, asyncRoute(handlePaidCitizenshipExamStart));
app.get('/api/exam/status', requireAuth, asyncRoute(handlePaidCitizenshipExamStatus));
app.post('/api/exam/submit', requireAuth, asyncRoute(handlePaidCitizenshipExamSubmit));
app.post('/api/citizenship/exam/submit', requireAuth, asyncRoute(handlePaidCitizenshipExamSubmit));
app.post('/api/citizenship-test/exam/submit', requireAuth, asyncRoute(handlePaidCitizenshipExamSubmit));
