...
function drawHeader(doc, submission, analysis, isFirstPage = false) {
  ...
  if (isFirstPage) {
    doc.save();
    doc.roundedRect(30, 22, 535, 110, 10).lineWidth(1).strokeColor('#dce4ef').stroke();

    if (fs.existsSync(lp)) {
      try {
        doc.image(lp, 42, 34, { fit: [150, 82], align: 'left', valign: 'center' });
      } catch (_) {}
    }

    doc.moveTo(310, 34).lineTo(310, 118).strokeColor('#dce4ef').stroke();

    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(13)
      .text(FIRM_PROFILE.businessName, 200, 38, { width: 180 });
    doc.fillColor('#172033').font('Helvetica').fontSize(9.5)
      .text(FIRM_PROFILE.website, 200, 58, { width: 180 })
      .text(FIRM_PROFILE.email, 200, 73, { width: 180 })
      .text(FIRM_PROFILE.phone, 200, 88, { width: 180 })
      .text(FIRM_PROFILE.address, 200, 103, { width: 180 });

    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(15)
      .text(FIRM_PROFILE.principalName, 326, 38, { width: 210 });
    doc.fillColor('#172033').font('Helvetica-Bold').fontSize(10)
      .text(FIRM_PROFILE.title, 326, 61, { width: 210 });
    doc.fillColor('#172033').font('Helvetica').fontSize(9.5)
      .text(`MARN: ${FIRM_PROFILE.marn}`, 326, 80, { width: 210 })
      .text(`MMIA: ${FIRM_PROFILE.mmia}`, 326, 95, { width: 95 })
      .text(`CMA: ${FIRM_PROFILE.cma}`, 401, 95, { width: 80 })
      .text(`JP No: ${FIRM_PROFILE.jpNo}`, 466, 95, { width: 74, align: 'right' });

    doc.restore();

    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(20)
      .text('Preliminary Visa Assessment Letter', 40, 158, { width: 360 });
    doc.fillColor('#9a6b00').font('Helvetica-Bold').fontSize(11)
      .text(`${submission.visaType || 'Subclass 482'} | ${analysis.eligibilityOutcome || 'Preliminary outcome'}`, 40, 186);

    doc.fillColor('#5f6b7d').font('Helvetica').fontSize(9.2)
      .text(`Date: ${todayHuman()}`, 425, 160, { width: 120, align: 'right' })
      .text(`Submission ID: ${submission.id}`, 365, 176, { width: 180, align: 'right' });

    doc.moveTo(40, 208).lineTo(555, 208).strokeColor('#dce4ef').stroke();
    doc.y = 224;
    return;
  }
  ...
  doc.save();
  doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(13)
    .text('Preliminary Visa Assessment Letter', 40, 34, { width: 280 });
  doc.fillColor('#5f6b7d').font('Helvetica').fontSize(9.2)
    .text(`${submission.visaType || 'Subclass 482'} | ${analysis.eligibilityOutcome || 'Preliminary outcome'}`, 40, 50, { width: 280 })
    .text(`Page heading`, 0, 0, { width: 0 });
  doc.moveTo(40, 74).lineTo(555, 74).strokeColor('#dce4ef').stroke();
  doc.restore();
...
function drawFooterOnCurrentPage(doc, pageNumber, pageCount) {
  ...
  doc.save();
  doc.moveTo(40, footerY - 8).lineTo(555, footerY - 8).strokeColor('#dce4ef').stroke();
  doc.fillColor('#5f6b7d').font('Helvetica').fontSize(8)
    .text('Bircan Migration & Education | www.bircanmigration.com.au | kenan@bircanmigration.com.au | 0421 618 522', 40, footerY, { width: 380 });
  doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(8)
    .text(`Page ${pageNumber} of ${pageCount}`, 420, footerY, { width: 120, align: 'right' });
  doc.restore();
  doc.fillColor('#172033');
}

function ensureSpace(doc, submission, analysis, threshold = 705) {
  if (doc.y > threshold) {
    doc.addPage();
...
function addSectionTitle(doc, submission, analysis, title) {
  ensureSpace(doc, submission, analysis, 685);
  doc.moveDown(0.35);
  doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(12.2).text(title);
  doc.moveDown(0.18);
}

function addParagraph(doc, submission, analysis, text) {
  ensureSpace(doc, submission, analysis, 724);
  doc.fillColor('#172033').font('Helvetica').fontSize(10.3).text(String(text || ''), {
    width: 515,
    lineGap: 3,
    align: 'left',
  });
...
function addBulletList(doc, submission, analysis, items, fallbackText = 'Not provided.') {
  ...
  list.forEach(item => {
    ensureSpace(doc, submission, analysis, 728);
    const y = doc.y;
    doc.circle(47, y + 6.4, 2).fill('#0b1f3a');
    doc.fillColor('#172033').font('Helvetica').fontSize(10.1).text(String(item || ''), 58, y, {
      width: 492,
      lineGap: 3,
    });
    doc.moveDown(0.34);
...
function addOutcomeSummary(doc, submission, analysis) {
  ensureSpace(doc, submission, analysis, 650);
  doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(12.5).text('Overall Preliminary Outcome');
  doc.moveDown(0.2);
  doc.fillColor('#9a6b00').font('Helvetica-Bold').fontSize(22).text(analysis.eligibilityOutcome || 'Preliminary outcome pending');
  doc.moveDown(0.25);
  addParagraph(doc, submission, analysis, analysis.executiveSummary || 'Assessment summary not available.');
}

function addClientMatterTable(doc, submission, analysis) {
  ensureSpace(doc, submission, analysis, 665);
  const rows = [
    ['Client Name', submission.client?.fullName || 'Not provided'],
    ['Nationality', submission.client?.nationality || 'Not provided'],
    ['Visa Matter', submission.visaType || 'Not provided'],
  ...
  rows.forEach((row, idx) => {
    ...
    const y = doc.y;
    doc.fillColor(idx % 2 === 0 ? '#f7f9fc' : '#ffffff').rect(40, y - 2, 515, 22).fill();
    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(9.6).text(row[0], 48, y + 4, { width: 145 });
    doc.fillColor('#172033').font('Helvetica').fontSize(9.6).text(row[1], 200, y + 4, { width: 340 });
    doc.y = y + 24;
  });
  doc.moveDown(0.3);
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    addOutcomeSummary(doc, submission, analysis);
    addClientMatterTable(doc, submission, analysis);

    addSectionTitle(doc, submission, analysis, 'Preliminary Advice');
    ...
    addSectionTitle(doc, submission, analysis, 'Key Strengths');
    ...
    addSectionTitle(doc, submission, analysis, 'Key Risks or Concerns');
    addBulletList(doc, submission, analysis, analysis.concerns, 'No material concerns were identified from the currently available information.');
    ...
    addSectionTitle(doc, submission, analysis, 'Assessment Ratings');
