...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null) {
  ...
  const submission = {
    ...
    stripeSessionId: session.id,
    stripePaymentIntentId: typeof session.payment_intent === 'string' ? session.payment_intent : (session.payment_intent?.id || null),
    userId: String(metadata.userId || payload.userId || '').trim() || extracted.userId || null,
    accountEmail: String(metadata.email || payload.email || session.customer_details?.email || session.customer_email || '').trim().toLowerCase() || extracted.accountEmail || null,
    plan,
  };
...
app.post(['/api/assessment/create-pending', '/api/visa/create-pending-assessment'], async (req, res, next) => {
  try {
    const payload = extractAssessmentPayload(req.body?.assessmentPayload || req.body || {});
    if (!payload.client.email) return res.status(400).json({ ok: false, error: 'Client email is required.' });

    const requestedPlanInput = findVisaPlanInput(req.body || {}) || payload.plan;
    ...
    const submission = {
      ...
      planCode: plan.code,
      deliveryPlan: plan.code,
      assessmentPlan: plan.code,
      turnaround: plan.code,
    };
    ...
    return res.status(201).json({
      ...
      message: 'Assessment saved pending secure account login and Stripe payment.'
    });
  } catch (error) { return next(error); }
});
...
app.post(['/create-visa-checkout-session', '/create-assessment-checkout-session', '/api/assessment/create-checkout-session', '/api/assessments/create-checkout-session', '/api/visa/create-checkout-session', '/api/checkout/visa'], async (req, res, next) => {
  try {
    ...
    const submissionId = String(req.body?.submissionId || req.body?.assessmentId || '').trim();
    const existingSubmission = submissionId ? await getSubmission(submissionId) : null;
    let assessmentPayload = req.body?.assessmentPayload || {};
    if (existingSubmission) assessmentPayload = { ...existingSubmission, ...assessmentPayload };
    const subclass = String(req.body?.subclass || assessmentPayload.subclass || assessmentPayload.visaType || '').trim() || 'Visa assessment';
    const requestedPlanInput = findVisaPlanInput(req.body) || (existingSubmission ? findVisaPlanInput({ assessmentPayload: existingSubmission }) : '');
    const plan = normalizeVisaCheckoutPlan(requestedPlanInput || '24h');
    const priceId = getVisaStripePriceIdForPlan(plan.code);
    console.log('[visa-checkout-plan]', { submissionId: submissionId || null, received: requestedPlanInput || null, normalized: plan.code, label: plan.label, hasConfiguredPriceId: Boolean(priceId) });
