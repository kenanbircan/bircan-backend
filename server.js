...
async function findPendingServiceForClient(email, opts = {}) {
  const clientEmail = normaliseEmail(email);
  if (!clientEmail) return null;

  const explicitVisaId = opts.assessmentId || opts.assessment_id || opts.visaAssessmentId || opts.visa_assessment_id;
  const explicitAppealId = opts.appealAssessmentId || opts.appeal_assessment_id || opts.appealId || opts.appeal_id;

  if (explicitVisaId) {
    const { rows } = await query(
      `SELECT 'visa_assessment' AS service_type, id, selected_plan AS plan, visa_type, status, payment_status, created_at
       FROM assessments
       WHERE id=$1 AND lower(client_email)=lower($2) AND COALESCE(payment_status,'unpaid') <> 'paid'
       LIMIT 1`,
      [explicitVisaId, clientEmail]
    );
  ...
  if (explicitAppealId) {
    const { rows } = await query(
      `SELECT 'appeals_assessment' AS service_type, id, selected_plan AS plan, visa_subclass AS visa_type, status, payment_status, created_at
       FROM appeals_assessments
       WHERE id=$1 AND lower(COALESCE(client_email, applicant_email))=lower($2) AND COALESCE(payment_status,'unpaid') <> 'paid'
       LIMIT 1`,
      [explicitAppealId, clientEmail]
    );
    if (rows[0]) return rows[0];
  }
  ...
  const { rows } = await query(
    `SELECT * FROM (
       ...
       WHERE lower(client_email)=lower($1)
         ...
         AND status IN ('submitted','checkout_created','payment_pending','unpaid')
       UNION ALL
       SELECT 'appeals_assessment' AS service_type, id, selected_plan AS plan, visa_subclass AS visa_type, status, payment_status, created_at
       FROM appeals_assessments
       WHERE lower(COALESCE(client_email, applicant_email))=lower($1)
         AND COALESCE(payment_status,'unpaid') <> 'paid'
         AND status IN ('submitted','checkout_created','payment_pending','unpaid')
     ) pending
...
async function claimPendingServiceForClient(client, pending) {
  if (!client || !pending) return null;
  if (pending.service_type === 'visa_assessment') {
    await query(
      `UPDATE assessments
       SET client_id=$1, client_email=$2, applicant_email=COALESCE(applicant_email,$2), active_plan=COALESCE(active_plan, selected_plan), updated_at=now()
       WHERE id=$3 AND lower(client_email)=lower($2) AND COALESCE(payment_status,'unpaid') <> 'paid'`,
      [client.id, client.email, pending.id]
    );
  } else if (pending.service_type === 'appeals_assessment') {
    await query(
      `UPDATE appeals_assessments
       SET client_id=$1, client_email=$2, applicant_email=COALESCE(applicant_email,$2), active_plan=COALESCE(active_plan, selected_plan), updated_at=now()
       WHERE id=$3 AND lower(COALESCE(client_email, applicant_email))=lower($2) AND COALESCE(payment_status,'unpaid') <> 'paid'`,
      [client.id, client.email, pending.id]
    );
  }
  return pending;
...
function buildPendingServiceResponse(pending) {
  if (!pending) return null;
  const isAppeal = pending.service_type === 'appeals_assessment';
  const serviceQuery = isAppeal ? `appeal_assessment_id=${encodeURIComponent(pending.id)}` : `assessment_id=${encodeURIComponent(pending.id)}`;
  return {
    ...
    plan: pending.plan,
    visaType: pending.visa_type || null,
    status: pending.status,
    paymentStatus: pending.payment_status,
    checkoutEndpoint: isAppeal ? '/api/appeals/create-checkout-session' : '/api/assessment/create-checkout-session',
    checkoutBody: isAppeal ? { appealAssessmentId: pending.id, plan: pending.plan } : { assessmentId: pending.id, plan: pending.plan },
    next: `/checkout-start.html?service=${encodeURIComponent(pending.service_type)}&${serviceQuery}`
  };
}

async function buildAuthSuccessPayload(client, token, req) {
  const pending = await findPendingServiceForClient(client.email, { ...(req.body || {}), ...(req.query || {}) });
  if (pending) await claimPendingServiceForClient(client, pending);
  const pendingService = buildPendingServiceResponse(pending);
  return {
    ok: true,
    token,
    client: { id: client.id, email: client.email, name: client.name },
    pendingService,
    pendingAssessment: pendingService && pendingService.serviceType === 'visa_assessment' ? pendingService : null,
    pendingAppealAssessment: pendingService && pendingService.serviceType === 'appeals_assessment' ? pendingService : null,
    redirectToCheckout: Boolean(pendingService),
    redirectUrl: pendingService ? pendingService.next : `${APP_BASE_URL}/account-dashboard.html`
  };
}

async function findVisaAssessmentForCheckout(req) {
  const explicitId = req.body.assessmentId || req.body.assessment_id || req.query.assessment_id || req.query.assessmentId;
  const email = normaliseEmail(req.client.email);
  if (explicitId) {
    const { rows } = await query('SELECT * FROM assessments WHERE id=$1 AND lower(client_email)=lower($2)', [explicitId, email]);
    return rows[0] || null;
  }
  const { rows } = await query(
    `SELECT * FROM assessments
     WHERE lower(client_email)=lower($1) AND COALESCE(payment_status,'unpaid') <> 'paid'
     ORDER BY created_at DESC
     LIMIT 1`,
    [email]
  );
...
async function findAppealAssessmentForCheckout(req) {
  const explicitId = req.body.assessmentId || req.body.assessment_id || req.body.appealAssessmentId || req.body.appeal_assessment_id || req.query.appeal_assessment_id || req.query.appealAssessmentId;
  const email = normaliseEmail(req.client.email);
  if (explicitId) {
    const { rows } = await query('SELECT * FROM appeals_assessments WHERE id=$1 AND lower(COALESCE(client_email, applicant_email))=lower($2)', [explicitId, email]);
    return rows[0] || null;
  }
  const { rows } = await query(
    `SELECT * FROM appeals_assessments
     WHERE lower(COALESCE(client_email, applicant_email))=lower($1) AND COALESCE(payment_status,'unpaid') <> 'paid'
     ORDER BY created_at DESC
     LIMIT 1`,
    [email]
  );
...
app.post('/api/auth/register', asyncRoute(async (req, res) => {
  ...
  res.json(await buildAuthSuccessPayload(client, token, req));
}));
...
app.get('/api/account/pending-service', requireAuth, asyncRoute(async (req, res) => {
  const pending = await findPendingServiceForClient(req.client.email, req.query || {});
  if (pending) await claimPendingServiceForClient(req.client, pending);
  res.json({ ok: true, client: req.client, pendingService: buildPendingServiceResponse(pending) });
}));

app.post('/api/checkout/create-session', requireAuth, asyncRoute(async (req, res, next) => {
  const service = String(req.body.service || req.body.service_type || req.query.service || '').toLowerCase();
  if (service.includes('appeal')) return app._router.handle({ ...req, url: '/api/appeals/create-checkout-session', originalUrl: req.originalUrl }, res, next);
  if (service.includes('citizen')) return app._router.handle({ ...req, url: '/api/citizenship/create-checkout-session', originalUrl: req.originalUrl }, res, next);
  return app._router.handle({ ...req, url: '/api/assessment/create-checkout-session', originalUrl: req.originalUrl }, res, next);
}));
...
async function handleAppealsAssessmentCreate(req, res) {
  ...
  await query(
    ...
    [
      ...
      req.body.tribunalDeadline || req.body.tribunal_deadline || null,
...
app.post('/api/appeals/create-checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  const assessment = await findAppealAssessmentForCheckout(req);
  if (!assessment) return res.status(404).json({ ok: false, error: 'Appeals assessment was not found for this account. Submit the appeals assessment with the same email address, then log in again.' });
...
app.post('/api/assessment/create-checkout-session', requireAuth, asyncRoute(async (req, res) => {
  if (!stripe) return res.status(500).json({ ok: false, error: 'Stripe is not configured.' });
  const assessment = await findVisaAssessmentForCheckout(req);
  if (!assessment) return res.status(404).json({ ok: false, error: 'Assessment was not found for this account. Submit the assessment with the same email address, then log in again.' });
  await query(
    `UPDATE assessments
     SET client_id=$1, client_email=$2, active_plan=COALESCE(active_plan, selected_plan), updated_at=now()
     WHERE id=$3`,
    [req.client.id, req.client.email, assessment.id]
  );
...
async function finaliseStripePayment(req, res) {
  ...
  const isAppeals = serviceType === 'appeals_assessment' || serviceType === 'appeals';
  const redirectUrl = isCitizenship
    ...
    : isAppeals
      ? `${APP_BASE_URL}/account-dashboard.html?payment=verified&service=appeals&appeal_assessment_id=${encodeURIComponent(result.assessmentId || '')}&session_id=${encodeURIComponent(sessionId)}`
      : `${APP_BASE_URL}/account-dashboard.html?payment=verified&service=visa&assessment_id=${encodeURIComponent(result.assessmentId || '')}&session_id=${encodeURIComponent(sessionId)}`;
  res.json({
    ok: true,
    status: 'paid',
    paymentLinked: true,
