...
async function syncDbServiceDocumentFromSubmission(submission) {
  try {
    ...
    const metaPatch = {
      ...
      pdfUrl: documentUrl,
      pdfStatus: physicalPdfReady ? 'ready' : (submission.pdfStatus || null),
...
async function ensurePaidVisaSubmissionProcessing(submission) {
  if (!submission || !submission.id) return;
  const pdfReady = Boolean(submission.pdfPath && fs.existsSync(submission.pdfPath));
  const processing = ['running', 'generating', 'queued', 'processing'].includes(String(submission.analysisStatus || submission.pdfStatus || submission.status || '').toLowerCase());
  if (pdfReady) {
    await syncDbServiceDocumentFromSubmission(submission);
    return;
  }
  if (submission.paymentStatus === 'paid' && !processing) {
    processSubmission(submission.id).catch(async (error) => {
      console.error('[visa-existing-paid-processing-error]', submission.id, error);
      ...
      const failed = await getSubmission(submission.id);
      if (failed) await syncDbServiceDocumentFromSubmission(failed);
    });
  }
}
...
function publicDashboardAssessment(submission = {}) {
  ...
  const physicalPdfReady = Boolean(submission.pdfPath && fs.existsSync(submission.pdfPath));
  const safePdfUrl = physicalPdfReady ? (submission.pdfUrl || submission.downloadUrl || `/api/assessment/${encodeURIComponent(submission.id)}/pdf`) : null;
  return {
...
app.post('/api/payments/finalise', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    if (product === 'visa_assessment') {
      ...
      verifiedPayment.assessment = submission ? publicDashboardAssessment(submission) : null;
