...
function getUsersStore() {
  ...
  if (!store.byEmail) store.byEmail = {};
  if (!store.byUsername) store.byUsername = {};
  return store;
}
async function saveUsersStore(store) { await writeCitizenshipStore(CITIZENSHIP_USERS_PATH, store); }
...
function assertProductionJwtSecret() {
  if (NODE_ENV === 'production' && (!process.env.JWT_SECRET || String(process.env.JWT_SECRET).length < 32)) {
    const error = new Error('Production security error: set JWT_SECRET to a strong random value of at least 32 characters.');
    error.statusCode = 500;
    throw error;
  }
}
function assertEntitlementBelongsToUser(entitlement, user) {
  if (!entitlement || !user) return false;
  if (entitlement.userId && entitlement.userId !== user.id) return false;
  const entEmail = String(entitlement.customerEmail || '').trim().toLowerCase();
  const userEmail = String(user.email || '').trim().toLowerCase();
  return entitlement.userId === user.id || (entEmail && entEmail === userEmail);
}
function requireRemainingEntitlementForUser(user) {
  const entitlement = findUserEntitlement(user.id);
  if (!entitlement) return null;
  if (!assertEntitlementBelongsToUser(entitlement, user)) return null;
  const total = Number(entitlement.total || 0);
  const used = Number(entitlement.used || 0);
  entitlement.remaining = Math.max(0, total - used);
  return entitlement;
}
...
function publicEntitlement(ent) { if (!ent) return null; return { plan: ent.plan, planLabel: ent.planLabel, total: ent.total, used: ent.used, remaining: Math.max(0, Number(ent.total || 0) - Number(ent.used || 0)), customerEmail: ent.customerEmail || null, userId: ent.userId || null, createdAt: ent.createdAt, updatedAt: ent.updatedAt, results: Array.isArray(ent.results) ? ent.results.slice(-10) : [] }; }
...
app.post('/api/auth/register', async (req, res, next) => {
  try {
    ...
    if (store.byEmail[email] || store.byUsername[username]) return res.status(409).json({ ok: false, error: 'Account already exists. Please login.' });
    const id = safeId('usr');
...
app.post('/api/auth/login', async (req, res, next) => {
  try {
    ...
    const password = String(req.body?.password || '');
    const store = getUsersStore();
    const userId = store.byEmail[login] || store.byUsername[login];
    ...
    user.lastLoginAt = nowIso(); store.users[user.id] = user; await saveUsersStore(store);
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    jwtSecretConfigured: Boolean(process.env.JWT_SECRET && String(process.env.JWT_SECRET).length >= 32),
...
app.get('/checkout/verify-session', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    const sameUser = (sessionUserId && sessionUserId === req.user.id) || (sessionEmail && sessionEmail === userEmail);
    if (!sameUser) return res.status(403).json({ ok: false, error: 'This payment belongs to a different account.' });

    const verified = Boolean(paid && productOk && planMatches && priceMatches);
    const entitlement = verified ? await upsertCitizenshipEntitlementFromSession(session) : null;
    return res.json({
      ok: verified,
      paid,
      verified,
      plan: metadataPlan || requestedPlan || null,
...
app.get('/api/citizenship/entitlement', requireCitizenshipAuth, async (req, res) => { const entitlement = requireRemainingEntitlementForUser(req.user); if (!entitlement) return res.status(404).json({ ok: false, error: 'No paid exam pack is attached to this account.' }); return res.json({ ok: true, entitlement: publicEntitlement(entitlement), user: publicUser(req.user) }); });
...
app.post('/api/citizenship/start-paid', requireCitizenshipAuth, async (req, res, next) => {
  try {
    const plan = String(req.body?.plan || '').trim().toLowerCase();
    const entitlement = requireRemainingEntitlementForUser(req.user);
    if (!entitlement) return res.status(403).json({ ok: false, error: 'Paid entitlement was not found or has not been verified for this account.' });
    if (plan && entitlement.plan !== plan) return res.status(400).json({ ok: false, error: 'Plan does not match paid entitlement.' });
    const remaining = Math.max(0, Number(entitlement.total || 0) - Number(entitlement.used || 0));
    if (remaining <= 0) return res.status(403).json({ ok: false, error: 'No paid attempts remaining.' });
    ...
    return res.json({ ok: true, examId, total: 20, timeLimitSeconds: 45 * 60, entitlement: publicEntitlement(entitlement), questions: questions.map(sanitizeCitizenshipQuestion) });
  } catch (error) { return next(error); }
});
app.post('/api/citizenship/submit-exam', async (req, res, next) => {
  try {
    ...
    const user = optionalCitizenshipAuth(req);
    if (exam.type === 'paid') {
      if (!user) return res.status(401).json({ ok: false, error: 'Login required to submit a paid exam.' });
      if (exam.userId && exam.userId !== user.id) return res.status(403).json({ ok: false, error: 'This paid exam belongs to a different account.' });
    }
    ...
    if (exam.type === 'free') {
      const clientToken = freeClientKey(exam.clientToken);
      const free = store.freeAttempts[clientToken] || { used: 0, total: 3, results: [] };
      free.used = Math.min(3, Number(free.used || 0) + 1);
      free.results = Array.isArray(free.results) ? free.results : [];
      free.results.push({ examId, score: result.score, valuesCorrect: result.valuesCorrect, passed: result.passed, at: nowIso() });
      free.lastQuestionIds = (exam.questions || []).map(questionKey);
      store.freeAttempts[clientToken] = free;
      attempts = { total: 3, used: free.used, remaining: Math.max(0, 3 - free.used), results: free.results.slice(-10) };
    } else if (exam.type === 'paid') {
      const entStore = getEntitlementsStore();
      entitlement = requireRemainingEntitlementForUser(user);
      if (!entitlement) return res.status(403).json({ ok: false, error: 'Paid entitlement was not found for this account.' });
      if (exam.accessToken && entitlement.token !== exam.accessToken) return res.status(403).json({ ok: false, error: 'This exam is not attached to the current paid entitlement.' });
      entitlement.used = Math.min(Number(entitlement.total || 0), Number(entitlement.used || 0) + 1);
      entitlement.remaining = Math.max(0, Number(entitlement.total || 0) - Number(entitlement.used || 0));
      entitlement.results = Array.isArray(entitlement.results) ? entitlement.results : [];
      entitlement.results.push({ examId, score: result.score, valuesCorrect: result.valuesCorrect, passed: result.passed, at: nowIso() });
      entitlement.lastQuestionIds = (exam.questions || []).map(questionKey);
      entitlement.updatedAt = nowIso();
      entStore.entitlements[entitlement.token] = entitlement;
      entStore.byUserId[user.id] = entitlement.token;
      await saveEntitlementsStore(entStore);
    }
    await saveExamStore(store);
    return res.json({ ok: true, result, attempts, entitlement: publicEntitlement(entitlement) });
  } catch (error) { return next(error); }
});
