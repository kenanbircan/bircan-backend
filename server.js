...
app.post(['/create-visa-checkout-session', '/create-assessment-checkout-session', '/api/assessment/create-checkout-session', '/api/assessments/create-checkout-session', '/api/visa/create-checkout-session', '/api/checkout/visa'], async (req, res, next) => {
  try {
    ...
    const extractedForEmailGuard = extractAssessmentPayload(assessmentPayload || req.body || {});
    const assessmentEmail = String(
      extractedForEmailGuard?.client?.email ||
      assessmentPayload?.client?.email ||
      assessmentPayload?.email ||
      req.body?.assessmentEmail ||
      req.body?.email ||
      ''
    ).trim().toLowerCase();

    // Production account rule: the account used for Stripe must be the same
    // email entered in the assessment form. When it is not, return a structured
    // response so the frontend can clear the wrong token and redirect to login
    // with the correct locked email instead of getting stuck on checkout.
    if (assessmentEmail && user.email && assessmentEmail !== String(user.email).trim().toLowerCase()) {
      const nextPath = req.body?.next || req.body?.returnUrl || req.body?.successUrl || '/account-dashboard.html';
      const loginUrl = `${APP_BASE_URL}/login.html?email=${encodeURIComponent(assessmentEmail)}&next=${encodeURIComponent(nextPath)}`;
      return res.status(409).json({
        ok: false,
        code: 'EMAIL_MISMATCH',
        error: `This assessment was completed with ${assessmentEmail}, but you are logged in as ${user.email}. Please login with the same email address used in the assessment form.`,
        expectedEmail: assessmentEmail,
        loggedInEmail: user.email,
        loginUrl,
        action: 'CLEAR_TOKEN_AND_LOGIN_WITH_ASSESSMENT_EMAIL',
      });
    }
