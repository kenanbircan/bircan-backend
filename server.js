...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.1.3-citizenship-finalise-dashboard-hardening',
    postgres: true,
    jsonFallback: false,
...
async function handleCitizenshipCheckoutSession(req, res) {
  ...
  const successUrl = `${APP_BASE_URL}/account-dashboard.html?paid=1&service=citizenship&session_id={CHECKOUT_SESSION_ID}`;
  const cancelUrl = `${APP_BASE_URL}/citizenship-test-stripe-wired.html?cancelled=1&plan=${encodeURIComponent(plan)}`;
...
async function attachPaidCitizenshipSession(session) {
  const md = session.metadata || {};
  let accessId = md.citizenship_access_id || session.client_reference_id || null;
  const email = normaliseEmail(md.client_email || session.customer_email);
  if (!email) throw new Error('Stripe citizenship session is missing client email.');

  let access = null;
  if (accessId) {
    const byId = await query('SELECT * FROM citizenship_access WHERE id=$1', [accessId]);
    access = byId.rows[0] || null;
  }

  // Defensive recovery for older return URLs or earlier checkout builds: if metadata/client_reference_id
  // is missing but the Stripe session id was saved, recover the citizenship record by stripe_session_id.
  if (!access && session.id) {
    const bySession = await query('SELECT * FROM citizenship_access WHERE stripe_session_id=$1 ORDER BY created_at DESC LIMIT 1', [session.id]);
    access = bySession.rows[0] || null;
    accessId = access ? access.id : accessId;
  }

  // Last-resort recovery: if Stripe is paid but the pre-checkout DB row is missing, create one
  // so a verified paying client is not locked out of the exam dashboard.
  if (!access) {
    const clientRows = await query('SELECT id, email, name FROM clients WHERE lower(email)=lower($1) LIMIT 1', [email]);
    const client = clientRows.rows[0];
    if (!client) throw new Error(`Citizenship client account not found for paid Stripe email ${email}.`);
    const planForRecovery = normaliseCitizenshipPlan(md.plan || '20');
    accessId = makeCitizenshipAccessId(planForRecovery);
    const inserted = await query(
      `INSERT INTO citizenship_access (id, client_id, client_email, selected_plan, active_plan, exam_allowance, attempts_used, status, payment_status, stripe_session_id, amount_cents, currency, raw_payload)
       VALUES ($1,$2,$3,$4,$4,$5,0,'checkout_created','unpaid',$6,$7,$8,$9)
       ON CONFLICT (stripe_session_id) DO UPDATE SET updated_at=now()
       RETURNING *`,
      [accessId, client.id, client.email, planForRecovery, citizenshipExamAllowance(planForRecovery), session.id || null, session.amount_total || null, session.currency || 'aud', session]
    );
    access = inserted.rows[0];
    accessId = access.id;
  }
  ...
  return { attached: true, assessmentId: accessId, accessId, citizenshipAccessId: accessId, type: 'citizenship_test', service: 'citizenship', plan, pdfReady: false, paymentAudit };
}
...
async function finaliseStripePayment(req, res) {
  ...
  const service = result.service || result.type || (session.metadata || {}).service_type || 'visa_assessment';
  const isCitizenship = service === 'citizenship' || service === 'citizenship_test';
  const redirectUrl = isCitizenship
    ? `${APP_BASE_URL}/account-dashboard.html?payment=verified&service=citizenship&citizenship=active&access_id=${encodeURIComponent(result.accessId || result.citizenshipAccessId || result.assessmentId || '')}&session_id=${encodeURIComponent(sessionId)}`
  ...
  res.json({
    ...
    status: 'paid',
    paymentLinked: true,
    service: isCitizenship ? 'citizenship' : service,
    sessionId,
    assessmentId: result.assessmentId,
    accessId: result.accessId || null,
    citizenshipAccessId: result.citizenshipAccessId || result.accessId || null,
    plan: result.plan || null,
    pdfReady: result.pdfReady,
    client,
    redirectUrl
...
app.post('/api/payment/finalize', asyncRoute(finaliseStripePayment));
app.get('/api/payments/finalise', asyncRoute(finaliseStripePayment));
app.get('/api/payments/finalize', asyncRoute(finaliseStripePayment));
app.get('/api/payment/finalise', asyncRoute(finaliseStripePayment));
app.get('/api/payment/finalize', asyncRoute(finaliseStripePayment));
