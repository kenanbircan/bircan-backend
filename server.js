...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '10.2.0-reference-resolver-node22',
    postgres: true,
    jsonFallback: false,
    stripeConfigured: Boolean(stripe),
    smtpConfigured: Boolean(process.env.SMTP_HOST),
    appBaseUrl: APP_BASE_URL,
    corsPatch: 'matched-frontend-backend-final-x-auth-token-reference-resolver',
...
async function generatePdfResponse(req, res, assessmentId) {
  ...
  const legacyPdfUrl = `/api/assessment/${encodeURIComponent(result.id || assessmentId)}/pdf`;
  res.json({
    ok: true,
    ready: true,
    final: true,
...
async function existingAssessmentReferenceColumns(dbClient) {
  const executor = dbClient && typeof dbClient.query === 'function' ? dbClient : { query };
  const { rows } = await executor.query(`
    SELECT column_name
    FROM information_schema.columns
    WHERE table_schema = current_schema()
      AND table_name = 'assessments'
      AND column_name IN ('id','submission_id','reference','assessment_reference','service_ref','stripe_session_id')
  `);
  const found = new Set(rows.map(r => r.column_name));
  if (!found.has('id')) found.add('id');
...
async function resolveAssessmentForAccount(dbClient, rawRef, accountEmail = null, options = {}) {
  if (!rawRef) throw new Error('assessmentId is required');
  const executor = dbClient && typeof dbClient.query === 'function' ? dbClient : { query };
  const requestedId = String(rawRef || '').trim();
  const lockClause = options.forUpdate ? ' FOR UPDATE' : '';
  const emailClause = accountEmail ? ' AND lower(client_email)=lower($2)' : '';
  const params = accountEmail ? [requestedId, accountEmail] : [requestedId];

  const columns = await existingAssessmentReferenceColumns(executor);
  const predicates = [];
  for (const col of columns) {
    if (col === 'id') predicates.push('id=$1');
    else predicates.push(`${col}=$1`);
  }

  let rows = (await executor.query(
    `SELECT * FROM assessments WHERE (${predicates.join(' OR ')})${emailClause} ORDER BY created_at DESC LIMIT 1${lockClause}`,
    params
  )).rows;
  if (rows[0]) return rows[0];

  // Older dashboard code sometimes sends a shortened id such as sub_1777591587924_186.
  // Resolve only within the same account when an account email is provided.
  if (/^sub_\d+_[a-z0-9]+$/i.test(requestedId)) {
    const likePattern = escapeLike(requestedId) + '\\_%';
    const likeParams = accountEmail ? [likePattern, accountEmail] : [likePattern];
    rows = (await executor.query(
      `SELECT * FROM assessments WHERE id LIKE $1 ESCAPE '\\'${emailClause} ORDER BY created_at DESC LIMIT 1${lockClause}`,
      likeParams
    )).rows;
    if (rows[0]) return rows[0];
  }

  // Payment audit rows may contain the canonical assessment reference even when the
  // dashboard sends a Stripe/session/payment reference. Resolve that indirection safely.
  try {
    const paymentParams = accountEmail ? [requestedId, accountEmail] : [requestedId];
    const paymentEmailClause = accountEmail ? ' AND lower(client_email)=lower($2)' : '';
    const payRows = (await executor.query(
      `SELECT service_ref
       FROM payments
       WHERE (service_ref=$1 OR stripe_session_id=$1 OR stripe_payment_intent=$1)${paymentEmailClause}
       ORDER BY created_at DESC
       LIMIT 1`,
      paymentParams
    )).rows;
    const serviceRef = payRows[0] && payRows[0].service_ref;
    if (serviceRef && serviceRef !== requestedId) {
      return await resolveAssessmentForAccount(executor, serviceRef, accountEmail, options);
    }
  } catch (err) {
    console.warn('Payment reference fallback skipped:', err.message);
  }

  throw new Error(`Assessment was not found for reference ${requestedId}.`);
}
...
async function generateAssessmentPdfNow(assessmentId, accountEmail = null) {
  ...
  return tx(async (client) => {
    const assessment = await resolveAssessmentForAccount(client, requestedId, accountEmail, { forUpdate: true });
...
app.get('/api/assessment/:id/status', requireAuth, asyncRoute(async (req, res) => {
  const assessment = await resolveAssessmentForAccount(null, req.params.id, req.client.email, { forUpdate: false });
  const { rows } = await query(
...
async function sendAssessmentPdf(req, res, rawId) {
  const resolved = await generateAssessmentPdfNow(rawId, req.client.email);
  const { rows } = await query('SELECT * FROM assessments WHERE id=$1 AND lower(client_email)=lower($2)', [resolved.id, req.client.email]);
  const assessment = rows[0];
  if (!assessment) return res.status(404).json({ ok: false, error: 'Assessment was not found for this account.' });
...
app.post('/api/assessment/:id/email-pdf', requireAuth, asyncRoute(async (req, res) => {
  const generated = await generateAssessmentPdfNow(req.params.id, req.client.email);
  let { rows } = await query('SELECT * FROM assessments WHERE id=$1 AND lower(client_email)=lower($2)', [generated.id, req.client.email]);
