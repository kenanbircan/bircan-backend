...
app.post('/api/assessment/create-checkout-session', requireCitizenshipAuth, async (req, res, next) => {
  try {
    ...
    const clientEmail = String(submission.client?.email || submission.clientEmail || '').trim().toLowerCase();
    const userEmail = String(req.user.email || '').trim().toLowerCase();
    if (clientEmail && userEmail && clientEmail !== userEmail) {
      // Firm account integrity rule. Avoid one account paying for another assessment accidentally.
      return res.status(403).json({ ok: false, error: 'This assessment was submitted with a different email address. Please login with the same email used in the form.' });
    }
    ...
    await updateSubmission(submissionId, { userId: req.user.id, accountEmail: req.user.email, stripeSessionId: session.id, paymentStatus: 'checkout_created', status: 'awaiting_payment' });
