...
app.get("/api/assessment/:id/status", requireAuth, (req, res) => { const item = getSubmission(req.params.id); if (!item) return res.status(404).json({ ok: false, error: "Assessment was not found." }); const itemEmail = normalizeEmail(item?.client?.email || item?.stripeCustomerEmail || item?.metadata?.clientEmail || item?.metadata?.applicantEmail); if (itemEmail !== normalizeEmail(req.user.email)) return res.status(403).json({ ok: false, error: "This assessment does not belong to the logged-in account." }); res.json({ ok: true, submission: item, ...item }); });
