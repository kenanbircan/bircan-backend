...
function normalizeVisaCheckoutPlan(input) {
  ...
  if (raw === 'instant' || raw === 'same-day' || raw === 'same day') return normalizePlan('instant');
  if (raw === '3-days' || raw === '3-day' || raw === '3d' || raw === '3 days') return normalizePlan('3d');
  if (raw === '24-hours' || raw === '24-hour' || raw === '24h' || raw === '24 hours') return normalizePlan('24h');
...
async function createVisaAssessmentFromPaidSession(session, existingPayload = null) {
  ...
  const payload = existingPayload || readJsonSafe(path.join(STORAGE_DIR, 'visa-checkout-payloads', `${sanitizeFileName(token)}.json`), null);
...
app.post('/create-visa-checkout-session', async (req, res, next) => {
  try {
    ...
    const plan = normalizeVisaCheckoutPlan(req.body?.plan || assessmentPayload.plan || assessmentPayload?.metadata?.selectedPlanKey || '24h');
    const priceId = getVisaStripePriceIdForPlan(plan.code);

    const successRaw = makeStripeSafeUrl(req.body?.successUrl, '/');
    const successUrl = successRaw.includes('{CHECKOUT_SESSION_ID}')
      ? successRaw
      : `${successRaw}${successRaw.includes('?') ? '&' : '?'}session_id={CHECKOUT_SESSION_ID}&visa_paid=1`;
    const cancelUrl = makeStripeSafeUrl(req.body?.cancelUrl, '/');
...
app.get('/checkout/verify-visa-session', async (req, res, next) => {
  try {
    assertStripeConfigured();
    assertStripeModeLooksConsistent();
    const sessionId = String(req.query?.session_id || req.query?.sessionId || '').trim();
    const requestedPlan = String(req.query?.plan || '').trim().toLowerCase();
    ...
    const planMatches = !requestedPlan || requestedPlan === plan.code || requestedPlan === String(metadata.plan || '').toLowerCase() || requestedPlan.replace(/-/g, '') === plan.code.replace(/-/g, '');
