...
function normaliseCitizenshipPlan(plan) {
  const rawOriginal = String(plan || '').trim();
  const raw = rawOriginal.toLowerCase().replace(/\s+/g, '').replace(/-/g, '').replace(/\$/g, '').replace(/^a/, '').replace(/aud/g, '');

  // Backend hardening:
  // Citizenship packages are exam packages, not visa turnaround plans.
  // Stripe pricing aliases:
  //   20 exams  -> A$50
  //   50 exams  -> A$100
  //   100 exams -> A$150
  //   unlimited -> configured unlimited price
  if (['20', '20exam', '20exams', 'starter', '50price', 'price50'].includes(raw)) return '20';
  if (['50', '50exam', '50exams', 'standard', '100price', 'price100'].includes(raw)) return '50';
  if (['100', '100exam', '100exams', 'premium', '150', '15000', '150price', 'price150', '3d', '3day', '3days', 'value'].includes(raw)) return '100';
  if (['unlimited', 'unlimitedexams', 'unlimitedexam', 'all', '200', '20000', '200price', 'price200'].includes(raw)) return 'unlimited';

  // Extra safety for labels sent from frontend cards.
  const label = rawOriginal.toLowerCase();
  if (label.includes('100 exams') || label.includes('a$150') || label.includes('$150') || label.includes('150.00')) return '100';
  if (label.includes('50 exams') || label.includes('a$100') || label.includes('$100') || label.includes('100.00')) return '50';
  if (label.includes('20 exams') || label.includes('a$50') || label.includes('$50') || label.includes('50.00')) return '20';
  if (label.includes('unlimited')) return 'unlimited';
...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '12.2.1-citizenship-150-plan-fix',
    postgres: true,
    jsonFallback: false,
...
// Citizenship plan mapping diagnostic. Safe: no Stripe session is created.
app.post('/api/debug/citizenship-plan', (req, res) => {
  const rawPlan = req.body.citizenshipPlan || req.body.citizenship_plan || req.body.examPlan || req.body.exam_plan || req.body.packageId || req.body.package_id || req.body.package || req.body.selectedPlan || req.body.selected_plan || req.body.planId || req.body.plan_id || req.body.pricePlan || req.body.price_plan || req.body.plan;
  res.json({
    ok: true,
    rawPlan: rawPlan || null,
    normalisedCitizenshipPlan: normaliseCitizenshipPlan(rawPlan || '20'),
    note: '20=A$50, 50=A$100, 100=A$150, unlimited=unlimited package'
  });
});
...
app.post('/api/service/checkout-session', requireAuth, asyncRoute(async (req, res) => {
  ...
  if (serviceSession.service_type === 'citizenship_test') {
    const requestedCitizenshipPlan = req.body.citizenshipPlan || req.body.citizenship_plan || req.body.examPlan || req.body.exam_plan || req.body.packageId || req.body.package_id || req.body.package || req.body.selectedPlan || req.body.selected_plan || req.body.planId || req.body.plan_id || req.body.pricePlan || req.body.price_plan || req.body.plan;
    const plan = normaliseCitizenshipPlan(requestedCitizenshipPlan || serviceSession.selected_plan || '20');
...
app.post('/api/service/start', asyncRoute(async (req, res) => {
  ...
  const plan = normaliseCitizenshipPlan(req.body.citizenshipPlan || req.body.citizenship_plan || req.body.examPlan || req.body.exam_plan || req.body.packageId || req.body.package_id || req.body.package || req.body.selectedPlan || req.body.selected_plan || req.body.planId || req.body.plan_id || req.body.pricePlan || req.body.price_plan || req.body.plan || '20');
