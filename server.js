...
function drawHeader(doc, submission, analysis) {
  const lp = logoPath();
  doc.save();
  doc.roundedRect(34, 26, 527, 104, 16).fill('#0b1f3a');

  if (fs.existsSync(lp)) {
    try {
      doc.image(lp, 48, 40, { fit: [140, 76], align: 'left', valign: 'center' });
    } catch (_) {}
  }

  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(18)
    .text('Preliminary Visa Assessment Letter', 220, 44, { width: 300, align: 'left' });

  doc.fillColor('#cfe0ff').font('Helvetica').fontSize(10)
    .text('Bircan Migration & Education', 220, 70, { width: 300, align: 'left' })
    .text(`${submission.visaType || 'Subclass 482'} | ${analysis.eligibilityOutcome || 'Preliminary assessment'}`, 220, 86, { width: 300, align: 'left' });
...
function drawFooterOnCurrentPage(doc, pageNumber, pageCount) {
  ...
  doc.save();
  doc.moveTo(40, footerY - 12).lineTo(555, footerY - 12).strokeColor('#dce4ef').stroke();
  doc.fillColor('#5f6b7d').font('Helvetica').fontSize(8.5);
  doc.text('Bircan Migration & Education | Preliminary migration assessment letter', 40, footerY, {
    width: 330,
    lineBreak: false
  });
  doc.text('This document is preliminary in nature and subject to final review.', 250, footerY, {
    width: 180,
    align: 'center',
    lineBreak: false
  });
  doc.text(`Page ${pageNumber} of ${pageCount}`, 450, footerY, {
    width: 105,
    align: 'right',
    lineBreak: false
  });
...
function ensureSpace(doc, submission, analysis, threshold = 710) {
  if (doc.y > threshold) {
    doc.addPage();
...
function addSectionTitle(doc, submission, analysis, title) {
  ...
  doc.moveTo(40, doc.y + 3).lineTo(555, doc.y + 3).strokeColor('#2f6fed').stroke();
  doc.moveDown(0.5);
}
...
function addBulletList(doc, submission, analysis, items, fallbackText = 'Not provided.') {
  ...
  list.forEach(item => {
    ...
    const y = doc.y;
    doc.circle(46, y + 7, 2.2).fill('#2f6fed');
    doc.fillColor('#172033').font('Helvetica').fontSize(10.5).text(String(item || ''), 56, y, {
      width: 495,
      lineGap: 3,
    });
    doc.moveDown(0.38);
...
function addMetaGrid(doc, submission, analysis, rows) {
  rows.forEach((row, index) => {
    ensureSpace(doc, submission, analysis, 730);
    const y = doc.y;
    doc.roundedRect(40, y - 2, 515, 25, 5).fill(index % 2 === 0 ? '#f6f9fc' : '#ffffff');
    doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(9.8).text(row[0], 50, y + 6, {
      width: 150,
      lineBreak: false
    });
    doc.fillColor('#172033').font('Helvetica').fontSize(9.8).text(row[1], 205, y + 6, {
      width: 335
    });
    doc.y = y + 27;
  });
...
function addOutcomePanel(doc, submission, analysis) {
  ensureSpace(doc, submission, analysis, 690);
  const boxY = doc.y;
  doc.roundedRect(40, boxY, 515, 76, 10).fill('#eef4ff');
  doc.fillColor('#0b1f3a').font('Helvetica-Bold').fontSize(12).text('Preliminary Outcome', 56, boxY + 14);
  doc.fillColor('#173e8a').font('Helvetica-Bold').fontSize(14).text(analysis.eligibilityOutcome || 'Needs professional review', 56, boxY + 33, {
    width: 470
  });
  doc.fillColor('#5f6b7d').font('Helvetica').fontSize(9.5).text('This outcome is preliminary only and subject to full documentary review, sponsorship and nomination outcomes, and departmental assessment.', 56, boxY + 54, {
    width: 470
  });
  doc.y = boxY + 92;
}

function addRatingsTable(doc, submission, analysis) {
  ...
  addMetaGrid(doc, submission, analysis, rows);
}
...
async function generateProfessionalPdf(submission) {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.fillColor('#172033').font('Helvetica').fontSize(10.5)
      .text(`Date: ${todayHuman()}`)
      .text(`Client: ${submission.client?.fullName || 'Client'}`)
      .text(`Matter: ${submission.visaType || 'Subclass 482'} preliminary assessment`)
      .moveDown(0.25);

    addOutcomePanel(doc, submission, analysis);

    addMetaGrid(doc, submission, analysis, [
      ['Submission ID', submission.id],
      ['Client email', submission.client?.email || 'Not provided'],
      ['Nationality', submission.client?.nationality || 'Not provided'],
      ['Date of birth', submission.client?.dob || 'Not provided'],
      ['Service plan', submission.plan?.label || 'Not provided'],
    ]);

    addSectionTitle(doc, submission, analysis, '1. Executive Summary');
    addParagraph(doc, submission, analysis, analysis.executiveSummary || submission.analysis?.summary || 'Assessment summary not available.');

    addSectionTitle(doc, submission, analysis, '2. Preliminary Advice');
    ...
    addSectionTitle(doc, submission, analysis, '3. Strengths Supporting the Case');
    ...
    addSectionTitle(doc, submission, analysis, '4. Key Risks, Limitations, or Concerns');
    ...
    addSectionTitle(doc, submission, analysis, '5. Further Information or Evidence Required');
    addBulletList(doc, submission, analysis, analysis.missingInformation, 'No additional information items were identified.');

    addSectionTitle(doc, submission, analysis, '6. Recommended Next Steps');
    addBulletList(doc, submission, analysis, analysis.recommendedNextSteps, 'No specific next steps were generated.');

    addSectionTitle(doc, submission, analysis, '7. Suggested Supporting Document Checklist');
    addBulletList(doc, submission, analysis, analysis.documentChecklist, 'No document checklist items were generated.');

    addSectionTitle(doc, submission, analysis, '8. Factual Assumptions Used for this Preliminary View');
    addBulletList(doc, submission, analysis, analysis.factualAssumptions, 'No specific factual assumptions were listed.');

    addSectionTitle(doc, submission, analysis, '9. Assessment Ratings Snapshot');
    addRatingsTable(doc, submission, analysis);

    addSectionTitle(doc, submission, analysis, '10. Questionnaire Record');
    addBulletList(doc, submission, analysis, answers.map(item => `${item.question}: ${item.answer || 'No answer provided'}`), 'No answers received.');

    addSectionTitle(doc, submission, analysis, '11. Important Notice');
    addParagraph(doc, submission, analysis, analysis.disclaimer);

    addParagraph(doc, submission, analysis, 'Yours faithfully,');
    addParagraph(doc, submission, analysis, 'Bircan Migration & Education');
