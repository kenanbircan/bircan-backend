...
// ---- Subclass 190 production legal engine + direct PDF renderer ----
// This path deliberately does NOT call generateMigrationAdvice() or buildAssessmentPdfBuffer().
// It prevents the old pdf.js error: "Advice-grade PDF generation requires GPT adviceBundle".
function getPayloadFlat(assessment) {
  const payload = isPlainObject(assessment && assessment.form_payload) ? assessment.form_payload : {};
  const answers = isPlainObject(payload.answers) ? payload.answers : payload;
  return flattenObject(answers || {});
}
...
function flatFind(flat, patterns) {
  const entries = Object.entries(flat || {});
  for (const pat of patterns) {
    const rx = pat instanceof RegExp ? pat : new RegExp(String(pat), 'i');
    const found = entries.find(([k, v]) => rx.test(k) && valueText(v));
    if (found) return valueText(found[1]);
  }
  return '';
}

function boolishYes(v) {
  const s = valueText(v).toLowerCase();
  return ['yes', 'y', 'true', 'held', 'approved', 'valid', 'current', 'positive', 'met'].includes(s);
}

function boolishBad(v) {
  const s = valueText(v).toLowerCase();
  return ['no', 'false', 'none', 'withdrawn', 'expired', 'refused', 'failed', 'not met', 'unclear', 'unknown', 'unsure', 'pending'].some(x => s.includes(x));
}

function build190LegalDecision(assessment) {
  const flat = getPayloadFlat(assessment);
  const dob = flatFind(flat, [/date.*birth/i, /dob/i, /birth/i]) || '';
  const invitation = flatFind(flat, [/invitation.*received/i, /skillselect.*invitation/i, /invitation.*held/i, /^invitation$/i]);
  const invitationDate = flatFind(flat, [/invitation.*date/i]);
  const nomination = flatFind(flat, [/nomination.*status/i, /state.*nomination/i, /territory.*nomination/i, /nominated.*state/i]);
  const skills = flatFind(flat, [/skills.*assessment/i, /skill.*assessment/i, /assessment.*authority/i]);
  const english = flatFind(flat, [/english/i, /ielts/i, /pte/i, /passport.*english/i]);
  const points = flatFind(flat, [/points.*total/i, /claimed.*points/i, /pass.*mark/i, /points/i]);
  const occupation = flatFind(flat, [/occupation/i, /anzsco/i]) || 'Not provided';
  const s48 = flatFind(flat, [/section.?48/i, /no further stay/i, /8503/i, /8534/i, /8535/i]);
  const health = flatFind(flat, [/health/i, /medical/i]);
  const character = flatFind(flat, [/character/i, /police/i, /criminal/i]);
  const integrity = flatFind(flat, [/4020/i, /integrity/i, /false/i, /misleading/i]);
  const family = flatFind(flat, [/family/i, /partner/i, /child/i, /dependent/i, /custody/i]);

  const findings = [];
  function add(ruleId, criterion, status, legalEffect, evidence, consequence, recommendation, severity='high') {
    findings.push({ ruleId, criterion, status, legalEffect, evidence, consequence, recommendation, severity });
  }

  if (!boolishYes(invitation)) {
    add('190_INVITATION', 'Valid SkillSelect invitation', 'fail', 'INVALID APPLICATION',
      'No valid SkillSelect invitation document is verified from the stored assessment payload.',
      'A Subclass 190 application must not be treated as lodgeable unless a valid invitation is held and verified.',
      'Obtain the SkillSelect invitation letter showing subclass, invitation date, nominated occupation and points score.', 'blocker');
  } else {
    add('190_INVITATION', 'Valid SkillSelect invitation', 'pass', 'REQUIRES DOCUMENT VERIFICATION',
      `Client answer indicates invitation: ${invitation}.`, 'The answer may support lodgement only after the invitation letter is verified.', 'Verify the original SkillSelect invitation letter before lodgement.', 'medium');
  }

  if (!boolishYes(nomination) || /withdrawn|expired|refused|pending|unknown|unsure/i.test(nomination)) {
    add('190_NOMINATION', 'Current state or territory nomination', 'fail', 'INVALID APPLICATION',
      `No current nomination approval is verified. Recorded nomination answer: ${nomination || 'not provided'}.`,
      'A missing, withdrawn or unverified nomination is a blocking issue for the Subclass 190 pathway.',
      'Obtain current state or territory nomination approval confirming nominated occupation and currency.', 'blocker');
  } else {
    add('190_NOMINATION', 'Current state or territory nomination', 'pass', 'REQUIRES DOCUMENT VERIFICATION',
      `Client answer indicates nomination: ${nomination}.`, 'Nomination may be relied on only after the approval letter is verified.', 'Verify nomination approval and matching occupation.', 'medium');
  }

  if (!boolishYes(skills) || boolishBad(skills)) {
    add('190_SKILLS', 'Suitable skills assessment', 'fail', 'REFUSAL LIKELY',
      `No positive skills assessment outcome letter is verified. Recorded answer: ${skills || 'not provided'}.`,
      'The application is likely to fail if a suitable skills assessment for the nominated occupation is not held at the required time.',
      'Obtain and verify the skills assessment outcome, authority, occupation, date and validity.', 'critical');
  } else {
    add('190_SKILLS', 'Suitable skills assessment', 'pass', 'REQUIRES DOCUMENT VERIFICATION',
      `Client answer indicates skills assessment: ${skills}.`, 'The criterion may be satisfied subject to document verification.', 'Verify authority, occupation match and validity at invitation.', 'medium');
  }

  if (!boolishYes(english) && !/passport|competent|proficient|superior|ielts|pte|toefl|oet/i.test(english)) {
    add('190_ENGLISH', 'Competent English', 'unknown', 'EVIDENCE GAP',
      'No English test result or eligible passport evidence is verified.',
      'Competent English cannot be safely accepted until evidence is reviewed.',
      'Obtain English test results or eligible passport evidence.', 'high');
  } else {
    add('190_ENGLISH', 'Competent English', 'unknown', 'REQUIRES DOCUMENT VERIFICATION',
      `English answer recorded: ${english}.`, 'The answer cannot be finally accepted without evidence.', 'Verify test scores, test date or passport basis.', 'medium');
  }

  const pointsNumber = Number((points || '').match(/\d+/)?.[0]);
  if (!pointsNumber || pointsNumber < 65) {
    add('190_POINTS', 'Points test threshold', 'fail', pointsNumber ? 'REFUSAL LIKELY' : 'EVIDENCE GAP',
      `No evidenced points score of at least 65 is verified. Recorded answer: ${points || 'not provided'}.`,
      'The matter should not proceed until a full evidenced points calculation confirms the pass mark.',
      'Prepare a points schedule with evidence for each claim.', 'critical');
  } else {
    add('190_POINTS', 'Points test threshold', 'unknown', 'REQUIRES DOCUMENT VERIFICATION',
      `Recorded points answer: ${points}.`, 'Points can be accepted only after each claim is evidenced.', 'Verify all points claims against documents.', 'medium');
  }

  add('190_OCCUPATION', 'Nominated occupation alignment', occupation === 'Not provided' ? 'unknown' : 'unknown', 'EVIDENCE GAP',
    `Recorded occupation: ${occupation}.`,
    'Occupation alignment with the invitation, skills assessment and state nomination must be verified.',
    'Confirm ANZSCO, skilled list eligibility and matching occupation across all documents.', 'high');

  if (/yes|present|condition|bar|48|8503|8534|8535|unresolved/i.test(s48) && !/^no$/i.test(s48)) {
    add('190_SECTION48', 'Section 48 / No Further Stay / onshore validity restrictions', 'fail', 'INVALID APPLICATION',
      `Onshore restriction answer indicates risk: ${s48}.`,
      'If a relevant bar or No Further Stay condition applies, lodgement may be invalid unless a lawful pathway or waiver applies.',
      'Review VEVO, current visa grant, refusal/cancellation history and any waiver decision before lodgement.', 'blocker');
  } else {
    add('190_SECTION48', 'Section 48 / No Further Stay / onshore validity restrictions', 'unknown', 'REQUIRES DOCUMENT VERIFICATION',
      `Recorded answer: ${s48 || 'not provided'}.`, 'Onshore validity must be verified from current visa and Department records.', 'Verify VEVO and all relevant visa notices.', 'medium');
  }

  add('190_HEALTH', 'Health requirement', /yes|issue|condition|review/i.test(health) ? 'risk' : 'unknown', 'DISCRETIONARY RISK',
    `Health answer: ${health || 'not provided'}.`, 'Health position remains subject to examination and any waiver analysis if applicable.', 'Obtain health examination results and relevant medical evidence.', 'medium');
  add('190_CHARACTER', 'Character requirement', /yes|issue|criminal|court|review/i.test(character) ? 'risk' : 'unknown', 'DISCRETIONARY RISK',
    `Character answer: ${character || 'not provided'}.`, 'Character position must be assessed before final strategy.', 'Obtain police certificates and court documents if relevant.', 'medium');
  add('190_PIC4020', 'Integrity / PIC 4020 risk', /yes|issue|false|misleading|review/i.test(integrity) ? 'risk' : 'unknown', 'SERIOUS RISK',
    `Integrity answer: ${integrity || 'not provided'}.`, 'Integrity concerns may create a serious refusal risk and must be reviewed before lodgement.', 'Review prior applications, Department correspondence and all previously submitted documents.', 'high');
  add('190_FAMILY', 'Family members / secondary applicants', family ? 'unknown' : 'pass', family ? 'EVIDENCE GAP' : 'NO CURRENT ISSUE IDENTIFIED',
    family ? `Family answer: ${family}.` : 'No included family issue is verified from the payload.',
    family ? 'Relationship, dependency and custody evidence may be required.' : 'No current secondary applicant issue is identified from the stored answer set.',
    family ? 'Verify family composition and supporting civil/custody documents.' : 'Re-check if family members are later included.', 'low');
  ...
  if (blockers.length) { lodgementPosition = 'NOT LODGEABLE'; riskLevel = 'CRITICAL'; }
  else if (criticalFails.length) { lodgementPosition = 'LODGEABLE_HIGH_RISK'; riskLevel = 'HIGH'; }

  return {
    engine: 'subclass-190-deterministic-legal-engine',
    version: '190-direct-pdf-v3',
    generatedAt: new Date().toISOString(),
    riskLevel,
    lodgementPosition,
    validity: {
      result: blockers.length ? 'INVALID APPLICATION / NOT LODGEABLE ON CURRENT INFORMATION' : 'VALIDITY NOT FINALLY CONFIRMED',
      blockers: blockers.map(b => b.criterion)
    },
    primaryReason: blockers[0]?.criterion || criticalFails[0]?.criterion || 'Evidence gaps require review',
    findings,
    evidenceRequired: [
      'SkillSelect invitation letter showing subclass, invitation date, occupation and points score',
      'State or territory nomination approval letter confirming current nomination',
      'Skills assessment outcome letter showing authority, occupation, date and validity',
      'Passport biodata page and identity evidence',
      'English test result or eligible passport evidence',
      'Full points schedule and documents supporting each points claim',
      'Current visa grant notice and VEVO check',
      'Any refusal, cancellation, section 48 or No Further Stay waiver documents',
      'Health examination results and medical reports if relevant',
      'Police certificates, court records and character material if relevant',
      'Prior visa/application records and Department correspondence for integrity review',
      'Family relationship, custody and dependency documents if family members are included'
    ]
  };
}
...
function wrapPdfText(text, width) {
  ...
  for (const word of words) {
    ...
    if (test.length > width && line) { lines.push(line); line = word; }
    else line = test;
  }
...
function buildSimplePdfBuffer(pages) {
  ...
  const pageIds = [];
  const pagesIdPlaceholder = 'PAGES_ID_PLACEHOLDER';
  for (const pageLines of pages) {
    let stream = 'BT\n/F1 10 Tf\n50 790 Td\n14 TL\n';
    for (const item of pageLines) {
      if (item === '__BOLD__') { stream += '/F1 12 Tf\n'; continue; }
      if (item === '__NORMAL__') { stream += '/F1 10 Tf\n'; continue; }
      stream += `(${pdfEscape(item)}) Tj\nT*\n`;
    }
    stream += 'ET\n';
...
function buildSubclass190LegalPdfBuffer(assessment) {
  const decision = build190LegalDecision(assessment);
  const lines = [];
  const applicant = assessment.applicant_name || assessment.applicant_email || assessment.client_email || 'Client';
  function h(s) { lines.push('__BOLD__', s, '__NORMAL__'); }
  function p(s) { for (const line of wrapPdfText(s, 88)) lines.push(line); lines.push(''); }
  h('Bircan Migration');
  h('Preliminary Migration Advice - Subclass 190 Skilled Nominated Visa');
  p(`Reference: ${assessment.id}`);
  p(`Client email: ${assessment.client_email || ''}`);
  p(`Applicant: ${applicant}`);
  p(`Subclass: 190`);
  p(`Risk level: ${decision.riskLevel}`);
  p(`Lodgement position: ${decision.lodgementPosition}`);
  p(`Generated: ${new Date().toLocaleString('en-AU', { timeZone: 'Australia/Sydney' })}`);
  h('Application validity assessment');
  p(`Result: ${decision.validity.result}`);
  p(`Primary reason: ${decision.primaryReason}`);
  if (decision.validity.blockers.length) p(`Blocking issues: ${decision.validity.blockers.join('; ')}.`);
  h('Scope and compliance safeguards');
  p('This is a preliminary written assessment based on the questionnaire information stored in the client portal. It is subject to identity verification, review of original supporting documents, conflict checks, service agreement, Consumer Guide and fee disclosure compliance, and confirmation of current law and policy before any lodgement action.');
  h('Summary of advice');
  p(decision.lodgementPosition === 'NOT LODGEABLE'
    ? 'On the information currently available, this matter is not lodgeable. The identified blocking issues must be resolved before any Subclass 190 application is prepared or lodged.'
    : 'On the information currently available, the matter cannot be treated as ready for lodgement until the evidence gaps and legal risks identified below are resolved.');
  h('Criterion-by-criterion findings');
  for (const f of decision.findings) {
    h(f.criterion);
    p(`Finding: ${f.status.toUpperCase()}.`);
    p(`Evidence position: ${f.evidence}`);
    p(`Legal classification: ${f.legalEffect}.`);
    p(`Consequence: ${f.consequence}`);
    p(`Recommendation: ${f.recommendation}`);
  }
  h('Evidence required before final advice or lodgement');
  for (const item of decision.evidenceRequired) p(`- ${item}`);
  h('Recommended next steps');
  p('Do not lodge a Subclass 190 application while any invalidity blocker remains unresolved. Obtain and review all listed evidence, then complete a final migration agent review before any immigration assistance proceeds to lodgement action.');
  h('Important qualification');
  p('This preliminary assessment is not a guarantee of visa grant and is not a substitute for final advice after document review. The legal position may change after review of original documents or changes to law and policy.');
  h('Yours faithfully,');
  p('Kenan Bircan JP | Registered Migration Agent | MARN: 1463685 | Bircan Migration & Education');
...
function isSubclass190Assessment(assessment) {
  return String(assessment && assessment.visa_type || '').replace(/[^0-9]/g, '') === '190';
}
...
app.get('/api/health', asyncRoute(async (_req, res) => {
  ...
  res.json({
    ...
    version: '11.2.0-subclass-190-direct-legal-pdf-no-gpt-bundle',
    postgres: true,
    jsonFallback: false,
...
async function generateAssessmentPdfNow(assessmentId, accountEmail = null, options = {}) {
  ...
  return tx(async (client) => {
    ...
    let pdf;
    try {
      if (isSubclass190Assessment(assessment)) {
        // Subclass 190 is rendered by the deterministic legal engine.
        // It does not require GPT adviceBundle and cannot fall back to weak template PDF generation.
        pdf = buildSubclass190LegalPdfBuffer(assessment);
      } else {
        const adviceBundle = await generateMigrationAdvice(assessment);
        pdf = await buildAssessmentPdfBuffer(assessment, adviceBundle);
      }
    } catch (err) {
      // Final safety net: if any older path still throws the GPT adviceBundle error for 190,
      // issue the deterministic 190 legal-engine PDF instead of failing payment finalisation.
      if (isSubclass190Assessment(assessment) && /adviceBundle|Weak template PDF generation/i.test(String(err && err.message || ''))) {
        pdf = buildSubclass190LegalPdfBuffer(assessment);
      } else {
